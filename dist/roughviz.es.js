function es(i, t) {
  return i < t ? -1 : i > t ? 1 : i >= t ? 0 : NaN;
}
function ss(i) {
  let t = i, e = i;
  function s(n, a, h, r) {
    for (h == null && (h = 0), r == null && (r = n.length); h < r; ) {
      const o = h + r >>> 1;
      e(n[o], a) < 0 ? h = o + 1 : r = o;
    }
    return h;
  }
  return i.length === 1 && (t = (n, a) => i(n) - a, e = function(n) {
    return (a, h) => es(n(a), h);
  }(i)), { left: s, center: function(n, a, h, r) {
    h == null && (h = 0), r == null && (r = n.length);
    const o = s(n, a, h, r - 1);
    return o > h && t(n[o - 1], a) > -t(n[o], a) ? o - 1 : o;
  }, right: function(n, a, h, r) {
    for (h == null && (h = 0), r == null && (r = n.length); h < r; ) {
      const o = h + r >>> 1;
      e(n[o], a) > 0 ? r = o : h = o + 1;
    }
    return h;
  } };
}
const Ds = ss(es).right;
ss(function(i) {
  return i === null ? NaN : +i;
}).center;
const ns = Ds;
function et(i, t) {
  let e, s;
  if (t === void 0)
    for (const n of i)
      n != null && (e === void 0 ? n >= n && (e = s = n) : (e > n && (e = n), s < n && (s = n)));
  else {
    let n = -1;
    for (let a of i)
      (a = t(a, ++n, i)) != null && (e === void 0 ? a >= a && (e = s = a) : (e > a && (e = a), s < a && (s = a)));
  }
  return [e, s];
}
var Ii = Math.sqrt(50), Li = Math.sqrt(10), Zi = Math.sqrt(2);
function fe(i, t, e) {
  var s = (t - i) / Math.max(0, e), n = Math.floor(Math.log(s) / Math.LN10), a = s / Math.pow(10, n);
  return n >= 0 ? (a >= Ii ? 10 : a >= Li ? 5 : a >= Zi ? 2 : 1) * Math.pow(10, n) : -Math.pow(10, -n) / (a >= Ii ? 10 : a >= Li ? 5 : a >= Zi ? 2 : 1);
}
function q(i, t) {
  let e;
  if (t === void 0)
    for (const s of i)
      s != null && (e < s || e === void 0 && s >= s) && (e = s);
  else {
    let s = -1;
    for (let n of i)
      (n = t(n, ++s, i)) != null && (e < n || e === void 0 && n >= n) && (e = n);
  }
  return e;
}
function ut(i, t) {
  let e;
  if (t === void 0)
    for (const s of i)
      s != null && (e > s || e === void 0 && s >= s) && (e = s);
  else {
    let s = -1;
    for (let n of i)
      (n = t(n, ++s, i)) != null && (e > n || e === void 0 && n >= n) && (e = n);
  }
  return e;
}
function rs(i, t, e) {
  i = +i, t = +t, e = (n = arguments.length) < 2 ? (t = i, i = 0, 1) : n < 3 ? 1 : +e;
  for (var s = -1, n = 0 | Math.max(0, Math.ceil((t - i) / e)), a = new Array(n); ++s < n; )
    a[s] = i + s * e;
  return a;
}
var wi = Array.prototype.slice;
function qs(i) {
  return i;
}
var pe = 1e-6;
function Hs(i) {
  return "translate(" + (i + 0.5) + ",0)";
}
function Ns(i) {
  return "translate(0," + (i + 0.5) + ")";
}
function Xs(i) {
  return function(t) {
    return +i(t);
  };
}
function Ks(i) {
  var t = Math.max(0, i.bandwidth() - 1) / 2;
  return i.round() && (t = Math.round(t)), function(e) {
    return +i(e) + t;
  };
}
function Qs() {
  return !this.__axis;
}
function as(i, t) {
  var e = [], s = null, n = null, a = 6, h = 6, r = 3, o = i === 1 || i === 4 ? -1 : 1, l = i === 4 || i === 2 ? "x" : "y", u = i === 1 || i === 3 ? Hs : Ns;
  function c(d) {
    var g = s ?? (t.ticks ? t.ticks.apply(t, e) : t.domain()), p = n ?? (t.tickFormat ? t.tickFormat.apply(t, e) : qs), f = Math.max(a, 0) + r, A = t.range(), m = +A[0] + 0.5, y = +A[A.length - 1] + 0.5, b = (t.bandwidth ? Ks : Xs)(t.copy()), k = d.selection ? d.selection() : d, S = k.selectAll(".domain").data([null]), v = k.selectAll(".tick").data(g, t).order(), M = v.exit(), C = v.enter().append("g").attr("class", "tick"), j = v.select("line"), z = v.select("text");
    S = S.merge(S.enter().insert("path", ".tick").attr("class", "domain").attr("stroke", "currentColor")), v = v.merge(C), j = j.merge(C.append("line").attr("stroke", "currentColor").attr(l + "2", o * a)), z = z.merge(C.append("text").attr("fill", "currentColor").attr(l, o * f).attr("dy", i === 1 ? "0em" : i === 3 ? "0.71em" : "0.32em")), d !== k && (S = S.transition(d), v = v.transition(d), j = j.transition(d), z = z.transition(d), M = M.transition(d).attr("opacity", pe).attr("transform", function(F) {
      return isFinite(F = b(F)) ? u(F) : this.getAttribute("transform");
    }), C.attr("opacity", pe).attr("transform", function(F) {
      var w = this.parentNode.__axis;
      return u(w && isFinite(w = w(F)) ? w : b(F));
    })), M.remove(), S.attr("d", i === 4 || i == 2 ? h ? "M" + o * h + "," + m + "H0.5V" + y + "H" + o * h : "M0.5," + m + "V" + y : h ? "M" + m + "," + o * h + "V0.5H" + y + "V" + o * h : "M" + m + ",0.5H" + y), v.attr("opacity", 1).attr("transform", function(F) {
      return u(b(F));
    }), j.attr(l + "2", o * a), z.attr(l, o * f).text(p), k.filter(Qs).attr("fill", "none").attr("font-size", 10).attr("font-family", "sans-serif").attr("text-anchor", i === 2 ? "start" : i === 4 ? "end" : "middle"), k.each(function() {
      this.__axis = b;
    });
  }
  return c.scale = function(d) {
    return arguments.length ? (t = d, c) : t;
  }, c.ticks = function() {
    return e = wi.call(arguments), c;
  }, c.tickArguments = function(d) {
    return arguments.length ? (e = d == null ? [] : wi.call(d), c) : e.slice();
  }, c.tickValues = function(d) {
    return arguments.length ? (s = d == null ? null : wi.call(d), c) : s && s.slice();
  }, c.tickFormat = function(d) {
    return arguments.length ? (n = d, c) : n;
  }, c.tickSize = function(d) {
    return arguments.length ? (a = h = +d, c) : a;
  }, c.tickSizeInner = function(d) {
    return arguments.length ? (a = +d, c) : a;
  }, c.tickSizeOuter = function(d) {
    return arguments.length ? (h = +d, c) : h;
  }, c.tickPadding = function(d) {
    return arguments.length ? (r = +d, c) : r;
  }, c;
}
function Yt(i) {
  return as(3, i);
}
function Dt(i) {
  return as(4, i);
}
var me = {}, Si = {};
function Ae(i) {
  return new Function("d", "return {" + i.map(function(t, e) {
    return JSON.stringify(t) + ": d[" + e + '] || ""';
  }).join(",") + "}");
}
function ye(i) {
  var t = /* @__PURE__ */ Object.create(null), e = [];
  return i.forEach(function(s) {
    for (var n in s)
      n in t || e.push(t[n] = n);
  }), e;
}
function H(i, t) {
  var e = i + "", s = e.length;
  return s < t ? new Array(t - s + 1).join(0) + e : e;
}
function Us(i) {
  var t, e = i.getUTCHours(), s = i.getUTCMinutes(), n = i.getUTCSeconds(), a = i.getUTCMilliseconds();
  return isNaN(i) ? "Invalid Date" : ((t = i.getUTCFullYear()) < 0 ? "-" + H(-t, 6) : t > 9999 ? "+" + H(t, 6) : H(t, 4)) + "-" + H(i.getUTCMonth() + 1, 2) + "-" + H(i.getUTCDate(), 2) + (a ? "T" + H(e, 2) + ":" + H(s, 2) + ":" + H(n, 2) + "." + H(a, 3) + "Z" : n ? "T" + H(e, 2) + ":" + H(s, 2) + ":" + H(n, 2) + "Z" : s || e ? "T" + H(e, 2) + ":" + H(s, 2) + "Z" : "");
}
function hs(i) {
  var t = new RegExp('["' + i + `
\r]`), e = i.charCodeAt(0);
  function s(r, o) {
    var l, u = [], c = r.length, d = 0, g = 0, p = c <= 0, f = !1;
    function A() {
      if (p)
        return Si;
      if (f)
        return f = !1, me;
      var y, b, k = d;
      if (r.charCodeAt(k) === 34) {
        for (; d++ < c && r.charCodeAt(d) !== 34 || r.charCodeAt(++d) === 34; )
          ;
        return (y = d) >= c ? p = !0 : (b = r.charCodeAt(d++)) === 10 ? f = !0 : b === 13 && (f = !0, r.charCodeAt(d) === 10 && ++d), r.slice(k + 1, y - 1).replace(/""/g, '"');
      }
      for (; d < c; ) {
        if ((b = r.charCodeAt(y = d++)) === 10)
          f = !0;
        else if (b === 13)
          f = !0, r.charCodeAt(d) === 10 && ++d;
        else if (b !== e)
          continue;
        return r.slice(k, y);
      }
      return p = !0, r.slice(k, c);
    }
    for (r.charCodeAt(c - 1) === 10 && --c, r.charCodeAt(c - 1) === 13 && --c; (l = A()) !== Si; ) {
      for (var m = []; l !== me && l !== Si; )
        m.push(l), l = A();
      o && (m = o(m, g++)) == null || u.push(m);
    }
    return u;
  }
  function n(r, o) {
    return r.map(function(l) {
      return o.map(function(u) {
        return h(l[u]);
      }).join(i);
    });
  }
  function a(r) {
    return r.map(h).join(i);
  }
  function h(r) {
    return r == null ? "" : r instanceof Date ? Us(r) : t.test(r += "") ? '"' + r.replace(/"/g, '""') + '"' : r;
  }
  return { parse: function(r, o) {
    var l, u, c = s(r, function(d, g) {
      if (l)
        return l(d, g - 1);
      u = d, l = o ? function(p, f) {
        var A = Ae(p);
        return function(m, y) {
          return f(A(m), y, p);
        };
      }(d, o) : Ae(d);
    });
    return c.columns = u || [], c;
  }, parseRows: s, format: function(r, o) {
    return o == null && (o = ye(r)), [o.map(h).join(i)].concat(n(r, o)).join(`
`);
  }, formatBody: function(r, o) {
    return o == null && (o = ye(r)), n(r, o).join(`
`);
  }, formatRows: function(r) {
    return r.map(a).join(`
`);
  }, formatRow: a, formatValue: h };
}
var Js = hs(",").parse, _s = hs("	").parse;
function $s(i) {
  if (!i.ok)
    throw new Error(i.status + " " + i.statusText);
  return i.text();
}
function os(i) {
  return function(t, e, s) {
    return arguments.length === 2 && typeof e == "function" && (s = e, e = void 0), function(n, a) {
      return fetch(n, a).then($s);
    }(t, e).then(function(n) {
      return i(n, s);
    });
  };
}
var xt = os(Js), vt = os(_s);
function tn(i) {
  if (!i.ok)
    throw new Error(i.status + " " + i.statusText);
  if (i.status !== 204 && i.status !== 205)
    return i.json();
}
function ls(i, t) {
  return fetch(i, t).then(tn);
}
function ni(i, t) {
  if ((e = (i = t ? i.toExponential(t - 1) : i.toExponential()).indexOf("e")) < 0)
    return null;
  var e, s = i.slice(0, e);
  return [s.length > 1 ? s[0] + s.slice(2) : s, +i.slice(e + 1)];
}
function wt(i) {
  return (i = ni(Math.abs(i))) ? i[1] : NaN;
}
var us, en = /^(?:(.)?([<>=^]))?([+\-( ])?([$#])?(0)?(\d+)?(,)?(\.\d+)?(~)?([a-z%])?$/i;
function ri(i) {
  if (!(t = en.exec(i)))
    throw new Error("invalid format: " + i);
  var t;
  return new Bi({ fill: t[1], align: t[2], sign: t[3], symbol: t[4], zero: t[5], width: t[6], comma: t[7], precision: t[8] && t[8].slice(1), trim: t[9], type: t[10] });
}
function Bi(i) {
  this.fill = i.fill === void 0 ? " " : i.fill + "", this.align = i.align === void 0 ? ">" : i.align + "", this.sign = i.sign === void 0 ? "-" : i.sign + "", this.symbol = i.symbol === void 0 ? "" : i.symbol + "", this.zero = !!i.zero, this.width = i.width === void 0 ? void 0 : +i.width, this.comma = !!i.comma, this.precision = i.precision === void 0 ? void 0 : +i.precision, this.trim = !!i.trim, this.type = i.type === void 0 ? "" : i.type + "";
}
function xe(i, t) {
  var e = ni(i, t);
  if (!e)
    return i + "";
  var s = e[0], n = e[1];
  return n < 0 ? "0." + new Array(-n).join("0") + s : s.length > n + 1 ? s.slice(0, n + 1) + "." + s.slice(n + 1) : s + new Array(n - s.length + 2).join("0");
}
ri.prototype = Bi.prototype, Bi.prototype.toString = function() {
  return this.fill + this.align + this.sign + this.symbol + (this.zero ? "0" : "") + (this.width === void 0 ? "" : Math.max(1, 0 | this.width)) + (this.comma ? "," : "") + (this.precision === void 0 ? "" : "." + Math.max(0, 0 | this.precision)) + (this.trim ? "~" : "") + this.type;
};
const ve = { "%": function(i, t) {
  return (100 * i).toFixed(t);
}, b: function(i) {
  return Math.round(i).toString(2);
}, c: function(i) {
  return i + "";
}, d: function(i) {
  return Math.abs(i = Math.round(i)) >= 1e21 ? i.toLocaleString("en").replace(/,/g, "") : i.toString(10);
}, e: function(i, t) {
  return i.toExponential(t);
}, f: function(i, t) {
  return i.toFixed(t);
}, g: function(i, t) {
  return i.toPrecision(t);
}, o: function(i) {
  return Math.round(i).toString(8);
}, p: function(i, t) {
  return xe(100 * i, t);
}, r: xe, s: function(i, t) {
  var e = ni(i, t);
  if (!e)
    return i + "";
  var s = e[0], n = e[1], a = n - (us = 3 * Math.max(-8, Math.min(8, Math.floor(n / 3)))) + 1, h = s.length;
  return a === h ? s : a > h ? s + new Array(a - h + 1).join("0") : a > 0 ? s.slice(0, a) + "." + s.slice(a) : "0." + new Array(1 - a).join("0") + ni(i, Math.max(0, t + a - 1))[0];
}, X: function(i) {
  return Math.round(i).toString(16).toUpperCase();
}, x: function(i) {
  return Math.round(i).toString(16);
} };
function be(i) {
  return i;
}
var Mi, ht, cs, ke = Array.prototype.map, we = ["y", "z", "a", "f", "p", "n", "µ", "m", "", "k", "M", "G", "T", "P", "E", "Z", "Y"];
function sn(i) {
  var t, e, s = i.grouping === void 0 || i.thousands === void 0 ? be : (t = ke.call(i.grouping, Number), e = i.thousands + "", function(d, g) {
    for (var p = d.length, f = [], A = 0, m = t[0], y = 0; p > 0 && m > 0 && (y + m + 1 > g && (m = Math.max(1, g - y)), f.push(d.substring(p -= m, p + m)), !((y += m + 1) > g)); )
      m = t[A = (A + 1) % t.length];
    return f.reverse().join(e);
  }), n = i.currency === void 0 ? "" : i.currency[0] + "", a = i.currency === void 0 ? "" : i.currency[1] + "", h = i.decimal === void 0 ? "." : i.decimal + "", r = i.numerals === void 0 ? be : function(d) {
    return function(g) {
      return g.replace(/[0-9]/g, function(p) {
        return d[+p];
      });
    };
  }(ke.call(i.numerals, String)), o = i.percent === void 0 ? "%" : i.percent + "", l = i.minus === void 0 ? "-" : i.minus + "", u = i.nan === void 0 ? "NaN" : i.nan + "";
  function c(d) {
    var g = (d = ri(d)).fill, p = d.align, f = d.sign, A = d.symbol, m = d.zero, y = d.width, b = d.comma, k = d.precision, S = d.trim, v = d.type;
    v === "n" ? (b = !0, v = "g") : ve[v] || (k === void 0 && (k = 12), S = !0, v = "g"), (m || g === "0" && p === "=") && (m = !0, g = "0", p = "=");
    var M = A === "$" ? n : A === "#" && /[boxX]/.test(v) ? "0" + v.toLowerCase() : "", C = A === "$" ? a : /[%p]/.test(v) ? o : "", j = ve[v], z = /[defgprs%]/.test(v);
    function F(w) {
      var W, E, G, O = M, I = C;
      if (v === "c")
        I = j(w) + I, w = "";
      else {
        var J = (w = +w) < 0 || 1 / w < 0;
        if (w = isNaN(w) ? u : j(Math.abs(w), k), S && (w = function($) {
          t:
            for (var lt, zt = $.length, tt = 1, it = -1; tt < zt; ++tt)
              switch ($[tt]) {
                case ".":
                  it = lt = tt;
                  break;
                case "0":
                  it === 0 && (it = tt), lt = tt;
                  break;
                default:
                  if (!+$[tt])
                    break t;
                  it > 0 && (it = 0);
              }
          return it > 0 ? $.slice(0, it) + $.slice(lt + 1) : $;
        }(w)), J && +w == 0 && f !== "+" && (J = !1), O = (J ? f === "(" ? f : l : f === "-" || f === "(" ? "" : f) + O, I = (v === "s" ? we[8 + us / 3] : "") + I + (J && f === "(" ? ")" : ""), z) {
          for (W = -1, E = w.length; ++W < E; )
            if (48 > (G = w.charCodeAt(W)) || G > 57) {
              I = (G === 46 ? h + w.slice(W + 1) : w.slice(W)) + I, w = w.slice(0, W);
              break;
            }
        }
      }
      b && !m && (w = s(w, 1 / 0));
      var _ = O.length + w.length + I.length, L = _ < y ? new Array(y - _ + 1).join(g) : "";
      switch (b && m && (w = s(L + w, L.length ? y - I.length : 1 / 0), L = ""), p) {
        case "<":
          w = O + w + I + L;
          break;
        case "=":
          w = O + L + w + I;
          break;
        case "^":
          w = L.slice(0, _ = L.length >> 1) + O + w + I + L.slice(_);
          break;
        default:
          w = L + O + w + I;
      }
      return r(w);
    }
    return k = k === void 0 ? 6 : /[gprs]/.test(v) ? Math.max(1, Math.min(21, k)) : Math.max(0, Math.min(20, k)), F.toString = function() {
      return d + "";
    }, F;
  }
  return { format: c, formatPrefix: function(d, g) {
    var p = c(((d = ri(d)).type = "f", d)), f = 3 * Math.max(-8, Math.min(8, Math.floor(wt(g) / 3))), A = Math.pow(10, -f), m = we[8 + f / 3];
    return function(y) {
      return p(A * y) + m;
    };
  } };
}
function _i(i, t) {
  switch (arguments.length) {
    case 0:
      break;
    case 1:
      this.range(i);
      break;
    default:
      this.range(t).domain(i);
  }
  return this;
}
Mi = sn({ decimal: ".", thousands: ",", grouping: [3], currency: ["$", ""], minus: "-" }), ht = Mi.format, cs = Mi.formatPrefix;
const Se = Symbol("implicit");
function mi() {
  var i = /* @__PURE__ */ new Map(), t = [], e = [], s = Se;
  function n(a) {
    var h = a + "", r = i.get(h);
    if (!r) {
      if (s !== Se)
        return s;
      i.set(h, r = t.push(a));
    }
    return e[(r - 1) % e.length];
  }
  return n.domain = function(a) {
    if (!arguments.length)
      return t.slice();
    t = [], i = /* @__PURE__ */ new Map();
    for (const h of a) {
      const r = h + "";
      i.has(r) || i.set(r, t.push(h));
    }
    return n;
  }, n.range = function(a) {
    return arguments.length ? (e = Array.from(a), n) : e.slice();
  }, n.unknown = function(a) {
    return arguments.length ? (s = a, n) : s;
  }, n.copy = function() {
    return mi(t, e).unknown(s);
  }, _i.apply(n, arguments), n;
}
function qt() {
  var i, t, e = mi().unknown(void 0), s = e.domain, n = e.range, a = 0, h = 1, r = !1, o = 0, l = 0, u = 0.5;
  function c() {
    var d = s().length, g = h < a, p = g ? h : a, f = g ? a : h;
    i = (f - p) / Math.max(1, d - o + 2 * l), r && (i = Math.floor(i)), p += (f - p - i * (d - o)) * u, t = i * (1 - o), r && (p = Math.round(p), t = Math.round(t));
    var A = rs(d).map(function(m) {
      return p + i * m;
    });
    return n(g ? A.reverse() : A);
  }
  return delete e.unknown, e.domain = function(d) {
    return arguments.length ? (s(d), c()) : s();
  }, e.range = function(d) {
    return arguments.length ? ([a, h] = d, a = +a, h = +h, c()) : [a, h];
  }, e.rangeRound = function(d) {
    return [a, h] = d, a = +a, h = +h, r = !0, c();
  }, e.bandwidth = function() {
    return t;
  }, e.step = function() {
    return i;
  }, e.round = function(d) {
    return arguments.length ? (r = !!d, c()) : r;
  }, e.padding = function(d) {
    return arguments.length ? (o = Math.min(1, l = +d), c()) : o;
  }, e.paddingInner = function(d) {
    return arguments.length ? (o = Math.min(1, d), c()) : o;
  }, e.paddingOuter = function(d) {
    return arguments.length ? (l = +d, c()) : l;
  }, e.align = function(d) {
    return arguments.length ? (u = Math.max(0, Math.min(1, d)), c()) : u;
  }, e.copy = function() {
    return qt(s(), [a, h]).round(r).paddingInner(o).paddingOuter(l).align(u);
  }, _i.apply(c(), arguments);
}
function ds(i) {
  var t = i.copy;
  return i.padding = i.paddingOuter, delete i.paddingInner, delete i.paddingOuter, i.copy = function() {
    return ds(t());
  }, i;
}
function Me() {
  return ds(qt.apply(null, arguments).paddingInner(1));
}
function Pi(i, t, e) {
  i.prototype = t.prototype = e, e.constructor = i;
}
function Pe(i, t) {
  var e = Object.create(i.prototype);
  for (var s in t)
    e[s] = t[s];
  return e;
}
function It() {
}
var Et = 0.7, Nt = 1 / Et, Rt = "\\s*([+-]?\\d+)\\s*", Bt = "\\s*([+-]?\\d*\\.?\\d+(?:[eE][+-]?\\d+)?)\\s*", at = "\\s*([+-]?\\d*\\.?\\d+(?:[eE][+-]?\\d+)?)%\\s*", nn = /^#([0-9a-f]{3,8})$/, rn = new RegExp("^rgb\\(" + [Rt, Rt, Rt] + "\\)$"), an = new RegExp("^rgb\\(" + [at, at, at] + "\\)$"), hn = new RegExp("^rgba\\(" + [Rt, Rt, Rt, Bt] + "\\)$"), on = new RegExp("^rgba\\(" + [at, at, at, Bt] + "\\)$"), ln = new RegExp("^hsl\\(" + [Bt, at, at] + "\\)$"), un = new RegExp("^hsla\\(" + [Bt, at, at, Bt] + "\\)$"), Re = { aliceblue: 15792383, antiquewhite: 16444375, aqua: 65535, aquamarine: 8388564, azure: 15794175, beige: 16119260, bisque: 16770244, black: 0, blanchedalmond: 16772045, blue: 255, blueviolet: 9055202, brown: 10824234, burlywood: 14596231, cadetblue: 6266528, chartreuse: 8388352, chocolate: 13789470, coral: 16744272, cornflowerblue: 6591981, cornsilk: 16775388, crimson: 14423100, cyan: 65535, darkblue: 139, darkcyan: 35723, darkgoldenrod: 12092939, darkgray: 11119017, darkgreen: 25600, darkgrey: 11119017, darkkhaki: 12433259, darkmagenta: 9109643, darkolivegreen: 5597999, darkorange: 16747520, darkorchid: 10040012, darkred: 9109504, darksalmon: 15308410, darkseagreen: 9419919, darkslateblue: 4734347, darkslategray: 3100495, darkslategrey: 3100495, darkturquoise: 52945, darkviolet: 9699539, deeppink: 16716947, deepskyblue: 49151, dimgray: 6908265, dimgrey: 6908265, dodgerblue: 2003199, firebrick: 11674146, floralwhite: 16775920, forestgreen: 2263842, fuchsia: 16711935, gainsboro: 14474460, ghostwhite: 16316671, gold: 16766720, goldenrod: 14329120, gray: 8421504, green: 32768, greenyellow: 11403055, grey: 8421504, honeydew: 15794160, hotpink: 16738740, indianred: 13458524, indigo: 4915330, ivory: 16777200, khaki: 15787660, lavender: 15132410, lavenderblush: 16773365, lawngreen: 8190976, lemonchiffon: 16775885, lightblue: 11393254, lightcoral: 15761536, lightcyan: 14745599, lightgoldenrodyellow: 16448210, lightgray: 13882323, lightgreen: 9498256, lightgrey: 13882323, lightpink: 16758465, lightsalmon: 16752762, lightseagreen: 2142890, lightskyblue: 8900346, lightslategray: 7833753, lightslategrey: 7833753, lightsteelblue: 11584734, lightyellow: 16777184, lime: 65280, limegreen: 3329330, linen: 16445670, magenta: 16711935, maroon: 8388608, mediumaquamarine: 6737322, mediumblue: 205, mediumorchid: 12211667, mediumpurple: 9662683, mediumseagreen: 3978097, mediumslateblue: 8087790, mediumspringgreen: 64154, mediumturquoise: 4772300, mediumvioletred: 13047173, midnightblue: 1644912, mintcream: 16121850, mistyrose: 16770273, moccasin: 16770229, navajowhite: 16768685, navy: 128, oldlace: 16643558, olive: 8421376, olivedrab: 7048739, orange: 16753920, orangered: 16729344, orchid: 14315734, palegoldenrod: 15657130, palegreen: 10025880, paleturquoise: 11529966, palevioletred: 14381203, papayawhip: 16773077, peachpuff: 16767673, peru: 13468991, pink: 16761035, plum: 14524637, powderblue: 11591910, purple: 8388736, rebeccapurple: 6697881, red: 16711680, rosybrown: 12357519, royalblue: 4286945, saddlebrown: 9127187, salmon: 16416882, sandybrown: 16032864, seagreen: 3050327, seashell: 16774638, sienna: 10506797, silver: 12632256, skyblue: 8900331, slateblue: 6970061, slategray: 7372944, slategrey: 7372944, snow: 16775930, springgreen: 65407, steelblue: 4620980, tan: 13808780, teal: 32896, thistle: 14204888, tomato: 16737095, turquoise: 4251856, violet: 15631086, wheat: 16113331, white: 16777215, whitesmoke: 16119285, yellow: 16776960, yellowgreen: 10145074 };
function ze() {
  return this.rgb().formatHex();
}
function Fe() {
  return this.rgb().formatRgb();
}
function Tt(i) {
  var t, e;
  return i = (i + "").trim().toLowerCase(), (t = nn.exec(i)) ? (e = t[1].length, t = parseInt(t[1], 16), e === 6 ? We(t) : e === 3 ? new U(t >> 8 & 15 | t >> 4 & 240, t >> 4 & 15 | 240 & t, (15 & t) << 4 | 15 & t, 1) : e === 8 ? Xt(t >> 24 & 255, t >> 16 & 255, t >> 8 & 255, (255 & t) / 255) : e === 4 ? Xt(t >> 12 & 15 | t >> 8 & 240, t >> 8 & 15 | t >> 4 & 240, t >> 4 & 15 | 240 & t, ((15 & t) << 4 | 15 & t) / 255) : null) : (t = rn.exec(i)) ? new U(t[1], t[2], t[3], 1) : (t = an.exec(i)) ? new U(255 * t[1] / 100, 255 * t[2] / 100, 255 * t[3] / 100, 1) : (t = hn.exec(i)) ? Xt(t[1], t[2], t[3], t[4]) : (t = on.exec(i)) ? Xt(255 * t[1] / 100, 255 * t[2] / 100, 255 * t[3] / 100, t[4]) : (t = ln.exec(i)) ? Ee(t[1], t[2] / 100, t[3] / 100, 1) : (t = un.exec(i)) ? Ee(t[1], t[2] / 100, t[3] / 100, t[4]) : Re.hasOwnProperty(i) ? We(Re[i]) : i === "transparent" ? new U(NaN, NaN, NaN, 0) : null;
}
function We(i) {
  return new U(i >> 16 & 255, i >> 8 & 255, 255 & i, 1);
}
function Xt(i, t, e, s) {
  return s <= 0 && (i = t = e = NaN), new U(i, t, e, s);
}
function Ti(i, t, e, s) {
  return arguments.length === 1 ? function(n) {
    return n instanceof It || (n = Tt(n)), n ? new U((n = n.rgb()).r, n.g, n.b, n.opacity) : new U();
  }(i) : new U(i, t, e, s ?? 1);
}
function U(i, t, e, s) {
  this.r = +i, this.g = +t, this.b = +e, this.opacity = +s;
}
function Ce() {
  return "#" + Ri(this.r) + Ri(this.g) + Ri(this.b);
}
function je() {
  var i = this.opacity;
  return ((i = isNaN(i) ? 1 : Math.max(0, Math.min(1, i))) === 1 ? "rgb(" : "rgba(") + Math.max(0, Math.min(255, Math.round(this.r) || 0)) + ", " + Math.max(0, Math.min(255, Math.round(this.g) || 0)) + ", " + Math.max(0, Math.min(255, Math.round(this.b) || 0)) + (i === 1 ? ")" : ", " + i + ")");
}
function Ri(i) {
  return ((i = Math.max(0, Math.min(255, Math.round(i) || 0))) < 16 ? "0" : "") + i.toString(16);
}
function Ee(i, t, e, s) {
  return s <= 0 ? i = t = e = NaN : e <= 0 || e >= 1 ? i = t = NaN : t <= 0 && (i = NaN), new rt(i, t, e, s);
}
function Oe(i) {
  if (i instanceof rt)
    return new rt(i.h, i.s, i.l, i.opacity);
  if (i instanceof It || (i = Tt(i)), !i)
    return new rt();
  if (i instanceof rt)
    return i;
  var t = (i = i.rgb()).r / 255, e = i.g / 255, s = i.b / 255, n = Math.min(t, e, s), a = Math.max(t, e, s), h = NaN, r = a - n, o = (a + n) / 2;
  return r ? (h = t === a ? (e - s) / r + 6 * (e < s) : e === a ? (s - t) / r + 2 : (t - e) / r + 4, r /= o < 0.5 ? a + n : 2 - a - n, h *= 60) : r = o > 0 && o < 1 ? 0 : h, new rt(h, r, o, i.opacity);
}
function rt(i, t, e, s) {
  this.h = +i, this.s = +t, this.l = +e, this.opacity = +s;
}
function zi(i, t, e) {
  return 255 * (i < 60 ? t + (e - t) * i / 60 : i < 180 ? e : i < 240 ? t + (e - t) * (240 - i) / 60 : t);
}
Pi(It, Tt, { copy: function(i) {
  return Object.assign(new this.constructor(), this, i);
}, displayable: function() {
  return this.rgb().displayable();
}, hex: ze, formatHex: ze, formatHsl: function() {
  return Oe(this).formatHsl();
}, formatRgb: Fe, toString: Fe }), Pi(U, Ti, Pe(It, { brighter: function(i) {
  return i = i == null ? Nt : Math.pow(Nt, i), new U(this.r * i, this.g * i, this.b * i, this.opacity);
}, darker: function(i) {
  return i = i == null ? Et : Math.pow(Et, i), new U(this.r * i, this.g * i, this.b * i, this.opacity);
}, rgb: function() {
  return this;
}, displayable: function() {
  return -0.5 <= this.r && this.r < 255.5 && -0.5 <= this.g && this.g < 255.5 && -0.5 <= this.b && this.b < 255.5 && 0 <= this.opacity && this.opacity <= 1;
}, hex: Ce, formatHex: Ce, formatRgb: je, toString: je })), Pi(rt, function(i, t, e, s) {
  return arguments.length === 1 ? Oe(i) : new rt(i, t, e, s ?? 1);
}, Pe(It, { brighter: function(i) {
  return i = i == null ? Nt : Math.pow(Nt, i), new rt(this.h, this.s, this.l * i, this.opacity);
}, darker: function(i) {
  return i = i == null ? Et : Math.pow(Et, i), new rt(this.h, this.s, this.l * i, this.opacity);
}, rgb: function() {
  var i = this.h % 360 + 360 * (this.h < 0), t = isNaN(i) || isNaN(this.s) ? 0 : this.s, e = this.l, s = e + (e < 0.5 ? e : 1 - e) * t, n = 2 * e - s;
  return new U(zi(i >= 240 ? i - 240 : i + 120, n, s), zi(i, n, s), zi(i < 120 ? i + 240 : i - 120, n, s), this.opacity);
}, displayable: function() {
  return (0 <= this.s && this.s <= 1 || isNaN(this.s)) && 0 <= this.l && this.l <= 1 && 0 <= this.opacity && this.opacity <= 1;
}, formatHsl: function() {
  var i = this.opacity;
  return ((i = isNaN(i) ? 1 : Math.max(0, Math.min(1, i))) === 1 ? "hsl(" : "hsla(") + (this.h || 0) + ", " + 100 * (this.s || 0) + "%, " + 100 * (this.l || 0) + "%" + (i === 1 ? ")" : ", " + i + ")");
} }));
const $i = (i) => () => i;
function cn(i) {
  return (i = +i) == 1 ? gs : function(t, e) {
    return e - t ? function(s, n, a) {
      return s = Math.pow(s, a), n = Math.pow(n, a) - s, a = 1 / a, function(h) {
        return Math.pow(s + h * n, a);
      };
    }(t, e, i) : $i(isNaN(t) ? e : t);
  };
}
function gs(i, t) {
  var e = t - i;
  return e ? function(s, n) {
    return function(a) {
      return s + a * n;
    };
  }(i, e) : $i(isNaN(i) ? t : i);
}
const Ge = function i(t) {
  var e = cn(t);
  function s(n, a) {
    var h = e((n = Ti(n)).r, (a = Ti(a)).r), r = e(n.g, a.g), o = e(n.b, a.b), l = gs(n.opacity, a.opacity);
    return function(u) {
      return n.r = h(u), n.g = r(u), n.b = o(u), n.opacity = l(u), n + "";
    };
  }
  return s.gamma = i, s;
}(1);
function dn(i, t) {
  t || (t = []);
  var e, s = i ? Math.min(t.length, i.length) : 0, n = t.slice();
  return function(a) {
    for (e = 0; e < s; ++e)
      n[e] = i[e] * (1 - a) + t[e] * a;
    return n;
  };
}
function gn(i, t) {
  var e, s = t ? t.length : 0, n = i ? Math.min(s, i.length) : 0, a = new Array(n), h = new Array(s);
  for (e = 0; e < n; ++e)
    a[e] = te(i[e], t[e]);
  for (; e < s; ++e)
    h[e] = t[e];
  return function(r) {
    for (e = 0; e < n; ++e)
      h[e] = a[e](r);
    return h;
  };
}
function fn(i, t) {
  var e = /* @__PURE__ */ new Date();
  return i = +i, t = +t, function(s) {
    return e.setTime(i * (1 - s) + t * s), e;
  };
}
function ai(i, t) {
  return i = +i, t = +t, function(e) {
    return i * (1 - e) + t * e;
  };
}
function pn(i, t) {
  var e, s = {}, n = {};
  for (e in i !== null && typeof i == "object" || (i = {}), t !== null && typeof t == "object" || (t = {}), t)
    e in i ? s[e] = te(i[e], t[e]) : n[e] = t[e];
  return function(a) {
    for (e in s)
      n[e] = s[e](a);
    return n;
  };
}
var Vi = /[-+]?(?:\d+\.?\d*|\.?\d+)(?:[eE][-+]?\d+)?/g, Fi = new RegExp(Vi.source, "g");
function mn(i, t) {
  var e, s, n, a = Vi.lastIndex = Fi.lastIndex = 0, h = -1, r = [], o = [];
  for (i += "", t += ""; (e = Vi.exec(i)) && (s = Fi.exec(t)); )
    (n = s.index) > a && (n = t.slice(a, n), r[h] ? r[h] += n : r[++h] = n), (e = e[0]) === (s = s[0]) ? r[h] ? r[h] += s : r[++h] = s : (r[++h] = null, o.push({ i: h, x: ai(e, s) })), a = Fi.lastIndex;
  return a < t.length && (n = t.slice(a), r[h] ? r[h] += n : r[++h] = n), r.length < 2 ? o[0] ? function(l) {
    return function(u) {
      return l(u) + "";
    };
  }(o[0].x) : function(l) {
    return function() {
      return l;
    };
  }(t) : (t = o.length, function(l) {
    for (var u, c = 0; c < t; ++c)
      r[(u = o[c]).i] = u.x(l);
    return r.join("");
  });
}
function te(i, t) {
  var e, s = typeof t;
  return t == null || s === "boolean" ? $i(t) : (s === "number" ? ai : s === "string" ? (e = Tt(t)) ? (t = e, Ge) : mn : t instanceof Tt ? Ge : t instanceof Date ? fn : function(n) {
    return ArrayBuffer.isView(n) && !(n instanceof DataView);
  }(t) ? dn : Array.isArray(t) ? gn : typeof t.valueOf != "function" && typeof t.toString != "function" || isNaN(t) ? pn : ai)(i, t);
}
function An(i, t) {
  return i = +i, t = +t, function(e) {
    return Math.round(i * (1 - e) + t * e);
  };
}
function yn(i) {
  return +i;
}
var Ie = [0, 1];
function St(i) {
  return i;
}
function Yi(i, t) {
  return (t -= i = +i) ? function(e) {
    return (e - i) / t;
  } : function(e) {
    return function() {
      return e;
    };
  }(isNaN(t) ? NaN : 0.5);
}
function xn(i, t, e) {
  var s = i[0], n = i[1], a = t[0], h = t[1];
  return n < s ? (s = Yi(n, s), a = e(h, a)) : (s = Yi(s, n), a = e(a, h)), function(r) {
    return a(s(r));
  };
}
function vn(i, t, e) {
  var s = Math.min(i.length, t.length) - 1, n = new Array(s), a = new Array(s), h = -1;
  for (i[s] < i[0] && (i = i.slice().reverse(), t = t.slice().reverse()); ++h < s; )
    n[h] = Yi(i[h], i[h + 1]), a[h] = e(t[h], t[h + 1]);
  return function(r) {
    var o = ns(i, r, 1, s) - 1;
    return a[o](n[o](r));
  };
}
function bn() {
  var i, t, e, s, n, a, h = Ie, r = Ie, o = te, l = St;
  function u() {
    var d = Math.min(h.length, r.length);
    return l !== St && (l = function(g, p) {
      var f;
      return g > p && (f = g, g = p, p = f), function(A) {
        return Math.max(g, Math.min(p, A));
      };
    }(h[0], h[d - 1])), s = d > 2 ? vn : xn, n = a = null, c;
  }
  function c(d) {
    return d == null || isNaN(d = +d) ? e : (n || (n = s(h.map(i), r, o)))(i(l(d)));
  }
  return c.invert = function(d) {
    return l(t((a || (a = s(r, h.map(i), ai)))(d)));
  }, c.domain = function(d) {
    return arguments.length ? (h = Array.from(d, yn), u()) : h.slice();
  }, c.range = function(d) {
    return arguments.length ? (r = Array.from(d), u()) : r.slice();
  }, c.rangeRound = function(d) {
    return r = Array.from(d), o = An, u();
  }, c.clamp = function(d) {
    return arguments.length ? (l = !!d || St, u()) : l !== St;
  }, c.interpolate = function(d) {
    return arguments.length ? (o = d, u()) : o;
  }, c.unknown = function(d) {
    return arguments.length ? (e = d, c) : e;
  }, function(d, g) {
    return i = d, t = g, u();
  };
}
function kn(i, t, e, s) {
  var n, a = function(r, o, l) {
    var u = Math.abs(o - r) / Math.max(0, l), c = Math.pow(10, Math.floor(Math.log(u) / Math.LN10)), d = u / c;
    return d >= Ii ? c *= 10 : d >= Li ? c *= 5 : d >= Zi && (c *= 2), o < r ? -c : c;
  }(i, t, e);
  switch ((s = ri(s ?? ",f")).type) {
    case "s":
      var h = Math.max(Math.abs(i), Math.abs(t));
      return s.precision != null || isNaN(n = function(r, o) {
        return Math.max(0, 3 * Math.max(-8, Math.min(8, Math.floor(wt(o) / 3))) - wt(Math.abs(r)));
      }(a, h)) || (s.precision = n), cs(s, h);
    case "":
    case "e":
    case "g":
    case "p":
    case "r":
      s.precision != null || isNaN(n = function(r, o) {
        return r = Math.abs(r), o = Math.abs(o) - r, Math.max(0, wt(o) - wt(r)) + 1;
      }(a, Math.max(Math.abs(i), Math.abs(t)))) || (s.precision = n - (s.type === "e"));
      break;
    case "f":
    case "%":
      s.precision != null || isNaN(n = function(r) {
        return Math.max(0, -wt(Math.abs(r)));
      }(a)) || (s.precision = n - 2 * (s.type === "%"));
  }
  return ht(s);
}
function wn(i) {
  var t = i.domain;
  return i.ticks = function(e) {
    var s = t();
    return function(n, a, h) {
      var r, o, l, u, c = -1;
      if (h = +h, (n = +n) == (a = +a) && h > 0)
        return [n];
      if ((r = a < n) && (o = n, n = a, a = o), (u = fe(n, a, h)) === 0 || !isFinite(u))
        return [];
      if (u > 0) {
        let d = Math.round(n / u), g = Math.round(a / u);
        for (d * u < n && ++d, g * u > a && --g, l = new Array(o = g - d + 1); ++c < o; )
          l[c] = (d + c) * u;
      } else {
        u = -u;
        let d = Math.round(n * u), g = Math.round(a * u);
        for (d / u < n && ++d, g / u > a && --g, l = new Array(o = g - d + 1); ++c < o; )
          l[c] = (d + c) / u;
      }
      return r && l.reverse(), l;
    }(s[0], s[s.length - 1], e ?? 10);
  }, i.tickFormat = function(e, s) {
    var n = t();
    return kn(n[0], n[n.length - 1], e ?? 10, s);
  }, i.nice = function(e) {
    e == null && (e = 10);
    var s, n, a = t(), h = 0, r = a.length - 1, o = a[h], l = a[r], u = 10;
    for (l < o && (n = o, o = l, l = n, n = h, h = r, r = n); u-- > 0; ) {
      if ((n = fe(o, l, e)) === s)
        return a[h] = o, a[r] = l, t(a);
      if (n > 0)
        o = Math.floor(o / n) * n, l = Math.ceil(l / n) * n;
      else {
        if (!(n < 0))
          break;
        o = Math.ceil(o * n) / n, l = Math.floor(l * n) / n;
      }
      s = n;
    }
    return i;
  }, i;
}
function Z() {
  var i = bn()(St, St);
  return i.copy = function() {
    return t = i, Z().domain(t.domain()).range(t.range()).interpolate(t.interpolate()).clamp(t.clamp()).unknown(t.unknown());
    var t;
  }, _i.apply(i, arguments), wn(i);
}
var Di = "http://www.w3.org/1999/xhtml";
const Le = { svg: "http://www.w3.org/2000/svg", xhtml: Di, xlink: "http://www.w3.org/1999/xlink", xml: "http://www.w3.org/XML/1998/namespace", xmlns: "http://www.w3.org/2000/xmlns/" };
function fs(i) {
  var t = i += "", e = t.indexOf(":");
  return e >= 0 && (t = i.slice(0, e)) !== "xmlns" && (i = i.slice(e + 1)), Le.hasOwnProperty(t) ? { space: Le[t], local: i } : i;
}
function Sn(i) {
  return function() {
    var t = this.ownerDocument, e = this.namespaceURI;
    return e === Di && t.documentElement.namespaceURI === Di ? t.createElement(i) : t.createElementNS(e, i);
  };
}
function Mn(i) {
  return function() {
    return this.ownerDocument.createElementNS(i.space, i.local);
  };
}
function Ze(i) {
  var t = fs(i);
  return (t.local ? Mn : Sn)(t);
}
function Pn() {
}
function Be(i) {
  return i == null ? Pn : function() {
    return this.querySelector(i);
  };
}
function Rn() {
  return [];
}
function Te(i) {
  return new Array(i.length);
}
function hi(i, t) {
  this.ownerDocument = i.ownerDocument, this.namespaceURI = i.namespaceURI, this._next = null, this._parent = i, this.__data__ = t;
}
hi.prototype = { constructor: hi, appendChild: function(i) {
  return this._parent.insertBefore(i, this._next);
}, insertBefore: function(i, t) {
  return this._parent.insertBefore(i, t);
}, querySelector: function(i) {
  return this._parent.querySelector(i);
}, querySelectorAll: function(i) {
  return this._parent.querySelectorAll(i);
} };
function zn(i, t, e, s, n, a) {
  for (var h, r = 0, o = t.length, l = a.length; r < l; ++r)
    (h = t[r]) ? (h.__data__ = a[r], s[r] = h) : e[r] = new hi(i, a[r]);
  for (; r < o; ++r)
    (h = t[r]) && (n[r] = h);
}
function Fn(i, t, e, s, n, a, h) {
  var r, o, l, u = {}, c = t.length, d = a.length, g = new Array(c);
  for (r = 0; r < c; ++r)
    (o = t[r]) && (g[r] = l = "$" + h.call(o, o.__data__, r, t), l in u ? n[r] = o : u[l] = o);
  for (r = 0; r < d; ++r)
    (o = u[l = "$" + h.call(i, a[r], r, a)]) ? (s[r] = o, o.__data__ = a[r], u[l] = null) : e[r] = new hi(i, a[r]);
  for (r = 0; r < c; ++r)
    (o = t[r]) && u[g[r]] === o && (n[r] = o);
}
function Wn(i, t) {
  return i < t ? -1 : i > t ? 1 : i >= t ? 0 : NaN;
}
function Cn(i) {
  return function() {
    this.removeAttribute(i);
  };
}
function jn(i) {
  return function() {
    this.removeAttributeNS(i.space, i.local);
  };
}
function En(i, t) {
  return function() {
    this.setAttribute(i, t);
  };
}
function On(i, t) {
  return function() {
    this.setAttributeNS(i.space, i.local, t);
  };
}
function Gn(i, t) {
  return function() {
    var e = t.apply(this, arguments);
    e == null ? this.removeAttribute(i) : this.setAttribute(i, e);
  };
}
function In(i, t) {
  return function() {
    var e = t.apply(this, arguments);
    e == null ? this.removeAttributeNS(i.space, i.local) : this.setAttributeNS(i.space, i.local, e);
  };
}
function ps(i) {
  return i.ownerDocument && i.ownerDocument.defaultView || i.document && i || i.defaultView;
}
function Ln(i) {
  return function() {
    this.style.removeProperty(i);
  };
}
function Zn(i, t, e) {
  return function() {
    this.style.setProperty(i, t, e);
  };
}
function Bn(i, t, e) {
  return function() {
    var s = t.apply(this, arguments);
    s == null ? this.style.removeProperty(i) : this.style.setProperty(i, s, e);
  };
}
function Tn(i) {
  return function() {
    delete this[i];
  };
}
function Vn(i, t) {
  return function() {
    this[i] = t;
  };
}
function Yn(i, t) {
  return function() {
    var e = t.apply(this, arguments);
    e == null ? delete this[i] : this[i] = e;
  };
}
function ms(i) {
  return i.trim().split(/^|\s+/);
}
function ie(i) {
  return i.classList || new As(i);
}
function As(i) {
  this._node = i, this._names = ms(i.getAttribute("class") || "");
}
function ys(i, t) {
  for (var e = ie(i), s = -1, n = t.length; ++s < n; )
    e.add(t[s]);
}
function xs(i, t) {
  for (var e = ie(i), s = -1, n = t.length; ++s < n; )
    e.remove(t[s]);
}
function Dn(i) {
  return function() {
    ys(this, i);
  };
}
function qn(i) {
  return function() {
    xs(this, i);
  };
}
function Hn(i, t) {
  return function() {
    (t.apply(this, arguments) ? ys : xs)(this, i);
  };
}
function Nn() {
  this.textContent = "";
}
function Xn(i) {
  return function() {
    this.textContent = i;
  };
}
function Kn(i) {
  return function() {
    var t = i.apply(this, arguments);
    this.textContent = t ?? "";
  };
}
function Qn() {
  this.innerHTML = "";
}
function Un(i) {
  return function() {
    this.innerHTML = i;
  };
}
function Jn(i) {
  return function() {
    var t = i.apply(this, arguments);
    this.innerHTML = t ?? "";
  };
}
function _n() {
  this.nextSibling && this.parentNode.appendChild(this);
}
function $n() {
  this.previousSibling && this.parentNode.insertBefore(this, this.parentNode.firstChild);
}
function tr() {
  return null;
}
function ir() {
  var i = this.parentNode;
  i && i.removeChild(this);
}
function er() {
  var i = this.cloneNode(!1), t = this.parentNode;
  return t ? t.insertBefore(i, this.nextSibling) : i;
}
function sr() {
  var i = this.cloneNode(!0), t = this.parentNode;
  return t ? t.insertBefore(i, this.nextSibling) : i;
}
As.prototype = { add: function(i) {
  this._names.indexOf(i) < 0 && (this._names.push(i), this._node.setAttribute("class", this._names.join(" ")));
}, remove: function(i) {
  var t = this._names.indexOf(i);
  t >= 0 && (this._names.splice(t, 1), this._node.setAttribute("class", this._names.join(" ")));
}, contains: function(i) {
  return this._names.indexOf(i) >= 0;
} };
var vs = {}, $t = null;
typeof document < "u" && ("onmouseenter" in document.documentElement || (vs = { mouseenter: "mouseover", mouseleave: "mouseout" }));
function nr(i, t, e) {
  return i = bs(i, t, e), function(s) {
    var n = s.relatedTarget;
    n && (n === this || 8 & n.compareDocumentPosition(this)) || i.call(this, s);
  };
}
function bs(i, t, e) {
  return function(s) {
    var n = $t;
    $t = s;
    try {
      i.call(this, this.__data__, t, e);
    } finally {
      $t = n;
    }
  };
}
function rr(i) {
  return function() {
    var t = this.__on;
    if (t) {
      for (var e, s = 0, n = -1, a = t.length; s < a; ++s)
        e = t[s], i.type && e.type !== i.type || e.name !== i.name ? t[++n] = e : this.removeEventListener(e.type, e.listener, e.capture);
      ++n ? t.length = n : delete this.__on;
    }
  };
}
function ar(i, t, e) {
  var s = vs.hasOwnProperty(i.type) ? nr : bs;
  return function(n, a, h) {
    var r, o = this.__on, l = s(t, a, h);
    if (o) {
      for (var u = 0, c = o.length; u < c; ++u)
        if ((r = o[u]).type === i.type && r.name === i.name)
          return this.removeEventListener(r.type, r.listener, r.capture), this.addEventListener(r.type, r.listener = l, r.capture = e), void (r.value = t);
    }
    this.addEventListener(i.type, l, e), r = { type: i.type, name: i.name, value: t, listener: l, capture: e }, o ? o.push(r) : this.__on = [r];
  };
}
function ks(i, t, e) {
  var s = ps(i), n = s.CustomEvent;
  typeof n == "function" ? n = new n(t, e) : (n = s.document.createEvent("Event"), e ? (n.initEvent(t, e.bubbles, e.cancelable), n.detail = e.detail) : n.initEvent(t, !1, !1)), i.dispatchEvent(n);
}
function hr(i, t) {
  return function() {
    return ks(this, i, t);
  };
}
function or(i, t) {
  return function() {
    return ks(this, i, t.apply(this, arguments));
  };
}
var ws = [null];
function K(i, t) {
  this._groups = i, this._parents = t;
}
function x(i) {
  return typeof i == "string" ? new K([[document.querySelector(i)]], [document.documentElement]) : new K([[i]], ws);
}
function lr() {
  for (var i, t = $t; i = t.sourceEvent; )
    t = i;
  return t;
}
function bt(i) {
  var t = lr();
  return t.changedTouches && (t = t.changedTouches[0]), function(e, s) {
    var n = e.ownerSVGElement || e;
    if (n.createSVGPoint) {
      var a = n.createSVGPoint();
      return a.x = s.clientX, a.y = s.clientY, [(a = a.matrixTransform(e.getScreenCTM().inverse())).x, a.y];
    }
    var h = e.getBoundingClientRect();
    return [s.clientX - h.left - e.clientLeft, s.clientY - h.top - e.clientTop];
  }(i, t);
}
function P(i) {
  return typeof i == "string" ? new K([document.querySelectorAll(i)], [document.documentElement]) : new K([i ?? []], ws);
}
function Wi(i, t, e) {
  if (i && i.length) {
    const [s, n] = t, a = Math.PI / 180 * e, h = Math.cos(a), r = Math.sin(a);
    for (const o of i) {
      const [l, u] = o;
      o[0] = (l - s) * h - (u - n) * r + s, o[1] = (l - s) * r + (u - n) * h + n;
    }
  }
}
function ur(i, t) {
  return i[0] === t[0] && i[1] === t[1];
}
function cr(i, t, e, s = 1) {
  const n = e, a = Math.max(t, 0.1), h = i[0] && i[0][0] && typeof i[0][0] == "number" ? [i] : i, r = [0, 0];
  if (n)
    for (const l of h)
      Wi(l, r, n);
  const o = function(l, u, c) {
    const d = [];
    for (const y of l) {
      const b = [...y];
      ur(b[0], b[b.length - 1]) || b.push([b[0][0], b[0][1]]), b.length > 2 && d.push(b);
    }
    const g = [];
    u = Math.max(u, 0.1);
    const p = [];
    for (const y of d)
      for (let b = 0; b < y.length - 1; b++) {
        const k = y[b], S = y[b + 1];
        if (k[1] !== S[1]) {
          const v = Math.min(k[1], S[1]);
          p.push({ ymin: v, ymax: Math.max(k[1], S[1]), x: v === k[1] ? k[0] : S[0], islope: (S[0] - k[0]) / (S[1] - k[1]) });
        }
      }
    if (p.sort((y, b) => y.ymin < b.ymin ? -1 : y.ymin > b.ymin ? 1 : y.x < b.x ? -1 : y.x > b.x ? 1 : y.ymax === b.ymax ? 0 : (y.ymax - b.ymax) / Math.abs(y.ymax - b.ymax)), !p.length)
      return g;
    let f = [], A = p[0].ymin, m = 0;
    for (; f.length || p.length; ) {
      if (p.length) {
        let y = -1;
        for (let b = 0; b < p.length && !(p[b].ymin > A); b++)
          y = b;
        p.splice(0, y + 1).forEach((b) => {
          f.push({ s: A, edge: b });
        });
      }
      if (f = f.filter((y) => !(y.edge.ymax <= A)), f.sort((y, b) => y.edge.x === b.edge.x ? 0 : (y.edge.x - b.edge.x) / Math.abs(y.edge.x - b.edge.x)), (c !== 1 || m % u == 0) && f.length > 1)
        for (let y = 0; y < f.length; y += 2) {
          const b = y + 1;
          if (b >= f.length)
            break;
          const k = f[y].edge, S = f[b].edge;
          g.push([[Math.round(k.x), A], [Math.round(S.x), A]]);
        }
      A += c, f.forEach((y) => {
        y.edge.x = y.edge.x + c * y.edge.islope;
      }), m++;
    }
    return g;
  }(h, a, s);
  if (n) {
    for (const l of h)
      Wi(l, r, -n);
    (function(l, u, c) {
      const d = [];
      l.forEach((g) => d.push(...g)), Wi(d, u, c);
    })(o, r, -n);
  }
  return o;
}
function Ht(i, t) {
  var e;
  const s = t.hachureAngle + 90;
  let n = t.hachureGap;
  n < 0 && (n = 4 * t.strokeWidth), n = Math.round(Math.max(n, 0.1));
  let a = 1;
  return t.roughness >= 1 && (((e = t.randomizer) === null || e === void 0 ? void 0 : e.next()) || Math.random()) > 0.7 && (a = n), cr(i, n, s, a || 1);
}
K.prototype = { constructor: K, select: function(i) {
  typeof i != "function" && (i = Be(i));
  for (var t = this._groups, e = t.length, s = new Array(e), n = 0; n < e; ++n)
    for (var a, h, r = t[n], o = r.length, l = s[n] = new Array(o), u = 0; u < o; ++u)
      (a = r[u]) && (h = i.call(a, a.__data__, u, r)) && ("__data__" in a && (h.__data__ = a.__data__), l[u] = h);
  return new K(s, this._parents);
}, selectAll: function(i) {
  typeof i != "function" && (i = function(u) {
    return u == null ? Rn : function() {
      return this.querySelectorAll(u);
    };
  }(i));
  for (var t = this._groups, e = t.length, s = [], n = [], a = 0; a < e; ++a)
    for (var h, r = t[a], o = r.length, l = 0; l < o; ++l)
      (h = r[l]) && (s.push(i.call(h, h.__data__, l, r)), n.push(h));
  return new K(s, n);
}, filter: function(i) {
  typeof i != "function" && (i = function(u) {
    return function() {
      return this.matches(u);
    };
  }(i));
  for (var t = this._groups, e = t.length, s = new Array(e), n = 0; n < e; ++n)
    for (var a, h = t[n], r = h.length, o = s[n] = [], l = 0; l < r; ++l)
      (a = h[l]) && i.call(a, a.__data__, l, h) && o.push(a);
  return new K(s, this._parents);
}, data: function(i, t) {
  if (!i)
    return g = new Array(this.size()), l = -1, this.each(function(S) {
      g[++l] = S;
    }), g;
  var e = t ? Fn : zn, s = this._parents, n = this._groups;
  typeof i != "function" && (i = function(S) {
    return function() {
      return S;
    };
  }(i));
  for (var a = n.length, h = new Array(a), r = new Array(a), o = new Array(a), l = 0; l < a; ++l) {
    var u = s[l], c = n[l], d = c.length, g = i.call(u, u && u.__data__, l, s), p = g.length, f = r[l] = new Array(p), A = h[l] = new Array(p);
    e(u, c, f, A, o[l] = new Array(d), g, t);
    for (var m, y, b = 0, k = 0; b < p; ++b)
      if (m = f[b]) {
        for (b >= k && (k = b + 1); !(y = A[k]) && ++k < p; )
          ;
        m._next = y || null;
      }
  }
  return (h = new K(h, s))._enter = r, h._exit = o, h;
}, enter: function() {
  return new K(this._enter || this._groups.map(Te), this._parents);
}, exit: function() {
  return new K(this._exit || this._groups.map(Te), this._parents);
}, join: function(i, t, e) {
  var s = this.enter(), n = this, a = this.exit();
  return s = typeof i == "function" ? i(s) : s.append(i + ""), t != null && (n = t(n)), e == null ? a.remove() : e(a), s && n ? s.merge(n).order() : n;
}, merge: function(i) {
  for (var t = this._groups, e = i._groups, s = t.length, n = e.length, a = Math.min(s, n), h = new Array(s), r = 0; r < a; ++r)
    for (var o, l = t[r], u = e[r], c = l.length, d = h[r] = new Array(c), g = 0; g < c; ++g)
      (o = l[g] || u[g]) && (d[g] = o);
  for (; r < s; ++r)
    h[r] = t[r];
  return new K(h, this._parents);
}, order: function() {
  for (var i = this._groups, t = -1, e = i.length; ++t < e; )
    for (var s, n = i[t], a = n.length - 1, h = n[a]; --a >= 0; )
      (s = n[a]) && (h && 4 ^ s.compareDocumentPosition(h) && h.parentNode.insertBefore(s, h), h = s);
  return this;
}, sort: function(i) {
  function t(c, d) {
    return c && d ? i(c.__data__, d.__data__) : !c - !d;
  }
  i || (i = Wn);
  for (var e = this._groups, s = e.length, n = new Array(s), a = 0; a < s; ++a) {
    for (var h, r = e[a], o = r.length, l = n[a] = new Array(o), u = 0; u < o; ++u)
      (h = r[u]) && (l[u] = h);
    l.sort(t);
  }
  return new K(n, this._parents).order();
}, call: function() {
  var i = arguments[0];
  return arguments[0] = this, i.apply(null, arguments), this;
}, nodes: function() {
  var i = new Array(this.size()), t = -1;
  return this.each(function() {
    i[++t] = this;
  }), i;
}, node: function() {
  for (var i = this._groups, t = 0, e = i.length; t < e; ++t)
    for (var s = i[t], n = 0, a = s.length; n < a; ++n) {
      var h = s[n];
      if (h)
        return h;
    }
  return null;
}, size: function() {
  var i = 0;
  return this.each(function() {
    ++i;
  }), i;
}, empty: function() {
  return !this.node();
}, each: function(i) {
  for (var t = this._groups, e = 0, s = t.length; e < s; ++e)
    for (var n, a = t[e], h = 0, r = a.length; h < r; ++h)
      (n = a[h]) && i.call(n, n.__data__, h, a);
  return this;
}, attr: function(i, t) {
  var e = fs(i);
  if (arguments.length < 2) {
    var s = this.node();
    return e.local ? s.getAttributeNS(e.space, e.local) : s.getAttribute(e);
  }
  return this.each((t == null ? e.local ? jn : Cn : typeof t == "function" ? e.local ? In : Gn : e.local ? On : En)(e, t));
}, style: function(i, t, e) {
  return arguments.length > 1 ? this.each((t == null ? Ln : typeof t == "function" ? Bn : Zn)(i, t, e ?? "")) : function(s, n) {
    return s.style.getPropertyValue(n) || ps(s).getComputedStyle(s, null).getPropertyValue(n);
  }(this.node(), i);
}, property: function(i, t) {
  return arguments.length > 1 ? this.each((t == null ? Tn : typeof t == "function" ? Yn : Vn)(i, t)) : this.node()[i];
}, classed: function(i, t) {
  var e = ms(i + "");
  if (arguments.length < 2) {
    for (var s = ie(this.node()), n = -1, a = e.length; ++n < a; )
      if (!s.contains(e[n]))
        return !1;
    return !0;
  }
  return this.each((typeof t == "function" ? Hn : t ? Dn : qn)(e, t));
}, text: function(i) {
  return arguments.length ? this.each(i == null ? Nn : (typeof i == "function" ? Kn : Xn)(i)) : this.node().textContent;
}, html: function(i) {
  return arguments.length ? this.each(i == null ? Qn : (typeof i == "function" ? Jn : Un)(i)) : this.node().innerHTML;
}, raise: function() {
  return this.each(_n);
}, lower: function() {
  return this.each($n);
}, append: function(i) {
  var t = typeof i == "function" ? i : Ze(i);
  return this.select(function() {
    return this.appendChild(t.apply(this, arguments));
  });
}, insert: function(i, t) {
  var e = typeof i == "function" ? i : Ze(i), s = t == null ? tr : typeof t == "function" ? t : Be(t);
  return this.select(function() {
    return this.insertBefore(e.apply(this, arguments), s.apply(this, arguments) || null);
  });
}, remove: function() {
  return this.each(ir);
}, clone: function(i) {
  return this.select(i ? sr : er);
}, datum: function(i) {
  return arguments.length ? this.property("__data__", i) : this.node().__data__;
}, on: function(i, t, e) {
  var s, n, a = function(c) {
    return c.trim().split(/^|\s+/).map(function(d) {
      var g = "", p = d.indexOf(".");
      return p >= 0 && (g = d.slice(p + 1), d = d.slice(0, p)), { type: d, name: g };
    });
  }(i + ""), h = a.length;
  if (!(arguments.length < 2)) {
    for (r = t ? ar : rr, e == null && (e = !1), s = 0; s < h; ++s)
      this.each(r(a[s], t, e));
    return this;
  }
  var r = this.node().__on;
  if (r) {
    for (var o, l = 0, u = r.length; l < u; ++l)
      for (s = 0, o = r[l]; s < h; ++s)
        if ((n = a[s]).type === o.type && n.name === o.name)
          return o.value;
  }
}, dispatch: function(i, t) {
  return this.each((typeof t == "function" ? or : hr)(i, t));
} };
class ee {
  constructor(t) {
    this.helper = t;
  }
  fillPolygons(t, e) {
    return this._fillPolygons(t, e);
  }
  _fillPolygons(t, e) {
    const s = Ht(t, e);
    return { type: "fillSketch", ops: this.renderLines(s, e) };
  }
  renderLines(t, e) {
    const s = [];
    for (const n of t)
      s.push(...this.helper.doubleLineOps(n[0][0], n[0][1], n[1][0], n[1][1], e));
    return s;
  }
}
function Ai(i) {
  const t = i[0], e = i[1];
  return Math.sqrt(Math.pow(t[0] - e[0], 2) + Math.pow(t[1] - e[1], 2));
}
class dr extends ee {
  fillPolygons(t, e) {
    let s = e.hachureGap;
    s < 0 && (s = 4 * e.strokeWidth), s = Math.max(s, 0.1);
    const n = Ht(t, Object.assign({}, e, { hachureGap: s })), a = Math.PI / 180 * e.hachureAngle, h = [], r = 0.5 * s * Math.cos(a), o = 0.5 * s * Math.sin(a);
    for (const [l, u] of n)
      Ai([l, u]) && h.push([[l[0] - r, l[1] + o], [...u]], [[l[0] + r, l[1] - o], [...u]]);
    return { type: "fillSketch", ops: this.renderLines(h, e) };
  }
}
class gr extends ee {
  fillPolygons(t, e) {
    const s = this._fillPolygons(t, e), n = Object.assign({}, e, { hachureAngle: e.hachureAngle + 90 }), a = this._fillPolygons(t, n);
    return s.ops = s.ops.concat(a.ops), s;
  }
}
class fr {
  constructor(t) {
    this.helper = t;
  }
  fillPolygons(t, e) {
    const s = Ht(t, e = Object.assign({}, e, { hachureAngle: 0 }));
    return this.dotsOnLines(s, e);
  }
  dotsOnLines(t, e) {
    const s = [];
    let n = e.hachureGap;
    n < 0 && (n = 4 * e.strokeWidth), n = Math.max(n, 0.1);
    let a = e.fillWeight;
    a < 0 && (a = e.strokeWidth / 2);
    const h = n / 4;
    for (const r of t) {
      const o = Ai(r), l = o / n, u = Math.ceil(l) - 1, c = o - u * n, d = (r[0][0] + r[1][0]) / 2 - n / 4, g = Math.min(r[0][1], r[1][1]);
      for (let p = 0; p < u; p++) {
        const f = g + c + p * n, A = d - h + 2 * Math.random() * h, m = f - h + 2 * Math.random() * h, y = this.helper.ellipse(A, m, a, a, e);
        s.push(...y.ops);
      }
    }
    return { type: "fillSketch", ops: s };
  }
}
let pr = class {
  constructor(i) {
    this.helper = i;
  }
  fillPolygons(i, t) {
    const e = Ht(i, t);
    return { type: "fillSketch", ops: this.dashedLine(e, t) };
  }
  dashedLine(i, t) {
    const e = t.dashOffset < 0 ? t.hachureGap < 0 ? 4 * t.strokeWidth : t.hachureGap : t.dashOffset, s = t.dashGap < 0 ? t.hachureGap < 0 ? 4 * t.strokeWidth : t.hachureGap : t.dashGap, n = [];
    return i.forEach((a) => {
      const h = Ai(a), r = Math.floor(h / (e + s)), o = (h + s - r * (e + s)) / 2;
      let l = a[0], u = a[1];
      l[0] > u[0] && (l = a[1], u = a[0]);
      const c = Math.atan((u[1] - l[1]) / (u[0] - l[0]));
      for (let d = 0; d < r; d++) {
        const g = d * (e + s), p = g + e, f = [l[0] + g * Math.cos(c) + o * Math.cos(c), l[1] + g * Math.sin(c) + o * Math.sin(c)], A = [l[0] + p * Math.cos(c) + o * Math.cos(c), l[1] + p * Math.sin(c) + o * Math.sin(c)];
        n.push(...this.helper.doubleLineOps(f[0], f[1], A[0], A[1], t));
      }
    }), n;
  }
};
class mr {
  constructor(t) {
    this.helper = t;
  }
  fillPolygons(t, e) {
    const s = e.hachureGap < 0 ? 4 * e.strokeWidth : e.hachureGap, n = e.zigzagOffset < 0 ? s : e.zigzagOffset, a = Ht(t, e = Object.assign({}, e, { hachureGap: s + n }));
    return { type: "fillSketch", ops: this.zigzagLines(a, n, e) };
  }
  zigzagLines(t, e, s) {
    const n = [];
    return t.forEach((a) => {
      const h = Ai(a), r = Math.round(h / (2 * e));
      let o = a[0], l = a[1];
      o[0] > l[0] && (o = a[1], l = a[0]);
      const u = Math.atan((l[1] - o[1]) / (l[0] - o[0]));
      for (let c = 0; c < r; c++) {
        const d = 2 * c * e, g = 2 * (c + 1) * e, p = Math.sqrt(2 * Math.pow(e, 2)), f = [o[0] + d * Math.cos(u), o[1] + d * Math.sin(u)], A = [o[0] + g * Math.cos(u), o[1] + g * Math.sin(u)], m = [f[0] + p * Math.cos(u + Math.PI / 4), f[1] + p * Math.sin(u + Math.PI / 4)];
        n.push(...this.helper.doubleLineOps(f[0], f[1], m[0], m[1], s), ...this.helper.doubleLineOps(m[0], m[1], A[0], A[1], s));
      }
    }), n;
  }
}
const N = {};
class Ar {
  constructor(t) {
    this.seed = t;
  }
  next() {
    return this.seed ? (2 ** 31 - 1 & (this.seed = Math.imul(48271, this.seed))) / 2 ** 31 : Math.random();
  }
}
const Kt = { A: 7, a: 7, C: 6, c: 6, H: 1, h: 1, L: 2, l: 2, M: 2, m: 2, Q: 4, q: 4, S: 4, s: 4, T: 2, t: 2, V: 1, v: 1, Z: 0, z: 0 };
function Ci(i, t) {
  return i.type === t;
}
function se(i) {
  const t = [], e = function(h) {
    const r = new Array();
    for (; h !== ""; )
      if (h.match(/^([ \t\r\n,]+)/))
        h = h.substr(RegExp.$1.length);
      else if (h.match(/^([aAcChHlLmMqQsStTvVzZ])/))
        r[r.length] = { type: 0, text: RegExp.$1 }, h = h.substr(RegExp.$1.length);
      else {
        if (!h.match(/^(([-+]?[0-9]+(\.[0-9]*)?|[-+]?\.[0-9]+)([eE][-+]?[0-9]+)?)/))
          return [];
        r[r.length] = { type: 1, text: `${parseFloat(RegExp.$1)}` }, h = h.substr(RegExp.$1.length);
      }
    return r[r.length] = { type: 2, text: "" }, r;
  }(i);
  let s = "BOD", n = 0, a = e[n];
  for (; !Ci(a, 2); ) {
    let h = 0;
    const r = [];
    if (s === "BOD") {
      if (a.text !== "M" && a.text !== "m")
        return se("M0,0" + i);
      n++, h = Kt[a.text], s = a.text;
    } else
      Ci(a, 1) ? h = Kt[s] : (n++, h = Kt[a.text], s = a.text);
    if (!(n + h < e.length))
      throw new Error("Path data ended short");
    for (let o = n; o < n + h; o++) {
      const l = e[o];
      if (!Ci(l, 1))
        throw new Error("Param not a number: " + s + "," + l.text);
      r[r.length] = +l.text;
    }
    if (typeof Kt[s] != "number")
      throw new Error("Bad segment: " + s);
    {
      const o = { key: s, data: r };
      t.push(o), n += h, a = e[n], s === "M" && (s = "L"), s === "m" && (s = "l");
    }
  }
  return t;
}
function Ss(i) {
  let t = 0, e = 0, s = 0, n = 0;
  const a = [];
  for (const { key: h, data: r } of i)
    switch (h) {
      case "M":
        a.push({ key: "M", data: [...r] }), [t, e] = r, [s, n] = r;
        break;
      case "m":
        t += r[0], e += r[1], a.push({ key: "M", data: [t, e] }), s = t, n = e;
        break;
      case "L":
        a.push({ key: "L", data: [...r] }), [t, e] = r;
        break;
      case "l":
        t += r[0], e += r[1], a.push({ key: "L", data: [t, e] });
        break;
      case "C":
        a.push({ key: "C", data: [...r] }), t = r[4], e = r[5];
        break;
      case "c": {
        const o = r.map((l, u) => u % 2 ? l + e : l + t);
        a.push({ key: "C", data: o }), t = o[4], e = o[5];
        break;
      }
      case "Q":
        a.push({ key: "Q", data: [...r] }), t = r[2], e = r[3];
        break;
      case "q": {
        const o = r.map((l, u) => u % 2 ? l + e : l + t);
        a.push({ key: "Q", data: o }), t = o[2], e = o[3];
        break;
      }
      case "A":
        a.push({ key: "A", data: [...r] }), t = r[5], e = r[6];
        break;
      case "a":
        t += r[5], e += r[6], a.push({ key: "A", data: [r[0], r[1], r[2], r[3], r[4], t, e] });
        break;
      case "H":
        a.push({ key: "H", data: [...r] }), t = r[0];
        break;
      case "h":
        t += r[0], a.push({ key: "H", data: [t] });
        break;
      case "V":
        a.push({ key: "V", data: [...r] }), e = r[0];
        break;
      case "v":
        e += r[0], a.push({ key: "V", data: [e] });
        break;
      case "S":
        a.push({ key: "S", data: [...r] }), t = r[2], e = r[3];
        break;
      case "s": {
        const o = r.map((l, u) => u % 2 ? l + e : l + t);
        a.push({ key: "S", data: o }), t = o[2], e = o[3];
        break;
      }
      case "T":
        a.push({ key: "T", data: [...r] }), t = r[0], e = r[1];
        break;
      case "t":
        t += r[0], e += r[1], a.push({ key: "T", data: [t, e] });
        break;
      case "Z":
      case "z":
        a.push({ key: "Z", data: [] }), t = s, e = n;
    }
  return a;
}
function Ms(i) {
  const t = [];
  let e = "", s = 0, n = 0, a = 0, h = 0, r = 0, o = 0;
  for (const { key: l, data: u } of i) {
    switch (l) {
      case "M":
        t.push({ key: "M", data: [...u] }), [s, n] = u, [a, h] = u;
        break;
      case "C":
        t.push({ key: "C", data: [...u] }), s = u[4], n = u[5], r = u[2], o = u[3];
        break;
      case "L":
        t.push({ key: "L", data: [...u] }), [s, n] = u;
        break;
      case "H":
        s = u[0], t.push({ key: "L", data: [s, n] });
        break;
      case "V":
        n = u[0], t.push({ key: "L", data: [s, n] });
        break;
      case "S": {
        let c = 0, d = 0;
        e === "C" || e === "S" ? (c = s + (s - r), d = n + (n - o)) : (c = s, d = n), t.push({ key: "C", data: [c, d, ...u] }), r = u[0], o = u[1], s = u[2], n = u[3];
        break;
      }
      case "T": {
        const [c, d] = u;
        let g = 0, p = 0;
        e === "Q" || e === "T" ? (g = s + (s - r), p = n + (n - o)) : (g = s, p = n);
        const f = s + 2 * (g - s) / 3, A = n + 2 * (p - n) / 3, m = c + 2 * (g - c) / 3, y = d + 2 * (p - d) / 3;
        t.push({ key: "C", data: [f, A, m, y, c, d] }), r = g, o = p, s = c, n = d;
        break;
      }
      case "Q": {
        const [c, d, g, p] = u, f = s + 2 * (c - s) / 3, A = n + 2 * (d - n) / 3, m = g + 2 * (c - g) / 3, y = p + 2 * (d - p) / 3;
        t.push({ key: "C", data: [f, A, m, y, g, p] }), r = c, o = d, s = g, n = p;
        break;
      }
      case "A": {
        const c = Math.abs(u[0]), d = Math.abs(u[1]), g = u[2], p = u[3], f = u[4], A = u[5], m = u[6];
        c === 0 || d === 0 ? (t.push({ key: "C", data: [s, n, A, m, A, m] }), s = A, n = m) : s === A && n === m || (Ps(s, n, A, m, c, d, g, p, f).forEach(function(y) {
          t.push({ key: "C", data: y });
        }), s = A, n = m);
        break;
      }
      case "Z":
        t.push({ key: "Z", data: [] }), s = a, n = h;
    }
    e = l;
  }
  return t;
}
function Wt(i, t, e) {
  return [i * Math.cos(e) - t * Math.sin(e), i * Math.sin(e) + t * Math.cos(e)];
}
function Ps(i, t, e, s, n, a, h, r, o, l) {
  const u = (c = h, Math.PI * c / 180);
  var c;
  let d = [], g = 0, p = 0, f = 0, A = 0;
  if (l)
    [g, p, f, A] = l;
  else {
    [i, t] = Wt(i, t, -u), [e, s] = Wt(e, s, -u);
    const W = (i - e) / 2, E = (t - s) / 2;
    let G = W * W / (n * n) + E * E / (a * a);
    G > 1 && (G = Math.sqrt(G), n *= G, a *= G);
    const O = n * n, I = a * a, J = O * I - O * E * E - I * W * W, _ = O * E * E + I * W * W, L = (r === o ? -1 : 1) * Math.sqrt(Math.abs(J / _));
    f = L * n * E / a + (i + e) / 2, A = L * -a * W / n + (t + s) / 2, g = Math.asin(parseFloat(((t - A) / a).toFixed(9))), p = Math.asin(parseFloat(((s - A) / a).toFixed(9))), i < f && (g = Math.PI - g), e < f && (p = Math.PI - p), g < 0 && (g = 2 * Math.PI + g), p < 0 && (p = 2 * Math.PI + p), o && g > p && (g -= 2 * Math.PI), !o && p > g && (p -= 2 * Math.PI);
  }
  let m = p - g;
  if (Math.abs(m) > 120 * Math.PI / 180) {
    const W = p, E = e, G = s;
    p = o && p > g ? g + 120 * Math.PI / 180 * 1 : g + 120 * Math.PI / 180 * -1, d = Ps(e = f + n * Math.cos(p), s = A + a * Math.sin(p), E, G, n, a, h, 0, o, [p, W, f, A]);
  }
  m = p - g;
  const y = Math.cos(g), b = Math.sin(g), k = Math.cos(p), S = Math.sin(p), v = Math.tan(m / 4), M = 4 / 3 * n * v, C = 4 / 3 * a * v, j = [i, t], z = [i + M * b, t - C * y], F = [e + M * S, s - C * k], w = [e, s];
  if (z[0] = 2 * j[0] - z[0], z[1] = 2 * j[1] - z[1], l)
    return [z, F, w].concat(d);
  {
    d = [z, F, w].concat(d);
    const W = [];
    for (let E = 0; E < d.length; E += 3) {
      const G = Wt(d[E][0], d[E][1], u), O = Wt(d[E + 1][0], d[E + 1][1], u), I = Wt(d[E + 2][0], d[E + 2][1], u);
      W.push([G[0], G[1], O[0], O[1], I[0], I[1]]);
    }
    return W;
  }
}
const yr = { randOffset: function(i, t) {
  return R(i, t);
}, randOffsetWithRange: function(i, t, e) {
  return oi(i, t, e);
}, ellipse: function(i, t, e, s, n) {
  return qi(i, t, n, zs(e, s, n)).opset;
}, doubleLineOps: function(i, t, e, s, n) {
  return ct(i, t, e, s, n, !0);
} };
function Rs(i, t, e, s, n) {
  return { type: "path", ops: ct(i, t, e, s, n) };
}
function Qt(i, t, e) {
  const s = (i || []).length;
  if (s > 2) {
    const n = [];
    for (let a = 0; a < s - 1; a++)
      n.push(...ct(i[a][0], i[a][1], i[a + 1][0], i[a + 1][1], e));
    return t && n.push(...ct(i[s - 1][0], i[s - 1][1], i[0][0], i[0][1], e)), { type: "path", ops: n };
  }
  return s === 2 ? Rs(i[0][0], i[0][1], i[1][0], i[1][1], e) : { type: "path", ops: [] };
}
function Ve(i, t) {
  if (i.length) {
    const e = typeof i[0][0] == "number" ? [i] : i, s = Ut(e[0], 1 * (1 + 0.2 * t.roughness), t), n = t.disableMultiStroke ? [] : Ut(e[0], 1.5 * (1 + 0.22 * t.roughness), qe(t));
    for (let a = 1; a < e.length; a++) {
      const h = e[a];
      if (h.length) {
        const r = Ut(h, 1 * (1 + 0.2 * t.roughness), t), o = t.disableMultiStroke ? [] : Ut(h, 1.5 * (1 + 0.22 * t.roughness), qe(t));
        for (const l of r)
          l.op !== "move" && s.push(l);
        for (const l of o)
          l.op !== "move" && n.push(l);
      }
    }
    return { type: "path", ops: s.concat(n) };
  }
  return { type: "path", ops: [] };
}
function zs(i, t, e) {
  const s = Math.sqrt(2 * Math.PI * Math.sqrt((Math.pow(i / 2, 2) + Math.pow(t / 2, 2)) / 2)), n = Math.ceil(Math.max(e.curveStepCount, e.curveStepCount / Math.sqrt(200) * s)), a = 2 * Math.PI / n;
  let h = Math.abs(i / 2), r = Math.abs(t / 2);
  const o = 1 - e.curveFitting;
  return h += R(h * o, e), r += R(r * o, e), { increment: a, rx: h, ry: r };
}
function qi(i, t, e, s) {
  const [n, a] = He(s.increment, i, t, s.rx, s.ry, 1, s.increment * oi(0.1, oi(0.4, 1, e), e), e);
  let h = li(n, null, e);
  if (!e.disableMultiStroke && e.roughness !== 0) {
    const [r] = He(s.increment, i, t, s.rx, s.ry, 1.5, 0, e), o = li(r, null, e);
    h = h.concat(o);
  }
  return { estimatedPoints: a, opset: { type: "path", ops: h } };
}
function Ye(i, t, e, s, n, a, h, r, o) {
  const l = i, u = t;
  let c = Math.abs(e / 2), d = Math.abs(s / 2);
  c += R(0.01 * c, o), d += R(0.01 * d, o);
  let g = n, p = a;
  for (; g < 0; )
    g += 2 * Math.PI, p += 2 * Math.PI;
  p - g > 2 * Math.PI && (g = 0, p = 2 * Math.PI);
  const f = 2 * Math.PI / o.curveStepCount, A = Math.min(f / 2, (p - g) / 2), m = Ne(A, l, u, c, d, g, p, 1, o);
  if (!o.disableMultiStroke) {
    const y = Ne(A, l, u, c, d, g, p, 1.5, o);
    m.push(...y);
  }
  return h && (r ? m.push(...ct(l, u, l + c * Math.cos(g), u + d * Math.sin(g), o), ...ct(l, u, l + c * Math.cos(p), u + d * Math.sin(p), o)) : m.push({ op: "lineTo", data: [l, u] }, { op: "lineTo", data: [l + c * Math.cos(g), u + d * Math.sin(g)] })), { type: "path", ops: m };
}
function De(i, t) {
  const e = Ms(Ss(se(i))), s = [];
  let n = [0, 0], a = [0, 0];
  for (const { key: h, data: r } of e)
    switch (h) {
      case "M":
        a = [r[0], r[1]], n = [r[0], r[1]];
        break;
      case "L":
        s.push(...ct(a[0], a[1], r[0], r[1], t)), a = [r[0], r[1]];
        break;
      case "C": {
        const [o, l, u, c, d, g] = r;
        s.push(...xr(o, l, u, c, d, g, a, t)), a = [d, g];
        break;
      }
      case "Z":
        s.push(...ct(a[0], a[1], n[0], n[1], t)), a = [n[0], n[1]];
    }
  return { type: "path", ops: s };
}
function ji(i, t) {
  const e = [];
  for (const s of i)
    if (s.length) {
      const n = t.maxRandomnessOffset || 0, a = s.length;
      if (a > 2) {
        e.push({ op: "move", data: [s[0][0] + R(n, t), s[0][1] + R(n, t)] });
        for (let h = 1; h < a; h++)
          e.push({ op: "lineTo", data: [s[h][0] + R(n, t), s[h][1] + R(n, t)] });
      }
    }
  return { type: "fillPath", ops: e };
}
function kt(i, t) {
  return function(e, s) {
    let n = e.fillStyle || "hachure";
    if (!N[n])
      switch (n) {
        case "zigzag":
          N[n] || (N[n] = new dr(s));
          break;
        case "cross-hatch":
          N[n] || (N[n] = new gr(s));
          break;
        case "dots":
          N[n] || (N[n] = new fr(s));
          break;
        case "dashed":
          N[n] || (N[n] = new pr(s));
          break;
        case "zigzag-line":
          N[n] || (N[n] = new mr(s));
          break;
        default:
          n = "hachure", N[n] || (N[n] = new ee(s));
      }
    return N[n];
  }(t, yr).fillPolygons(i, t);
}
function qe(i) {
  const t = Object.assign({}, i);
  return t.randomizer = void 0, i.seed && (t.seed = i.seed + 1), t;
}
function Fs(i) {
  return i.randomizer || (i.randomizer = new Ar(i.seed || 0)), i.randomizer.next();
}
function oi(i, t, e, s = 1) {
  return e.roughness * s * (Fs(e) * (t - i) + i);
}
function R(i, t, e = 1) {
  return oi(-i, i, t, e);
}
function ct(i, t, e, s, n, a = !1) {
  const h = a ? n.disableMultiStrokeFill : n.disableMultiStroke, r = Hi(i, t, e, s, n, !0, !1);
  if (h)
    return r;
  const o = Hi(i, t, e, s, n, !0, !0);
  return r.concat(o);
}
function Hi(i, t, e, s, n, a, h) {
  const r = Math.pow(i - e, 2) + Math.pow(t - s, 2), o = Math.sqrt(r);
  let l = 1;
  l = o < 200 ? 1 : o > 500 ? 0.4 : -16668e-7 * o + 1.233334;
  let u = n.maxRandomnessOffset || 0;
  u * u * 100 > r && (u = o / 10);
  const c = u / 2, d = 0.2 + 0.2 * Fs(n);
  let g = n.bowing * n.maxRandomnessOffset * (s - t) / 200, p = n.bowing * n.maxRandomnessOffset * (i - e) / 200;
  g = R(g, n, l), p = R(p, n, l);
  const f = [], A = () => R(c, n, l), m = () => R(u, n, l), y = n.preserveVertices;
  return a && (h ? f.push({ op: "move", data: [i + (y ? 0 : A()), t + (y ? 0 : A())] }) : f.push({ op: "move", data: [i + (y ? 0 : R(u, n, l)), t + (y ? 0 : R(u, n, l))] })), h ? f.push({ op: "bcurveTo", data: [g + i + (e - i) * d + A(), p + t + (s - t) * d + A(), g + i + 2 * (e - i) * d + A(), p + t + 2 * (s - t) * d + A(), e + (y ? 0 : A()), s + (y ? 0 : A())] }) : f.push({ op: "bcurveTo", data: [g + i + (e - i) * d + m(), p + t + (s - t) * d + m(), g + i + 2 * (e - i) * d + m(), p + t + 2 * (s - t) * d + m(), e + (y ? 0 : m()), s + (y ? 0 : m())] }), f;
}
function Ut(i, t, e) {
  if (!i.length)
    return [];
  const s = [];
  s.push([i[0][0] + R(t, e), i[0][1] + R(t, e)]), s.push([i[0][0] + R(t, e), i[0][1] + R(t, e)]);
  for (let n = 1; n < i.length; n++)
    s.push([i[n][0] + R(t, e), i[n][1] + R(t, e)]), n === i.length - 1 && s.push([i[n][0] + R(t, e), i[n][1] + R(t, e)]);
  return li(s, null, e);
}
function li(i, t, e) {
  const s = i.length, n = [];
  if (s > 3) {
    const a = [], h = 1 - e.curveTightness;
    n.push({ op: "move", data: [i[1][0], i[1][1]] });
    for (let r = 1; r + 2 < s; r++) {
      const o = i[r];
      a[0] = [o[0], o[1]], a[1] = [o[0] + (h * i[r + 1][0] - h * i[r - 1][0]) / 6, o[1] + (h * i[r + 1][1] - h * i[r - 1][1]) / 6], a[2] = [i[r + 1][0] + (h * i[r][0] - h * i[r + 2][0]) / 6, i[r + 1][1] + (h * i[r][1] - h * i[r + 2][1]) / 6], a[3] = [i[r + 1][0], i[r + 1][1]], n.push({ op: "bcurveTo", data: [a[1][0], a[1][1], a[2][0], a[2][1], a[3][0], a[3][1]] });
    }
    if (t && t.length === 2) {
      const r = e.maxRandomnessOffset;
      n.push({ op: "lineTo", data: [t[0] + R(r, e), t[1] + R(r, e)] });
    }
  } else
    s === 3 ? (n.push({ op: "move", data: [i[1][0], i[1][1]] }), n.push({ op: "bcurveTo", data: [i[1][0], i[1][1], i[2][0], i[2][1], i[2][0], i[2][1]] })) : s === 2 && n.push(...Hi(i[0][0], i[0][1], i[1][0], i[1][1], e, !0, !0));
  return n;
}
function He(i, t, e, s, n, a, h, r) {
  const o = [], l = [];
  if (r.roughness === 0) {
    i /= 4, l.push([t + s * Math.cos(-i), e + n * Math.sin(-i)]);
    for (let u = 0; u <= 2 * Math.PI; u += i) {
      const c = [t + s * Math.cos(u), e + n * Math.sin(u)];
      o.push(c), l.push(c);
    }
    l.push([t + s * Math.cos(0), e + n * Math.sin(0)]), l.push([t + s * Math.cos(i), e + n * Math.sin(i)]);
  } else {
    const u = R(0.5, r) - Math.PI / 2;
    l.push([R(a, r) + t + 0.9 * s * Math.cos(u - i), R(a, r) + e + 0.9 * n * Math.sin(u - i)]);
    const c = 2 * Math.PI + u - 0.01;
    for (let d = u; d < c; d += i) {
      const g = [R(a, r) + t + s * Math.cos(d), R(a, r) + e + n * Math.sin(d)];
      o.push(g), l.push(g);
    }
    l.push([R(a, r) + t + s * Math.cos(u + 2 * Math.PI + 0.5 * h), R(a, r) + e + n * Math.sin(u + 2 * Math.PI + 0.5 * h)]), l.push([R(a, r) + t + 0.98 * s * Math.cos(u + h), R(a, r) + e + 0.98 * n * Math.sin(u + h)]), l.push([R(a, r) + t + 0.9 * s * Math.cos(u + 0.5 * h), R(a, r) + e + 0.9 * n * Math.sin(u + 0.5 * h)]);
  }
  return [l, o];
}
function Ne(i, t, e, s, n, a, h, r, o) {
  const l = a + R(0.1, o), u = [];
  u.push([R(r, o) + t + 0.9 * s * Math.cos(l - i), R(r, o) + e + 0.9 * n * Math.sin(l - i)]);
  for (let c = l; c <= h; c += i)
    u.push([R(r, o) + t + s * Math.cos(c), R(r, o) + e + n * Math.sin(c)]);
  return u.push([t + s * Math.cos(h), e + n * Math.sin(h)]), u.push([t + s * Math.cos(h), e + n * Math.sin(h)]), li(u, null, o);
}
function xr(i, t, e, s, n, a, h, r) {
  const o = [], l = [r.maxRandomnessOffset || 1, (r.maxRandomnessOffset || 1) + 0.3];
  let u = [0, 0];
  const c = r.disableMultiStroke ? 1 : 2, d = r.preserveVertices;
  for (let g = 0; g < c; g++)
    g === 0 ? o.push({ op: "move", data: [h[0], h[1]] }) : o.push({ op: "move", data: [h[0] + (d ? 0 : R(l[0], r)), h[1] + (d ? 0 : R(l[0], r))] }), u = d ? [n, a] : [n + R(l[g], r), a + R(l[g], r)], o.push({ op: "bcurveTo", data: [i + R(l[g], r), t + R(l[g], r), e + R(l[g], r), s + R(l[g], r), u[0], u[1]] });
  return o;
}
function Ct(i) {
  return [...i];
}
function Xe(i, t = 0) {
  const e = i.length;
  if (e < 3)
    throw new Error("A curve must have at least three points.");
  const s = [];
  if (e === 3)
    s.push(Ct(i[0]), Ct(i[1]), Ct(i[2]), Ct(i[2]));
  else {
    const n = [];
    n.push(i[0], i[0]);
    for (let r = 1; r < i.length; r++)
      n.push(i[r]), r === i.length - 1 && n.push(i[r]);
    const a = [], h = 1 - t;
    s.push(Ct(n[0]));
    for (let r = 1; r + 2 < n.length; r++) {
      const o = n[r];
      a[0] = [o[0], o[1]], a[1] = [o[0] + (h * n[r + 1][0] - h * n[r - 1][0]) / 6, o[1] + (h * n[r + 1][1] - h * n[r - 1][1]) / 6], a[2] = [n[r + 1][0] + (h * n[r][0] - h * n[r + 2][0]) / 6, n[r + 1][1] + (h * n[r][1] - h * n[r + 2][1]) / 6], a[3] = [n[r + 1][0], n[r + 1][1]], s.push(a[1], a[2], a[3]);
    }
  }
  return s;
}
function ti(i, t) {
  return Math.pow(i[0] - t[0], 2) + Math.pow(i[1] - t[1], 2);
}
function vr(i, t, e) {
  const s = ti(t, e);
  if (s === 0)
    return ti(i, t);
  let n = ((i[0] - t[0]) * (e[0] - t[0]) + (i[1] - t[1]) * (e[1] - t[1])) / s;
  return n = Math.max(0, Math.min(1, n)), ti(i, pt(t, e, n));
}
function pt(i, t, e) {
  return [i[0] + (t[0] - i[0]) * e, i[1] + (t[1] - i[1]) * e];
}
function Ni(i, t, e, s) {
  const n = s || [];
  if (function(r, o) {
    const l = r[o + 0], u = r[o + 1], c = r[o + 2], d = r[o + 3];
    let g = 3 * u[0] - 2 * l[0] - d[0];
    g *= g;
    let p = 3 * u[1] - 2 * l[1] - d[1];
    p *= p;
    let f = 3 * c[0] - 2 * d[0] - l[0];
    f *= f;
    let A = 3 * c[1] - 2 * d[1] - l[1];
    return A *= A, g < f && (g = f), p < A && (p = A), g + p;
  }(i, t) < e) {
    const r = i[t + 0];
    n.length ? (a = n[n.length - 1], h = r, Math.sqrt(ti(a, h)) > 1 && n.push(r)) : n.push(r), n.push(i[t + 3]);
  } else {
    const o = i[t + 0], l = i[t + 1], u = i[t + 2], c = i[t + 3], d = pt(o, l, 0.5), g = pt(l, u, 0.5), p = pt(u, c, 0.5), f = pt(d, g, 0.5), A = pt(g, p, 0.5), m = pt(f, A, 0.5);
    Ni([o, d, f, m], 0, e, n), Ni([m, A, p, c], 0, e, n);
  }
  var a, h;
  return n;
}
function br(i, t) {
  return ui(i, 0, i.length, t);
}
function ui(i, t, e, s, n) {
  const a = n || [], h = i[t], r = i[e - 1];
  let o = 0, l = 1;
  for (let u = t + 1; u < e - 1; ++u) {
    const c = vr(i[u], h, r);
    c > o && (o = c, l = u);
  }
  return Math.sqrt(o) > s ? (ui(i, t, l + 1, s, a), ui(i, l, e, s, a)) : (a.length || a.push(h), a.push(r)), a;
}
function Ei(i, t = 0.15, e) {
  const s = [], n = (i.length - 1) / 3;
  for (let a = 0; a < n; a++)
    Ni(i, 3 * a, t, s);
  return e && e > 0 ? ui(s, 0, s.length, e) : s;
}
const Q = "none";
class kr {
  constructor(t) {
    this.defaultOptions = { maxRandomnessOffset: 2, roughness: 1, bowing: 1, stroke: "#000", strokeWidth: 1, curveTightness: 0, curveFitting: 0.95, curveStepCount: 9, fillStyle: "hachure", fillWeight: -1, hachureAngle: -41, hachureGap: -1, dashOffset: -1, dashGap: -1, zigzagOffset: -1, seed: 0, disableMultiStroke: !1, disableMultiStrokeFill: !1, preserveVertices: !1, fillShapeRoughnessGain: 0.8 }, this.config = t || {}, this.config.options && (this.defaultOptions = this._o(this.config.options));
  }
  static newSeed() {
    return Math.floor(Math.random() * 2 ** 31);
  }
  _o(t) {
    return t ? Object.assign({}, this.defaultOptions, t) : this.defaultOptions;
  }
  _d(t, e, s) {
    return { shape: t, sets: e || [], options: s || this.defaultOptions };
  }
  line(t, e, s, n, a) {
    const h = this._o(a);
    return this._d("line", [Rs(t, e, s, n, h)], h);
  }
  rectangle(t, e, s, n, a) {
    const h = this._o(a), r = [], o = function(l, u, c, d, g) {
      return function(p, f) {
        return Qt(p, !0, f);
      }([[l, u], [l + c, u], [l + c, u + d], [l, u + d]], g);
    }(t, e, s, n, h);
    if (h.fill) {
      const l = [[t, e], [t + s, e], [t + s, e + n], [t, e + n]];
      h.fillStyle === "solid" ? r.push(ji([l], h)) : r.push(kt([l], h));
    }
    return h.stroke !== Q && r.push(o), this._d("rectangle", r, h);
  }
  ellipse(t, e, s, n, a) {
    const h = this._o(a), r = [], o = zs(s, n, h), l = qi(t, e, h, o);
    if (h.fill)
      if (h.fillStyle === "solid") {
        const u = qi(t, e, h, o).opset;
        u.type = "fillPath", r.push(u);
      } else
        r.push(kt([l.estimatedPoints], h));
    return h.stroke !== Q && r.push(l.opset), this._d("ellipse", r, h);
  }
  circle(t, e, s, n) {
    const a = this.ellipse(t, e, s, s, n);
    return a.shape = "circle", a;
  }
  linearPath(t, e) {
    const s = this._o(e);
    return this._d("linearPath", [Qt(t, !1, s)], s);
  }
  arc(t, e, s, n, a, h, r = !1, o) {
    const l = this._o(o), u = [], c = Ye(t, e, s, n, a, h, r, !0, l);
    if (r && l.fill)
      if (l.fillStyle === "solid") {
        const d = Object.assign({}, l);
        d.disableMultiStroke = !0;
        const g = Ye(t, e, s, n, a, h, !0, !1, d);
        g.type = "fillPath", u.push(g);
      } else
        u.push(function(d, g, p, f, A, m, y) {
          const b = d, k = g;
          let S = Math.abs(p / 2), v = Math.abs(f / 2);
          S += R(0.01 * S, y), v += R(0.01 * v, y);
          let M = A, C = m;
          for (; M < 0; )
            M += 2 * Math.PI, C += 2 * Math.PI;
          C - M > 2 * Math.PI && (M = 0, C = 2 * Math.PI);
          const j = (C - M) / y.curveStepCount, z = [];
          for (let F = M; F <= C; F += j)
            z.push([b + S * Math.cos(F), k + v * Math.sin(F)]);
          return z.push([b + S * Math.cos(C), k + v * Math.sin(C)]), z.push([b, k]), kt([z], y);
        }(t, e, s, n, a, h, l));
    return l.stroke !== Q && u.push(c), this._d("arc", u, l);
  }
  curve(t, e) {
    const s = this._o(e), n = [], a = Ve(t, s);
    if (s.fill && s.fill !== Q)
      if (s.fillStyle === "solid") {
        const h = Ve(t, Object.assign(Object.assign({}, s), { disableMultiStroke: !0, roughness: s.roughness ? s.roughness + s.fillShapeRoughnessGain : 0 }));
        n.push({ type: "fillPath", ops: this._mergedShape(h.ops) });
      } else {
        const h = [], r = t;
        if (r.length) {
          const o = typeof r[0][0] == "number" ? [r] : r;
          for (const l of o)
            l.length < 3 ? h.push(...l) : l.length === 3 ? h.push(...Ei(Xe([l[0], l[0], l[1], l[2]]), 10, (1 + s.roughness) / 2)) : h.push(...Ei(Xe(l), 10, (1 + s.roughness) / 2));
        }
        h.length && n.push(kt([h], s));
      }
    return s.stroke !== Q && n.push(a), this._d("curve", n, s);
  }
  polygon(t, e) {
    const s = this._o(e), n = [], a = Qt(t, !0, s);
    return s.fill && (s.fillStyle === "solid" ? n.push(ji([t], s)) : n.push(kt([t], s))), s.stroke !== Q && n.push(a), this._d("polygon", n, s);
  }
  path(t, e) {
    const s = this._o(e), n = [];
    if (!t)
      return this._d("path", n, s);
    t = (t || "").replace(/\n/g, " ").replace(/(-\s)/g, "-").replace("/(ss)/g", " ");
    const a = s.fill && s.fill !== "transparent" && s.fill !== Q, h = s.stroke !== Q, r = !!(s.simplification && s.simplification < 1), o = function(u, c, d) {
      const g = Ms(Ss(se(u))), p = [];
      let f = [], A = [0, 0], m = [];
      const y = () => {
        m.length >= 4 && f.push(...Ei(m, 1)), m = [];
      }, b = () => {
        y(), f.length && (p.push(f), f = []);
      };
      for (const { key: S, data: v } of g)
        switch (S) {
          case "M":
            b(), A = [v[0], v[1]], f.push(A);
            break;
          case "L":
            y(), f.push([v[0], v[1]]);
            break;
          case "C":
            if (!m.length) {
              const M = f.length ? f[f.length - 1] : A;
              m.push([M[0], M[1]]);
            }
            m.push([v[0], v[1]]), m.push([v[2], v[3]]), m.push([v[4], v[5]]);
            break;
          case "Z":
            y(), f.push([A[0], A[1]]);
        }
      if (b(), !d)
        return p;
      const k = [];
      for (const S of p) {
        const v = br(S, d);
        v.length && k.push(v);
      }
      return k;
    }(t, 0, r ? 4 - 4 * (s.simplification || 1) : (1 + s.roughness) / 2), l = De(t, s);
    if (a)
      if (s.fillStyle === "solid")
        if (o.length === 1) {
          const u = De(t, Object.assign(Object.assign({}, s), { disableMultiStroke: !0, roughness: s.roughness ? s.roughness + s.fillShapeRoughnessGain : 0 }));
          n.push({ type: "fillPath", ops: this._mergedShape(u.ops) });
        } else
          n.push(ji(o, s));
      else
        n.push(kt(o, s));
    return h && (r ? o.forEach((u) => {
      n.push(Qt(u, !1, s));
    }) : n.push(l)), this._d("path", n, s);
  }
  opsToPath(t, e) {
    let s = "";
    for (const n of t.ops) {
      const a = typeof e == "number" && e >= 0 ? n.data.map((h) => +h.toFixed(e)) : n.data;
      switch (n.op) {
        case "move":
          s += `M${a[0]} ${a[1]} `;
          break;
        case "bcurveTo":
          s += `C${a[0]} ${a[1]}, ${a[2]} ${a[3]}, ${a[4]} ${a[5]} `;
          break;
        case "lineTo":
          s += `L${a[0]} ${a[1]} `;
      }
    }
    return s.trim();
  }
  toPaths(t) {
    const e = t.sets || [], s = t.options || this.defaultOptions, n = [];
    for (const a of e) {
      let h = null;
      switch (a.type) {
        case "path":
          h = { d: this.opsToPath(a), stroke: s.stroke, strokeWidth: s.strokeWidth, fill: Q };
          break;
        case "fillPath":
          h = { d: this.opsToPath(a), stroke: Q, strokeWidth: 0, fill: s.fill || Q };
          break;
        case "fillSketch":
          h = this.fillSketch(a, s);
      }
      h && n.push(h);
    }
    return n;
  }
  fillSketch(t, e) {
    let s = e.fillWeight;
    return s < 0 && (s = e.strokeWidth / 2), { d: this.opsToPath(t), stroke: e.fill || Q, strokeWidth: s, fill: Q };
  }
  _mergedShape(t) {
    return t.filter((e, s) => s === 0 || e.op !== "move");
  }
}
const Jt = "http://www.w3.org/2000/svg";
class wr {
  constructor(t, e) {
    this.svg = t, this.gen = new kr(e);
  }
  draw(t) {
    const e = t.sets || [], s = t.options || this.getDefaultOptions(), n = this.svg.ownerDocument || window.document, a = n.createElementNS(Jt, "g"), h = t.options.fixedDecimalPlaceDigits;
    for (const r of e) {
      let o = null;
      switch (r.type) {
        case "path":
          o = n.createElementNS(Jt, "path"), o.setAttribute("d", this.opsToPath(r, h)), o.setAttribute("stroke", s.stroke), o.setAttribute("stroke-width", s.strokeWidth + ""), o.setAttribute("fill", "none"), s.strokeLineDash && o.setAttribute("stroke-dasharray", s.strokeLineDash.join(" ").trim()), s.strokeLineDashOffset && o.setAttribute("stroke-dashoffset", `${s.strokeLineDashOffset}`);
          break;
        case "fillPath":
          o = n.createElementNS(Jt, "path"), o.setAttribute("d", this.opsToPath(r, h)), o.setAttribute("stroke", "none"), o.setAttribute("stroke-width", "0"), o.setAttribute("fill", s.fill || ""), t.shape !== "curve" && t.shape !== "polygon" || o.setAttribute("fill-rule", "evenodd");
          break;
        case "fillSketch":
          o = this.fillSketch(n, r, s);
      }
      o && a.appendChild(o);
    }
    return a;
  }
  fillSketch(t, e, s) {
    let n = s.fillWeight;
    n < 0 && (n = s.strokeWidth / 2);
    const a = t.createElementNS(Jt, "path");
    return a.setAttribute("d", this.opsToPath(e, s.fixedDecimalPlaceDigits)), a.setAttribute("stroke", s.fill || ""), a.setAttribute("stroke-width", n + ""), a.setAttribute("fill", "none"), s.fillLineDash && a.setAttribute("stroke-dasharray", s.fillLineDash.join(" ").trim()), s.fillLineDashOffset && a.setAttribute("stroke-dashoffset", `${s.fillLineDashOffset}`), a;
  }
  get generator() {
    return this.gen;
  }
  getDefaultOptions() {
    return this.gen.defaultOptions;
  }
  opsToPath(t, e) {
    return this.gen.opsToPath(t, e);
  }
  line(t, e, s, n, a) {
    const h = this.gen.line(t, e, s, n, a);
    return this.draw(h);
  }
  rectangle(t, e, s, n, a) {
    const h = this.gen.rectangle(t, e, s, n, a);
    return this.draw(h);
  }
  ellipse(t, e, s, n, a) {
    const h = this.gen.ellipse(t, e, s, n, a);
    return this.draw(h);
  }
  circle(t, e, s, n) {
    const a = this.gen.circle(t, e, s, n);
    return this.draw(a);
  }
  linearPath(t, e) {
    const s = this.gen.linearPath(t, e);
    return this.draw(s);
  }
  polygon(t, e) {
    const s = this.gen.polygon(t, e);
    return this.draw(s);
  }
  arc(t, e, s, n, a, h, r = !1, o) {
    const l = this.gen.arc(t, e, s, n, a, h, r, o);
    return this.draw(l);
  }
  curve(t, e) {
    const s = this.gen.curve(t, e);
    return this.draw(s);
  }
  path(t, e) {
    const s = this.gen.path(t, e);
    return this.draw(s);
  }
}
var T = (i, t) => new wr(i, t);
class ot {
  constructor(t) {
    this.el = t.element, this.element = t.element, this.title = t.title, this.titleFontSize = t.titleFontSize || "17px", this.font = t.font || 0, this.fillStyle = t.fillStyle, this.tooltipFontSize = t.tooltipFontSize || "0.95rem", this.bowing = t.bowing || 0, this.simplification = t.simplification || 0.2, this.interactive = t.interactive !== !1, this.dataFormat = typeof t.data == "object" ? "object" : "file";
  }
  setSvg() {
    this.svg = x(this.el).append("svg").attr("viewBox", `0 0 ${this.width + this.margin.left + this.margin.right}
       ${this.height + this.margin.top + this.margin.bottom}`).append("g").attr("id", this.roughId).attr("transform", "translate(" + this.margin.left + "," + this.margin.top + ")");
  }
  resolveFont() {
    this.font === 0 || this.font === void 0 || this.font.toString().toLowerCase() === "gaegu" ? (this.svg.append("defs").append("style").attr("type", "text/css").text(`@font-face {
    font-family: 'gaeguregular';
    src: url(data:application/font-woff2;charset=utf-8;base64,) format('woff2'),
         url(data:application/font-woff;charset=utf-8;base64,) format('woff');
    font-weight: normal;
    font-style: normal;
}`), this.fontFamily = "gaeguregular") : this.font === 1 || this.font.toString().toLowerCase() === "indie flower" ? (((t) => {
      t.append("defs").append("style").attr("type", "text/css").text(`@font-face {
    font-family: 'indie_flowerregular';
    src: url(data:application/font-woff2;charset=utf-8;base64,) format('woff2'),
         url(data:application/font-woff;charset=utf-8;base64,) format('woff');
    font-weight: normal;
    font-style: normal;
	}`);
    })(this.svg), this.fontFamily = "indie_flowerregular") : this.fontFamily = this.font;
  }
}
const st = ({ roughness: i, ceiling: t = 20, defaultValue: e = 1 }) => i === void 0 || typeof i != "number" ? e : i > t ? t : i;
class Xr extends ot {
  constructor(t) {
    super(t), this.data = t.data, this.margin = t.margin || { top: 20, right: 10, bottom: 20, left: 20 }, this.color = t.color || "red", this.highlight = t.highlight || "coral", this.roughness = st({ roughness: t.roughness }), this.stroke = t.stroke || "black", this.strokeWidth = t.strokeWidth || 1, this.axisStrokeWidth = t.axisStrokeWidth || 0.5, this.axisRoughness = t.axisRoughness || 0.5, this.innerStrokeWidth = t.innerStrokeWidth || 1, this.fillWeight = t.fillWeight || 0.5, this.axisFontSize = t.axisFontSize, this.labels = this.dataFormat === "object" ? "labels" : t.labels, this.values = this.dataFormat === "object" ? "values" : t.values, this.xValueFormat = t.xValueFormat, this.yValueFormat = t.yValueFormat, this.padding = t.padding || 0.1, this.xLabel = t.xLabel || "", this.yLabel = t.yLabel || "", this.labelFontSize = t.labelFontSize || "1rem", this.responsive = !0, this.boundRedraw = this.redraw.bind(this, t), this.initChartValues(t), this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title), window.addEventListener("resize", this.resizeHandler.bind(this));
  }
  resizeHandler() {
    this.responsive && this.boundRedraw();
  }
  remove() {
    x(this.el).select("svg").remove(), x(this.el).select(".tooltip").remove();
  }
  redraw(t) {
    this.remove(), this.initChartValues(t), this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title);
  }
  initChartValues(t) {
    this.roughness = t.roughness || this.roughness, this.color = t.color || this.color, this.stroke = t.stroke || this.stroke, this.strokeWidth = t.strokeWidth || this.strokeWidth, this.axisStrokeWidth = t.axisStrokeWidth || this.axisStrokeWidth, this.axisRoughness = t.axisRoughness || this.axisRoughness, this.innerStrokeWidth = t.innerStrokeWidth || this.innerStrokeWidth, this.fillWeight = t.fillWeight || this.fillWeight, this.fillStyle = t.fillStyle || this.fillStyle, this.title = t.title || this.title;
    const e = x(this.el).node().getBoundingClientRect(), s = e.width, n = e.height;
    this.width = s - this.margin.left - this.margin.right, this.height = n - this.margin.top - this.margin.bottom, this.roughId = this.el + "_svg", this.graphClass = this.el.substring(1, this.el.length), this.interactionG = "g." + this.graphClass, this.setSvg();
  }
  resolveData(t) {
    return typeof t != "string" ? () => {
      this.data = t, this.drawFromObject();
    } : t.includes(".csv") ? () => {
      xt(t).then((e) => {
        this.data = e, this.drawFromFile();
      });
    } : t.includes(".tsv") ? () => {
      vt(t).then((e) => {
        this.data = e, this.drawFromFile();
      });
    } : void 0;
  }
  addScales() {
    const t = this;
    this.xScale = qt().rangeRound([0, this.width]).padding(this.padding).domain(this.dataFormat === "file" ? this.data.map((e) => e[t.labels]) : this.data[t.labels]), this.yScale = Z().rangeRound([this.height, 0]).domain(this.dataFormat === "file" ? [0, q(this.data, (e) => +e[t.values])] : [0, q(this.data[t.values])]);
  }
  addLabels() {
    this.xLabel !== "" && this.svg.append("text").attr("x", this.width / 2).attr("y", this.height + this.margin.bottom / 2).attr("dx", "1em").attr("class", "labelText").style("text-anchor", "middle").style("font-family", this.fontFamily).style("font-size", this.labelFontSize).text(this.xLabel), this.yLabel !== "" && this.svg.append("text").attr("transform", "rotate(-90)").attr("y", 0 - this.margin.left / 1.4).attr("x", 0 - this.height / 2).attr("dy", "1em").attr("class", "labelText").style("text-anchor", "middle").style("font-family", this.fontFamily).style("font-size", this.labelFontSize).text(this.yLabel);
  }
  addAxes() {
    const t = Yt(this.xScale).tickSize(0).tickFormat((s) => this.xValueFormat ? ht(this.xValueFormat)(s) : s), e = Dt(this.yScale).tickSize(0).tickFormat((s) => this.yValueFormat ? ht(this.yValueFormat)(s) : s);
    this.svg.append("g").attr("transform", "translate(0," + this.height + ")").call(t).attr("class", `xAxis${this.graphClass}`).selectAll("text").attr("transform", "translate(-10,0)rotate(-45)").style("text-anchor", "end").style("font-family", this.fontFamily).style("font-size", this.axisFontSize === void 0 ? `${Math.min(0.8, Math.min(this.width, this.height) / 140)}rem` : this.axisFontSize).style("opacity", 0.9), this.svg.append("g").call(e).attr("class", `yAxis${this.graphClass}`).selectAll("text").style("font-family", this.fontFamily).style("font-size", this.axisFontSize === void 0 ? `${Math.min(0.95, Math.min(this.width, this.height) / 140)}rem` : this.axisFontSize).style("opacity", 0.9), P("path.domain").attr("stroke", "transparent");
  }
  makeAxesRough(t, e) {
    const s = `xAxis${this.graphClass}`, n = `yAxis${this.graphClass}`, a = `rough-${s}`, h = `rough-${n}`;
    x(`.${s}`).selectAll("path.domain").each(function(r, o) {
      const l = x(this).node().getAttribute("d"), u = e.path(l, { fillStyle: "hachure" });
      u.setAttribute("class", a), t.appendChild(u);
    }), P(`.${a}`).attr("transform", `translate(0, ${this.height})`), x(`.${n}`).selectAll("path.domain").each(function(r, o) {
      const l = x(this).node().getAttribute("d"), u = e.path(l, { fillStyle: "hachure" });
      u.setAttribute("class", h), t.appendChild(u);
    });
  }
  setTitle(t) {
    this.svg.append("text").attr("x", this.width / 2).attr("y", 0 - this.margin.top / 2).attr("class", "title").attr("text-anchor", "middle").style("font-size", this.titleFontSize === void 0 ? `${Math.min(40, Math.min(this.width, this.height) / 5)}px` : this.titleFontSize).style("font-family", this.fontFamily).style("opacity", 0.8).text(t);
  }
  addInteraction() {
    P(this.interactionG).data(this.dataFormat === "file" ? this.data : this.data.values).append("rect").attr("x", (s, n) => this.dataFormat === "file" ? this.xScale(s[this.labels]) : this.xScale(this.data[this.labels][n])).attr("y", (s, n) => this.dataFormat === "file" ? this.yScale(+s[this.values]) : this.yScale(this.data[this.values][n])).attr("width", this.xScale.bandwidth()).attr("height", (s, n) => this.dataFormat === "file" ? this.height - this.yScale(+s[this.values]) : this.height - this.yScale(this.data[this.values][n])).attr("fill", "transparent");
    const t = x(this.el).append("div").style("opacity", 0).attr("class", "tooltip").style("position", "absolute").style("background-color", "white").style("border", "solid").style("border-width", "1px").style("border-radius", "5px").style("padding", "3px").style("font-family", this.fontFamily).style("font-size", this.tooltipFontSize).style("pointer-events", "none"), e = this;
    P(this.interactionG).on("mouseover", function() {
      t.style("opacity", 1), x(this).select("path").style("stroke", e.highlight), x(this).selectAll("path:nth-child(2)").style("stroke-width", e.strokeWidth + 1.2);
    }), P(this.interactionG).on("mouseout", function() {
      t.style("opacity", 0), x(this).select("path").style("stroke", e.color), x(this).selectAll("path:nth-child(2)").style("stroke-width", e.strokeWidth);
    }), P(this.interactionG).on("mousemove", function(s) {
      const n = x(this).attr("attrX"), a = x(this).attr("attrY"), h = bt(this);
      t.html(`<b>${n}</b>: ${a}`).style("opacity", 0.95).style("transform", `translate(${h[0] + 10 + e.margin.left}px, 
              ${h[1] - 10 - (e.height + e.margin.top + e.margin.bottom / 2)}px)`);
    });
  }
  initRoughObjects() {
    this.roughSvg = document.getElementById(this.roughId), this.rcAxis = T(this.roughSvg, { options: { strokeWidth: this.axisStrokeWidth, roughness: this.axisRoughness } }), this.rc = T(this.roughSvg, { options: { fill: this.color, stroke: this.stroke === "none" ? void 0 : this.stroke, strokeWidth: this.innerStrokeWidth, roughness: this.roughness, bowing: this.bowing, fillStyle: this.fillStyle } });
  }
  drawFromObject() {
    this.initRoughObjects(), this.addScales(), this.addAxes(), this.makeAxesRough(this.roughSvg, this.rcAxis), this.addLabels(), this.data.values.forEach((t, e) => {
      const s = this.rc.rectangle(this.xScale(this.data[this.labels][e]), this.yScale(+t), this.xScale.bandwidth(), this.height - this.yScale(+t), { simplification: this.simplification, fillWeight: this.fillWeight }), n = this.roughSvg.appendChild(s);
      n.setAttribute("class", this.graphClass), n.setAttribute("attrX", this.data[this.labels][e]), n.setAttribute("attrY", +t);
    }), P(this.interactionG).selectAll("path:nth-child(2)").style("stroke-width", this.strokeWidth), this.interactive === !0 && this.addInteraction();
  }
  drawFromFile() {
    this.initRoughObjects(), this.addScales(), this.addAxes(), this.makeAxesRough(this.roughSvg, this.rcAxis), this.addLabels(), this.data.forEach((t) => {
      const e = this.rc.rectangle(this.xScale(t[this.labels]), this.yScale(+t[this.values]), this.xScale.bandwidth(), this.height - this.yScale(+t[this.values]), { simplification: this.simplification, fillWeight: this.fillWeight }), s = this.roughSvg.appendChild(e);
      s.setAttribute("class", this.graphClass), s.setAttribute("attrX", t[this.labels]), s.setAttribute("attrY", +t[this.values]);
    }), P(this.interactionG).selectAll("path:nth-child(2)").style("stroke-width", this.strokeWidth), this.interactive === !0 && this.addInteraction();
  }
}
class Kr extends ot {
  constructor(t) {
    super(t), this.margin = t.margin || { top: 50, right: 20, bottom: 50, left: 100 }, this.color = t.color || "red", this.highlight = t.highlight || "coral", this.roughness = st({ roughness: t.roughness }), this.stroke = t.stroke || "black", this.strokeWidth = t.strokeWidth || 1, this.axisStrokeWidth = t.axisStrokeWidth || 0.5, this.axisRoughness = t.axisRoughness || 0.5, this.innerStrokeWidth = t.innerStrokeWidth || 1, this.fillWeight = t.fillWeight || 0.5, this.axisFontSize = t.axisFontSize, this.labels = this.dataFormat === "object" ? "labels" : t.labels, this.values = this.dataFormat === "object" ? "values" : t.values, this.xValueFormat = t.xValueFormat, this.yValueFormat = t.yValueFormat, this.padding = t.padding || 0.1, this.xLabel = t.xLabel || "", this.yLabel = t.yLabel || "", this.labelFontSize = t.labelFontSize || "1rem", this.responsive = !0, this.boundRedraw = this.redraw.bind(this, t), this.initChartValues(t), this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title), window.addEventListener("resize", this.resizeHandler.bind(this));
  }
  resizeHandler() {
    this.responsive && this.boundRedraw();
  }
  remove() {
    x(this.el).select("svg").remove();
  }
  redraw(t) {
    this.remove(), this.initChartValues(t), this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title);
  }
  initChartValues(t) {
    this.roughness = t.roughness || this.roughness, this.stroke = t.stroke || this.stroke, this.color = t.color || this.color, this.strokeWidth = t.strokeWidth || this.strokeWidth, this.axisStrokeWidth = t.axisStrokeWidth || this.axisStrokeWidth, this.axisRoughness = t.axisRoughness || this.axisRoughness, this.innerStrokeWidth = t.innerStrokeWidth || this.innerStrokeWidth, this.fillWeight = t.fillWeight || this.fillWeight, this.fillStyle = t.fillStyle || this.fillStyle;
    const e = x(this.el).node().getBoundingClientRect(), s = e.width, n = e.height;
    this.width = s - this.margin.left - this.margin.right, this.height = n - this.margin.top - this.margin.bottom, this.roughId = this.el + "_svg", this.graphClass = this.el.substring(1, this.el.length), this.interactionG = "g." + this.graphClass, this.setSvg();
  }
  resolveData(t) {
    return typeof t != "string" ? () => {
      this.data = t, this.drawFromObject();
    } : t.includes(".csv") ? () => {
      xt(t).then((e) => {
        this.data = e, this.drawFromFile();
      });
    } : t.includes(".tsv") ? () => {
      vt(t).then((e) => {
        this.data = e, this.drawFromFile();
      });
    } : void 0;
  }
  addScales() {
    const t = this;
    this.yScale = qt().rangeRound([0, this.height]).padding(this.padding).domain(this.dataFormat === "file" ? this.data.map((e) => e[t.labels]) : this.data[t.labels]), this.xScale = Z().rangeRound([0, this.width]).domain(this.dataFormat === "file" ? [0, q(this.data, (e) => +e[t.values])] : [0, q(this.data[t.values])]);
  }
  addLabels() {
    this.xLabel !== "" && this.svg.append("text").attr("x", this.width / 2).attr("y", this.height + this.margin.bottom / 2.4).attr("dx", "1em").attr("class", "labelText").style("text-anchor", "middle").style("font-family", this.fontFamily).style("font-size", this.labelFontSize).text(this.xLabel), this.yLabel !== "" && this.svg.append("text").attr("transform", "rotate(-90)").attr("y", 0 - this.margin.left / 1.5).attr("x", 0 - this.height / 2).attr("dy", "1em").attr("class", "labelText").style("text-anchor", "middle").style("font-family", this.fontFamily).style("font-size", this.labelFontSize).text(this.yLabel);
  }
  addAxes() {
    const t = Yt(this.xScale).tickSize(0).tickFormat((s) => this.xValueFormat ? ht(this.xValueFormat)(s) : s), e = Dt(this.yScale).tickSize(0).tickFormat((s) => this.yValueFormat ? ht(this.yValueFormat)(s) : s);
    this.svg.append("g").attr("transform", `translate(0, ${this.height})`).call(t).attr("class", `xAxis${this.graphClass}`).selectAll("text").attr("transform", "translate(-10,0)rotate(-45)").style("text-anchor", "end").style("font-family", this.fontFamily).style("font-size", this.axisFontSize === void 0 ? `${Math.min(0.95, Math.min(this.width, this.height) / 140)}rem` : this.axisFontSize).style("opacity", 0.85), this.svg.append("g").call(e).attr("class", `yAxis${this.graphClass}`).selectAll("text").style("font-family", this.fontFamily).style("font-size", this.axisFontSize === void 0 ? `${Math.min(0.95, Math.min(this.width, this.height) / 140)}rem` : this.axisFontSize).style("opacity", 0.85), P("path.domain").attr("stroke", "transparent");
  }
  makeAxesRough(t, e) {
    const s = `xAxis${this.graphClass}`, n = `yAxis${this.graphClass}`, a = `rough-${s}`, h = `rough-${n}`;
    x(`.${s}`).selectAll("path.domain").each(function(r, o) {
      const l = x(this).node().getAttribute("d"), u = e.path(l, { stroke: "black", fillStyle: "hachure" });
      u.setAttribute("class", a), t.appendChild(u);
    }), P(`.${a}`).attr("transform", `translate(0, ${this.height})`), x(`.${n}`).selectAll("path.domain").each(function(r, o) {
      const l = x(this).node().getAttribute("d"), u = e.path(l, { stroke: "black", fillStyle: "hachure" });
      u.setAttribute("class", h), t.appendChild(u);
    });
  }
  setTitle(t) {
    this.svg.append("text").attr("x", this.width / 2).attr("y", 0 - this.margin.top / 2).attr("class", "title").attr("text-anchor", "middle").style("font-size", this.titleFontSize === void 0 ? `${Math.min(40, Math.min(this.width, this.height) / 5)}px` : this.titleFontSize).style("font-family", this.fontFamily).style("opacity", 0.8).text(t);
  }
  addInteraction() {
    P(this.interactionG).data(this.dataFormat === "file" ? this.data : this.data.values).append("rect").attr("x", 0).attr("y", (s, n) => this.dataFormat === "file" ? this.yScale(s[this.labels]) : this.yScale(this.data[this.labels][n])).attr("width", (s, n) => this.dataFormat === "file" ? this.xScale(+s[this.values]) : this.xScale(this.data[this.values][n])).attr("height", this.yScale.bandwidth()).attr("fill", "transparent");
    const t = x(this.el).append("div").style("opacity", 0).attr("class", "tooltip").style("position", "absolute").style("background-color", "white").style("border", "solid").style("border-width", "1px").style("border-radius", "5px").style("padding", "3px").style("font-family", this.fontFamily).style("font-size", this.tooltipFontSize).style("pointer-events", "none"), e = this;
    P(this.interactionG).on("mouseover", function() {
      t.style("opacity", 1), x(this).select("path").style("stroke", e.highlight), x(this).selectAll("path:nth-child(2)").style("stroke-width", e.strokeWidth + 1.2);
    }), P(this.interactionG).on("mouseout", function() {
      t.style("opacity", 0), x(this).select("path").style("stroke", e.color), x(this).selectAll("path:nth-child(2)").style("stroke-width", e.strokeWidth);
    }), P(this.interactionG).on("mousemove", function(s) {
      const n = x(this).attr("attrX"), a = x(this).attr("attrY"), h = bt(this);
      t.html(`<b>${n}</b>: ${a}`).style("opacity", 0.95).style("transform", `translate(${h[0] + e.margin.left}px, 
              ${h[1] - (e.height + e.margin.top + e.margin.bottom / 2)}px)`);
    });
  }
  initRoughObjects() {
    this.roughSvg = document.getElementById(this.roughId), this.rcAxis = T(this.roughSvg, { options: { strokeWidth: this.axisStrokeWidth, roughness: this.axisRoughness } }), this.rc = T(this.roughSvg, { options: { fill: this.color, stroke: this.stroke === "none" ? void 0 : this.stroke, strokeWidth: this.innerStrokeWidth, roughness: this.roughness, bowing: this.bowing, fillStyle: this.fillStyle } });
  }
  drawFromObject() {
    this.initRoughObjects(), this.addScales(), this.addAxes(), this.makeAxesRough(this.roughSvg, this.rcAxis), this.addLabels(), this.data.values.forEach((t, e) => {
      const s = this.rc.rectangle(0, this.yScale(this.data[this.labels][e]), this.xScale(t), this.yScale.bandwidth(), { simplification: this.simplification, fillWeight: this.fillWeight }), n = this.roughSvg.appendChild(s);
      n.setAttribute("class", this.graphClass), n.setAttribute("attrX", this.data[this.labels][e]), n.setAttribute("attrY", +t);
    }), P(this.interactionG).selectAll("path:nth-child(2)").style("stroke-width", this.strokeWidth), this.interactive === !0 && this.addInteraction();
  }
  drawFromFile() {
    this.initRoughObjects(), this.addScales(), this.addAxes(), this.makeAxesRough(this.roughSvg, this.rcAxis), this.addLabels(), this.data.forEach((t) => {
      const e = this.rc.rectangle(0, this.yScale(t[this.labels]), this.xScale(+t[this.values]), this.yScale.bandwidth(), { simplification: this.simplification, fillWeight: this.fillWeight }), s = this.roughSvg.appendChild(e);
      s.setAttribute("class", this.graphClass), s.setAttribute("attrX", t[this.labels]), s.setAttribute("attrY", +t[this.values]);
    }), P(this.interactionG).selectAll("path:nth-child(2)").style("stroke-width", this.strokeWidth), this.interactive === !0 && this.addInteraction();
  }
}
var Xi = Math.PI, Ki = 2 * Xi, mt = 1e-6, Sr = Ki - mt;
function Qi() {
  this._x0 = this._y0 = this._x1 = this._y1 = null, this._ = "";
}
function ne() {
  return new Qi();
}
function B(i) {
  return function() {
    return i;
  };
}
Qi.prototype = ne.prototype = { constructor: Qi, moveTo: function(i, t) {
  this._ += "M" + (this._x0 = this._x1 = +i) + "," + (this._y0 = this._y1 = +t);
}, closePath: function() {
  this._x1 !== null && (this._x1 = this._x0, this._y1 = this._y0, this._ += "Z");
}, lineTo: function(i, t) {
  this._ += "L" + (this._x1 = +i) + "," + (this._y1 = +t);
}, quadraticCurveTo: function(i, t, e, s) {
  this._ += "Q" + +i + "," + +t + "," + (this._x1 = +e) + "," + (this._y1 = +s);
}, bezierCurveTo: function(i, t, e, s, n, a) {
  this._ += "C" + +i + "," + +t + "," + +e + "," + +s + "," + (this._x1 = +n) + "," + (this._y1 = +a);
}, arcTo: function(i, t, e, s, n) {
  i = +i, t = +t, e = +e, s = +s, n = +n;
  var a = this._x1, h = this._y1, r = e - i, o = s - t, l = a - i, u = h - t, c = l * l + u * u;
  if (n < 0)
    throw new Error("negative radius: " + n);
  if (this._x1 === null)
    this._ += "M" + (this._x1 = i) + "," + (this._y1 = t);
  else if (c > mt)
    if (Math.abs(u * r - o * l) > mt && n) {
      var d = e - a, g = s - h, p = r * r + o * o, f = d * d + g * g, A = Math.sqrt(p), m = Math.sqrt(c), y = n * Math.tan((Xi - Math.acos((p + c - f) / (2 * A * m))) / 2), b = y / m, k = y / A;
      Math.abs(b - 1) > mt && (this._ += "L" + (i + b * l) + "," + (t + b * u)), this._ += "A" + n + "," + n + ",0,0," + +(u * d > l * g) + "," + (this._x1 = i + k * r) + "," + (this._y1 = t + k * o);
    } else
      this._ += "L" + (this._x1 = i) + "," + (this._y1 = t);
}, arc: function(i, t, e, s, n, a) {
  i = +i, t = +t, a = !!a;
  var h = (e = +e) * Math.cos(s), r = e * Math.sin(s), o = i + h, l = t + r, u = 1 ^ a, c = a ? s - n : n - s;
  if (e < 0)
    throw new Error("negative radius: " + e);
  this._x1 === null ? this._ += "M" + o + "," + l : (Math.abs(this._x1 - o) > mt || Math.abs(this._y1 - l) > mt) && (this._ += "L" + o + "," + l), e && (c < 0 && (c = c % Ki + Ki), c > Sr ? this._ += "A" + e + "," + e + ",0,1," + u + "," + (i - h) + "," + (t - r) + "A" + e + "," + e + ",0,1," + u + "," + (this._x1 = o) + "," + (this._y1 = l) : c > mt && (this._ += "A" + e + "," + e + ",0," + +(c >= Xi) + "," + u + "," + (this._x1 = i + e * Math.cos(n)) + "," + (this._y1 = t + e * Math.sin(n))));
}, rect: function(i, t, e, s) {
  this._ += "M" + (this._x0 = this._x1 = +i) + "," + (this._y0 = this._y1 = +t) + "h" + +e + "v" + +s + "h" + -e + "Z";
}, toString: function() {
  return this._;
} };
var Ke = Math.abs, V = Math.atan2, ft = Math.cos, Mr = Math.max, Oi = Math.min, nt = Math.sin, Mt = Math.sqrt, X = 1e-12, Lt = Math.PI, ci = Lt / 2, ii = 2 * Lt;
function Qe(i) {
  return i >= 1 ? ci : i <= -1 ? -ci : Math.asin(i);
}
function Pr(i) {
  return i.innerRadius;
}
function Rr(i) {
  return i.outerRadius;
}
function zr(i) {
  return i.startAngle;
}
function Fr(i) {
  return i.endAngle;
}
function Wr(i) {
  return i && i.padAngle;
}
function _t(i, t, e, s, n, a, h) {
  var r = i - e, o = t - s, l = (h ? a : -a) / Mt(r * r + o * o), u = l * o, c = -l * r, d = i + u, g = t + c, p = e + u, f = s + c, A = (d + p) / 2, m = (g + f) / 2, y = p - d, b = f - g, k = y * y + b * b, S = n - a, v = d * f - p * g, M = (b < 0 ? -1 : 1) * Mt(Mr(0, S * S * k - v * v)), C = (v * b - y * M) / k, j = (-v * y - b * M) / k, z = (v * b + y * M) / k, F = (-v * y + b * M) / k, w = C - A, W = j - m, E = z - A, G = F - m;
  return w * w + W * W > E * E + G * G && (C = z, j = F), { cx: C, cy: j, x01: -u, y01: -c, x11: C * (n / S - 1), y11: j * (n / S - 1) };
}
function di() {
  var i = Pr, t = Rr, e = B(0), s = null, n = zr, a = Fr, h = Wr, r = null;
  function o() {
    var l, u, c = +i.apply(this, arguments), d = +t.apply(this, arguments), g = n.apply(this, arguments) - ci, p = a.apply(this, arguments) - ci, f = Ke(p - g), A = p > g;
    if (r || (r = l = ne()), d < c && (u = d, d = c, c = u), d > X)
      if (f > ii - X)
        r.moveTo(d * ft(g), d * nt(g)), r.arc(0, 0, d, g, p, !A), c > X && (r.moveTo(c * ft(p), c * nt(p)), r.arc(0, 0, c, p, g, A));
      else {
        var m, y, b = g, k = p, S = g, v = p, M = f, C = f, j = h.apply(this, arguments) / 2, z = j > X && (s ? +s.apply(this, arguments) : Mt(c * c + d * d)), F = Oi(Ke(d - c) / 2, +e.apply(this, arguments)), w = F, W = F;
        if (z > X) {
          var E = Qe(z / c * nt(j)), G = Qe(z / d * nt(j));
          (M -= 2 * E) > X ? (S += E *= A ? 1 : -1, v -= E) : (M = 0, S = v = (g + p) / 2), (C -= 2 * G) > X ? (b += G *= A ? 1 : -1, k -= G) : (C = 0, b = k = (g + p) / 2);
        }
        var O = d * ft(b), I = d * nt(b), J = c * ft(v), _ = c * nt(v);
        if (F > X) {
          var L, $ = d * ft(k), lt = d * nt(k), zt = c * ft(S), tt = c * nt(S);
          if (f < Lt && (L = function(gt, ki, Bs, Ts, oe, le, Vs, Ys) {
            var ue = Bs - gt, ce = Ts - ki, de = Vs - oe, ge = Ys - le, Ft = ge * ue - de * ce;
            if (!(Ft * Ft < X))
              return [gt + (Ft = (de * (ki - le) - ge * (gt - oe)) / Ft) * ue, ki + Ft * ce];
          }(O, I, zt, tt, $, lt, J, _))) {
            var it = O - L[0], xi = I - L[1], vi = $ - L[0], bi = lt - L[1], ae = 1 / nt(function(gt) {
              return gt > 1 ? 0 : gt < -1 ? Lt : Math.acos(gt);
            }((it * vi + xi * bi) / (Mt(it * it + xi * xi) * Mt(vi * vi + bi * bi))) / 2), he = Mt(L[0] * L[0] + L[1] * L[1]);
            w = Oi(F, (c - he) / (ae - 1)), W = Oi(F, (d - he) / (ae + 1));
          }
        }
        C > X ? W > X ? (m = _t(zt, tt, O, I, d, W, A), y = _t($, lt, J, _, d, W, A), r.moveTo(m.cx + m.x01, m.cy + m.y01), W < F ? r.arc(m.cx, m.cy, W, V(m.y01, m.x01), V(y.y01, y.x01), !A) : (r.arc(m.cx, m.cy, W, V(m.y01, m.x01), V(m.y11, m.x11), !A), r.arc(0, 0, d, V(m.cy + m.y11, m.cx + m.x11), V(y.cy + y.y11, y.cx + y.x11), !A), r.arc(y.cx, y.cy, W, V(y.y11, y.x11), V(y.y01, y.x01), !A))) : (r.moveTo(O, I), r.arc(0, 0, d, b, k, !A)) : r.moveTo(O, I), c > X && M > X ? w > X ? (m = _t(J, _, $, lt, c, -w, A), y = _t(O, I, zt, tt, c, -w, A), r.lineTo(m.cx + m.x01, m.cy + m.y01), w < F ? r.arc(m.cx, m.cy, w, V(m.y01, m.x01), V(y.y01, y.x01), !A) : (r.arc(m.cx, m.cy, w, V(m.y01, m.x01), V(m.y11, m.x11), !A), r.arc(0, 0, c, V(m.cy + m.y11, m.cx + m.x11), V(y.cy + y.y11, y.cx + y.x11), A), r.arc(y.cx, y.cy, w, V(y.y11, y.x11), V(y.y01, y.x01), !A))) : r.arc(0, 0, c, v, S, A) : r.lineTo(J, _);
      }
    else
      r.moveTo(0, 0);
    if (r.closePath(), l)
      return r = null, l + "" || null;
  }
  return o.centroid = function() {
    var l = (+i.apply(this, arguments) + +t.apply(this, arguments)) / 2, u = (+n.apply(this, arguments) + +a.apply(this, arguments)) / 2 - Lt / 2;
    return [ft(u) * l, nt(u) * l];
  }, o.innerRadius = function(l) {
    return arguments.length ? (i = typeof l == "function" ? l : B(+l), o) : i;
  }, o.outerRadius = function(l) {
    return arguments.length ? (t = typeof l == "function" ? l : B(+l), o) : t;
  }, o.cornerRadius = function(l) {
    return arguments.length ? (e = typeof l == "function" ? l : B(+l), o) : e;
  }, o.padRadius = function(l) {
    return arguments.length ? (s = l == null ? null : typeof l == "function" ? l : B(+l), o) : s;
  }, o.startAngle = function(l) {
    return arguments.length ? (n = typeof l == "function" ? l : B(+l), o) : n;
  }, o.endAngle = function(l) {
    return arguments.length ? (a = typeof l == "function" ? l : B(+l), o) : a;
  }, o.padAngle = function(l) {
    return arguments.length ? (h = typeof l == "function" ? l : B(+l), o) : h;
  }, o.context = function(l) {
    return arguments.length ? (r = l ?? null, o) : r;
  }, o;
}
function Ws(i) {
  this._context = i;
}
function Cr(i) {
  return new Ws(i);
}
function jr(i) {
  return i[0];
}
function Er(i) {
  return i[1];
}
function Or(i, t) {
  return t < i ? -1 : t > i ? 1 : t >= i ? 0 : NaN;
}
function Gr(i) {
  return i;
}
function gi() {
  var i = Gr, t = Or, e = null, s = B(0), n = B(ii), a = B(0);
  function h(r) {
    var o, l, u, c, d, g = r.length, p = 0, f = new Array(g), A = new Array(g), m = +s.apply(this, arguments), y = Math.min(ii, Math.max(-ii, n.apply(this, arguments) - m)), b = Math.min(Math.abs(y) / g, a.apply(this, arguments)), k = b * (y < 0 ? -1 : 1);
    for (o = 0; o < g; ++o)
      (d = A[f[o] = o] = +i(r[o], o, r)) > 0 && (p += d);
    for (t != null ? f.sort(function(S, v) {
      return t(A[S], A[v]);
    }) : e != null && f.sort(function(S, v) {
      return e(r[S], r[v]);
    }), o = 0, u = p ? (y - g * k) / p : 0; o < g; ++o, m = c)
      l = f[o], c = m + ((d = A[l]) > 0 ? d * u : 0) + k, A[l] = { data: r[l], index: o, value: d, startAngle: m, endAngle: c, padAngle: b };
    return A;
  }
  return h.value = function(r) {
    return arguments.length ? (i = typeof r == "function" ? r : B(+r), h) : i;
  }, h.sortValues = function(r) {
    return arguments.length ? (t = r, e = null, h) : t;
  }, h.sort = function(r) {
    return arguments.length ? (e = r, t = null, h) : e;
  }, h.startAngle = function(r) {
    return arguments.length ? (s = typeof r == "function" ? r : B(+r), h) : s;
  }, h.endAngle = function(r) {
    return arguments.length ? (n = typeof r == "function" ? r : B(+r), h) : n;
  }, h.padAngle = function(r) {
    return arguments.length ? (a = typeof r == "function" ? r : B(+r), h) : a;
  }, h;
}
Ws.prototype = { areaStart: function() {
  this._line = 0;
}, areaEnd: function() {
  this._line = NaN;
}, lineStart: function() {
  this._point = 0;
}, lineEnd: function() {
  (this._line || this._line !== 0 && this._point === 1) && this._context.closePath(), this._line = 1 - this._line;
}, point: function(i, t) {
  switch (i = +i, t = +t, this._point) {
    case 0:
      this._point = 1, this._line ? this._context.lineTo(i, t) : this._context.moveTo(i, t);
      break;
    case 1:
      this._point = 2;
    default:
      this._context.lineTo(i, t);
  }
} };
const yt = ["coral", "skyblue", "#66c2a5", "tan", "#8da0cb", "#e78ac3", "#a6d854", "#ffd92f", "coral", "skyblue", "tan", "orange"], dt = (i, t, e, s, n) => {
  i.svg.append("svg").attr("x", i.legendPosition === "left" ? 5 : i.width - (e + 2)).attr("y", 0);
  const a = n === void 0 ? 5 - i.margin.left : n, h = i.rc.rectangle(i.legendPosition === "left" ? a : i.width + i.margin.right - 2 - e, -i.margin.top / 3, e, s, { fill: "white", fillWeight: 0.1, strokeWidth: 0.75, roughness: 2 }), r = i.roughSvg.appendChild(h), o = "rough" + i.el.substring(1, i.el.length);
  r.setAttribute("class", o), t.forEach((l, u) => {
    const c = x("." + o).append("g").attr("transform", `translate(
        ${i.legendPosition === "left" ? 5 : i.width - (e + 2)},
        0)`);
    c.append("rect").style("fill", i.colors[u]).attr("width", 20).attr("height", 8).attr("x", i.legendPosition === "left" ? a : i.margin.right + 5).attr("y", 6 + 11 * u - i.margin.top / 3), c.append("text").style("font-size", ".8rem").style("font-family", i.fontFamily).attr("x", i.legendPosition === "left" ? a + 25 : i.margin.right + 30).attr("y", 6 + 11 * u + 8 - i.margin.top / 3).text(l.text);
  });
};
class Qr extends ot {
  constructor(t) {
    super(t), this.margin = t.margin || { top: 50, right: 20, bottom: 10, left: 20 }, this.colors = t.colors || yt, this.highlight = t.highlight, this.roughness = st({ roughness: t.roughness, ceiling: 30 }), this.strokeWidth = t.strokeWidth || 0.75, this.innerStrokeWidth = t.innerStrokeWidth || 0.75, this.fillWeight = t.fillWeight || 0.85, this.labels = this.dataFormat === "object" ? "labels" : t.labels, this.values = this.dataFormat === "object" ? "values" : t.values, this.labels !== void 0 && this.values !== void 0 ? (this.legend = t.legend !== !1, this.legendPosition = t.legendPosition || "right", this.responsive = !0, this.boundRedraw = this.redraw.bind(this, t), this.initChartValues(t), this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title), window.addEventListener("resize", this.resizeHandler.bind(this))) : console.log(`Error for ${this.el}: Must include labels and values when        instantiating Donut chart. Skipping chart.`);
  }
  resizeHandler() {
    this.responsive && this.boundRedraw();
  }
  remove() {
    x(this.el).select("svg").remove();
  }
  redraw(t) {
    this.remove(), this.initChartValues(t), this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title);
  }
  initChartValues(t) {
    this.roughness = t.roughness || this.roughness, this.stroke = t.stroke || this.stroke, this.strokeWidth = t.strokeWidth || this.strokeWidth, this.axisStrokeWidth = t.axisStrokeWidth || this.axisStrokeWidth, this.axisRoughness = t.axisRoughness || this.axisRoughness, this.innerStrokeWidth = t.innerStrokeWidth || this.innerStrokeWidth, this.fillWeight = t.fillWeight || this.fillWeight, this.fillStyle = t.fillStyle || this.fillStyle;
    const e = x(this.el).node().getBoundingClientRect(), s = e.width, n = e.height;
    this.width = s - this.margin.left - this.margin.right, this.height = n - this.margin.top - this.margin.bottom, this.roughId = this.el + "_svg", this.graphClass = this.el.substring(1, this.el.length), this.interactionG = "g." + this.graphClass, this.radius = Math.min(this.width, this.height) / 2, this.setSvg();
  }
  resolveData(t) {
    return typeof t != "string" ? () => {
      this.data = t, this.drawFromObject();
    } : t.includes(".csv") ? () => {
      xt(t).then((e) => {
        this.data = e, this.drawFromFile();
      });
    } : t.includes(".tsv") ? () => {
      vt(t).then((e) => {
        this.data = e, this.drawFromFile();
      });
    } : t.includes(".json") ? () => {
      ls(t).then((e) => {
        this.data = e, this.drawFromFile();
      });
    } : void 0;
  }
  setTitle(t) {
    this.svg.append("text").attr("x", this.width / 2).attr("y", 0 - this.margin.top / 3).attr("class", "title").attr("text-anchor", "middle").style("font-size", this.titleFontSize === void 0 ? `${Math.min(40, Math.min(this.width, this.height) / 4)}px` : this.titleFontSize).style("font-family", this.fontFamily).style("opacity", 0.8).text(t);
  }
  addInteraction() {
    P(this.interactionG).append("g").attr("transform", `translate(${this.width / 2}, ${this.height / 2})`).data(this.dataFormat === "object" ? this.makePie(this.data[this.values]) : this.makePie(this.data)).append("path").attr("d", this.makeArc).attr("stroke-width", "0px").attr("fill", "transparent");
    const t = x(this.el).append("div").style("opacity", 0).attr("class", "tooltip").style("position", "absolute").style("background-color", "white").style("border", "solid").style("border-width", "1px").style("border-radius", "5px").style("padding", "3px").style("font-family", this.fontFamily).style("font-size", this.tooltipFontSize).style("pointer-events", "none"), e = this;
    let s;
    P(this.interactionG).on("mouseover", function() {
      t.style("opacity", 1), s = x(this).selectAll("path").style("stroke"), e.highlight === void 0 ? x(this).selectAll("path").style("opacity", 0.5) : x(this).selectAll("path").style("stroke", e.highlight);
    }), P(this.interactionG).on("mouseout", function() {
      t.style("opacity", 0), x(this).selectAll("path").style("stroke", s), x(this).selectAll("path").style("opacity", 1);
    }), P(this.interactionG).on("mousemove", function(n) {
      const a = x(this).attr("attrX"), h = x(this).attr("attrY"), r = bt(this);
      t.html(`<b>${a}</b>: ${h}`).style("opacity", 0.95).style("transform", `translate(${r[0] + e.margin.left}px, 
              ${r[1] - (e.height + e.margin.top + e.margin.bottom / 2)}px)`);
    });
  }
  initRoughObjects() {
    this.roughSvg = document.getElementById(this.roughId), this.rcAxis = T(this.roughSvg, { options: { strokeWidth: this.strokeWidth >= 3 ? 3 : this.strokeWidth } }), this.rc = T(this.roughSvg, { options: { fill: this.color, strokeWidth: this.innerStrokeWidth, roughness: this.roughness, bowing: this.bowing, fillStyle: this.fillStyle, fillWeight: this.fillWeight } });
  }
  drawFromObject() {
    this.initRoughObjects(), this.makePie = gi(), this.makeArc = di().innerRadius(0).outerRadius(this.radius), this.arcs = this.makePie(this.data[this.values]), this.arcs.forEach((a, h) => {
      if (a.value !== 0) {
        const r = this.rc.arc(this.width / 2, this.height / 2, 2 * this.radius, 2 * this.radius, a.startAngle - Math.PI / 2, a.endAngle - Math.PI / 2, !0, { fill: this.colors[h], stroke: this.colors[h] });
        r.setAttribute("class", this.graphClass);
        const o = this.roughSvg.appendChild(r);
        o.setAttribute("attrY", this.data[this.values][h]), o.setAttribute("attrX", this.data[this.labels][h]);
      }
    });
    const t = this.rc.circle(this.width / 2, this.height / 2, this.radius, { fill: "white", strokeWidth: 0.05, fillWeight: 10, fillStyle: "solid" });
    this.roughSvg.appendChild(t), P(this.interactionG).selectAll("path:nth-child(2)").style("stroke-width", this.strokeWidth);
    const e = this.data.labels.map((a, h) => ({ color: this.colors[h], text: a })), s = 6 * e.reduce((a, h) => a > h.text.length ? a : h.text.length, 0) + 35, n = 11 * e.length + 8;
    this.legend === !0 && dt(this, e, s, n), this.interactive === !0 && this.addInteraction();
  }
  drawFromFile() {
    this.initRoughObjects(), this.makePie = gi().value((h) => h[this.values]).sort(null);
    const t = [];
    this.makeArc = di().innerRadius(0).outerRadius(this.radius), this.arcs = this.makePie(this.data), this.arcs.forEach((h, r) => {
      if (h.value !== 0) {
        const o = this.rc.arc(this.width / 2, this.height / 2, 2 * this.radius, 2 * this.radius, h.startAngle - Math.PI / 2, h.endAngle - Math.PI / 2, !0, { fill: this.colors[r], stroke: this.colors[r] });
        o.setAttribute("class", this.graphClass);
        const l = this.roughSvg.appendChild(o);
        l.setAttribute("attrY", h.data[this.values]), l.setAttribute("attrX", h.data[this.labels]);
      }
      t.push(h.data[this.labels]);
    });
    const e = this.rc.circle(this.width / 2, this.height / 2, this.radius, { fill: "white", strokeWidth: 0.05, fillWeight: 10, fillStyle: "solid" });
    this.roughSvg.appendChild(e), P(this.interactionG).selectAll("path:nth-child(2)").style("stroke-width", this.strokeWidth);
    const s = t.map((h, r) => ({ color: this.colors[r], text: h })), n = 6 * s.reduce((h, r) => h > r.text.length ? h : r.text.length, 0) + 35, a = 11 * s.length + 8;
    this.legend === !0 && dt(this, s, n, a), this.interactive === !0 && this.addInteraction();
  }
}
class Ur extends ot {
  constructor(t) {
    super(t), this.margin = t.margin || { top: 50, right: 20, bottom: 50, left: 100 }, this.roughness = st({ roughness: t.roughness, defaultValue: 2.2 }), this.axisStrokeWidth = t.axisStrokeWidth || 0.5, this.axisRoughness = t.axisRoughness || 0.5, this.stroke = t.stroke || "black", this.fillWeight = t.fillWeight || 0.5, this.colors = t.colors, this.strokeWidth = t.strokeWidth || 1, this.axisFontSize = t.axisFontSize, this.x = t.x, this.y = this.dataFormat === "object" ? "y" : t.y, this.xValueFormat = t.xValueFormat, this.yValueFormat = t.yValueFormat, this.legend = t.legend !== !1, this.legendPosition = t.legendPosition || "right", this.circle = t.circle !== !1, this.circleRadius = t.circleRadius || 10, this.circleRoughness = st({ roughness: t.circleRoughness, defaultValue: 2 }), this.xLabel = t.xLabel || "", this.yLabel = t.yLabel || "", this.labelFontSize = t.labelFontSize || "1rem", this.dataFormat === "file" && (this.dataSources = [], this.yKeys = Object.keys(t).filter((e) => /y/.test(e)), this.yKeys.map((e, s) => {
      e !== "yLabel" && this.dataSources.push(t[e]);
    })), this.responsive = !0, this.boundRedraw = this.redraw.bind(this, t), this.initChartValues(t), this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title), window.addEventListener("resize", this.resizeHandler.bind(this));
  }
  resizeHandler() {
    this.responsive && this.boundRedraw();
  }
  remove() {
    x(this.el).select("svg").remove();
  }
  redraw(t) {
    this.remove(), this.initChartValues(t), this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title);
  }
  initChartValues(t) {
    this.roughness = t.roughness || this.roughness, this.stroke = t.stroke || this.stroke, this.strokeWidth = t.strokeWidth || this.strokeWidth, this.axisStrokeWidth = t.axisStrokeWidth || this.axisStrokeWidth, this.axisRoughness = t.axisRoughness || this.axisRoughness, this.innerStrokeWidth = t.innerStrokeWidth || this.innerStrokeWidth, this.fillWeight = t.fillWeight || this.fillWeight, this.fillStyle = t.fillStyle || this.fillStyle;
    const e = x(this.el).node().getBoundingClientRect(), s = e.width, n = e.height;
    this.width = s - this.margin.left - this.margin.right, this.height = n - this.margin.top - this.margin.bottom, this.roughId = this.el + "_svg", this.graphClass = this.el.substring(1, this.el.length), this.interactionG = "g." + this.graphClass, this.setSvg();
  }
  resolveData(t) {
    return typeof t != "string" ? () => {
      this.data = t, this.drawFromObject();
    } : t.includes(".csv") ? () => {
      xt(t).then((e) => {
        this.data = e, this.drawFromFile();
      });
    } : t.includes(".tsv") ? () => {
      vt(t).then((e) => {
        this.data = e, this.drawFromFile();
      });
    } : void 0;
  }
  addScales() {
    let t, e;
    if (this.dataFormat !== "file")
      t = ((a) => {
        const h = Object.keys(a).map((l) => et(a[l])), r = ut(h, (l) => l[0]), o = q(h, (l) => l[1]);
        return [r, o];
      })(this.data);
    else {
      const a = this.dataSources.map((o) => et(this.data, (l) => +l[o])), h = ut(a, (o) => o[0]), r = q(a, (o) => o[1]);
      t = [h, r];
    }
    if (this.x === void 0) {
      const a = q(Object.keys(this.data).map((h) => this.data[h].length));
      e = this.dataFormat === "file" ? [0, this.data.length] : [0, a];
    } else
      e = et(this.x);
    const s = t, n = s[1] - s[0];
    this.xScale = this.x === void 0 ? Me().range([0, this.width]).domain([...Array(e[1]).keys()]) : Me().range([0, this.width]).domain(this.x), this.yScale = Z().range([this.height, 0]).domain([0, s[1] + 0.05 * n]);
  }
  addLabels() {
    this.xLabel !== "" && this.svg.append("text").attr("x", this.width / 2).attr("y", this.height + this.margin.bottom / 1.3).attr("dx", "1em").attr("class", "labelText").style("text-anchor", "middle").style("font-family", this.fontFamily).style("font-size", this.labelFontSize).text(this.xLabel), this.yLabel !== "" && this.svg.append("text").attr("transform", "rotate(-90)").attr("y", 0 - this.margin.left / 2).attr("x", 0 - this.height / 2).attr("dy", "1em").attr("class", "labelText").style("text-anchor", "middle").style("font-family", this.fontFamily).style("font-size", this.labelFontSize).text(this.yLabel);
  }
  addAxes() {
    const t = Yt(this.xScale).tickSize(0).tickFormat((s) => this.xValueFormat ? ht(this.xValueFormat)(s) : s), e = Dt(this.yScale).tickSize(0).tickFormat((s) => this.yValueFormat ? ht(this.yValueFormat)(s) : s);
    this.svg.append("g").attr("transform", "translate(0," + this.height + ")").call(t).attr("class", `xAxis${this.graphClass}`).selectAll("text").attr("transform", "translate(-10, 0)rotate(-45)").style("text-anchor", "end").style("font-family", this.fontFamily).style("font-size", this.axisFontSize === void 0 ? `${Math.min(0.95, Math.min(this.width, this.height) / 140)}rem` : this.axisFontSize), this.svg.append("g").call(e).attr("class", `yAxis${this.graphClass}`).selectAll("text").style("font-family", this.fontFamily).style("font-size", this.axisFontSize === void 0 ? `${Math.min(0.95, Math.min(this.width, this.height) / 140)}rem` : this.axisFontSize), P("path.domain").attr("stroke", "transparent"), P("g.tick").style("opacity", 1);
  }
  makeAxesRough(t, e) {
    const s = `xAxis${this.graphClass}`, n = `yAxis${this.graphClass}`, a = `rough-${s}`, h = `rough-${n}`;
    x(`.${s}`).selectAll("path.domain").each(function(r, o) {
      const l = x(this).node().getAttribute("d"), u = e.path(l, { stroke: "black", fillStyle: "hachure" });
      u.setAttribute("class", a), t.appendChild(u);
    }), P(`.${a}`).attr("transform", `translate(0, ${this.height})`), x(`.${n}`).selectAll("path.domain").each(function(r, o) {
      const l = x(this).node().getAttribute("d"), u = e.path(l, { stroke: "black", fillStyle: "hachure" });
      u.setAttribute("class", h), t.appendChild(u);
    });
  }
  setTitle(t) {
    this.svg.append("text").attr("x", this.width / 2).attr("y", 0 - this.margin.top / 2).attr("text-anchor", "middle").style("font-size", this.titleFontSize === void 0 ? `${Math.min(20, Math.min(this.width, this.height) / 4)}px` : this.titleFontSize).style("font-family", this.fontFamily).style("opacity", 0.8).text(t);
  }
  addInteraction() {
    const t = this;
    this.chartScreen = this.svg.append("g").attr("pointer-events", "all"), this.dataSources.map((e, s) => {
      const n = (this.dataFormat === "file" ? this.data : this.data[e]).map((o, l) => this.x === void 0 ? [this.xScale(l), this.yScale(o[e])] : [this.xScale(this.x[l]), this.yScale(+o[e])]), a = n.filter((o) => o[0] !== void 0), h = function() {
        var o = jr, l = Er, u = B(!0), c = null, d = Cr, g = null;
        function p(f) {
          var A, m, y, b = f.length, k = !1;
          for (c == null && (g = d(y = ne())), A = 0; A <= b; ++A)
            !(A < b && u(m = f[A], A, f)) === k && ((k = !k) ? g.lineStart() : g.lineEnd()), k && g.point(+o(m, A, f), +l(m, A, f));
          if (y)
            return g = null, y + "" || null;
        }
        return p.x = function(f) {
          return arguments.length ? (o = typeof f == "function" ? f : B(+f), p) : o;
        }, p.y = function(f) {
          return arguments.length ? (l = typeof f == "function" ? f : B(+f), p) : l;
        }, p.defined = function(f) {
          return arguments.length ? (u = typeof f == "function" ? f : B(!!f), p) : u;
        }, p.curve = function(f) {
          return arguments.length ? (d = f, c != null && (g = d(c)), p) : d;
        }, p.context = function(f) {
          return arguments.length ? (f == null ? c = g = null : g = d(c = f), p) : c;
        }, p;
      }().x((o) => o[0]).y((o) => o[1]);
      this.svg.append("path").datum(a).attr("fill", "none").attr("stroke", "blue").attr("stroke-width", 1.5).attr("d", h).attr("visibility", "hidden");
      const r = e + "class";
      this.svg.append("g").attr("class", r + "text").append("text").style("font-size", this.tooltipFontSize).style("opacity", 0).style("font-family", this.fontFamily).attr("text-anchor", "middle").attr("alignment-baseline", "middle");
    }), this.chartScreen.append("rect").attr("width", this.width).attr("height", this.height).attr("fill", "none").on("mousemove", function(e) {
      const s = bt(this)[0], n = t.xScale.domain(), a = t.xScale.range(), h = rs(a[0], a[1] + 1, t.xScale.step()), r = ns(h, s), o = n[r];
      t.dataSources.map((l, u) => {
        const c = t.dataFormat === "file" ? t.x === void 0 ? t.data[o] : t.data[r] : t.data[l][r], d = "." + l + "classtext";
        t.dataFormat === "file" ? x(d).selectAll("text").style("opacity", 1).html(t.x === void 0 ? `(${r},${c[l]})` : `(${t.x[r]}, ${c[l]})`).attr("x", t.x === void 0 ? t.xScale(r) : t.xScale(t.x[r])).attr("y", t.yScale(c[l]) - 6) : x(d).selectAll("text").style("opacity", 1).html(t.x === void 0 ? `(${r}, ${c})` : `(${t.x[r]}, ${c})`).attr("x", t.x === void 0 ? t.xScale(r) : t.xScale(t.x[r])).attr("y", t.yScale(c));
      });
    }).on("mouseout", () => {
      t.dataSources.map((e) => {
        x("." + e + "classtext").selectAll("text").style("opacity", 0);
      });
    });
  }
  initRoughObjects() {
    this.roughSvg = document.getElementById(this.roughId), this.rcAxis = T(this.roughSvg, { options: { strokeWidth: this.axisStrokeWidth, roughness: this.axisRoughness } }), this.rc = T(this.roughSvg, { options: { stroke: this.stroke === "none" ? void 0 : this.stroke, strokeWidth: this.strokeWidth, roughness: this.roughness, bowing: this.bowing, fillStyle: this.fillStyle } });
  }
  drawFromObject() {
    const t = this;
    this.colors === void 0 && (this.colors = yt), this.dataSources = Object.keys(this.data), this.initRoughObjects(), this.addScales(), this.dataSources.map((a, h) => {
      const r = this.data[a].map((u, c) => this.x === void 0 ? [this.xScale(c), this.yScale(+u)] : [this.xScale(this.x[c]), this.yScale(u)]), o = r.filter((u) => u[0] !== void 0), l = this.rc.curve(o, { stroke: t.colors.length === 1 ? t.colors[0] : t.colors[h], roughness: t.roughness, bowing: t.bowing });
      this.roughSvg.appendChild(l).setAttribute("class", this.graphClass), this.circle === !0 && r.forEach((u, c) => {
        const d = this.rc.circle(u[0], u[1], this.circleRadius, { stroke: this.colors[h], fill: this.colors[h], fillStyle: "solid", strokeWidth: 1, roughness: this.circleRoughness });
        this.roughSvg.appendChild(d);
      });
    });
    const e = this.dataSources.map((a, h) => ({ color: this.colors[h], text: a })), s = 6 * e.reduce((a, h) => a > h.text.length ? a : h.text.length, 0) + 35, n = 11 * e.length + 8;
    this.legend === !0 && dt(this, e, s, n, 2), this.addAxes(), this.addLabels(), this.makeAxesRough(this.roughSvg, this.rcAxis), this.interactive === !0 && this.addInteraction();
  }
  drawFromFile() {
    this.colors === void 0 && (this.colors = yt), this.initRoughObjects(), this.addScales(), this.dataSources.map((n, a) => {
      const h = this.data.map((l, u) => this.x === void 0 ? [this.xScale(u), this.yScale(l[n])] : [this.xScale(this.x[u]), this.yScale(+l[n])]), r = h.filter((l) => l[0] !== void 0), o = this.rc.curve(r, { stroke: this.colors[a], strokeWidth: this.strokeWidth, roughness: 1, bowing: 10 });
      this.roughSvg.appendChild(o), this.circle === !0 && r.forEach((l, u) => {
        const c = this.rc.circle(l[0], l[1], this.circleRadius, { stroke: this.colors[a], fill: this.colors[a], fillStyle: "solid", strokeWidth: 1, roughness: this.circleRoughness });
        this.roughSvg.appendChild(c);
      });
    });
    const t = this.dataSources.map((n, a) => ({ color: this.colors[a], text: n })), e = 6 * t.reduce((n, a) => n > a.text.length ? n : a.text.length, 0) + 35, s = 11 * t.length + 8;
    this.legend === !0 && dt(this, t, e, s, 2), this.addAxes(), this.addLabels(), this.makeAxesRough(this.roughSvg, this.rcAxis), this.interactive === !0 && this.addInteraction();
  }
}
function Cs(i, t) {
  var e, s = 1;
  function n() {
    var a, h, r = e.length, o = 0, l = 0;
    for (a = 0; a < r; ++a)
      o += (h = e[a]).x, l += h.y;
    for (o = (o / r - i) * s, l = (l / r - t) * s, a = 0; a < r; ++a)
      (h = e[a]).x -= o, h.y -= l;
  }
  return i == null && (i = 0), t == null && (t = 0), n.initialize = function(a) {
    e = a;
  }, n.x = function(a) {
    return arguments.length ? (i = +a, n) : i;
  }, n.y = function(a) {
    return arguments.length ? (t = +a, n) : t;
  }, n.strength = function(a) {
    return arguments.length ? (s = +a, n) : s;
  }, n;
}
function Ue(i, t, e, s) {
  if (isNaN(t) || isNaN(e))
    return i;
  var n, a, h, r, o, l, u, c, d, g = i._root, p = { data: s }, f = i._x0, A = i._y0, m = i._x1, y = i._y1;
  if (!g)
    return i._root = p, i;
  for (; g.length; )
    if ((l = t >= (a = (f + m) / 2)) ? f = a : m = a, (u = e >= (h = (A + y) / 2)) ? A = h : y = h, n = g, !(g = g[c = u << 1 | l]))
      return n[c] = p, i;
  if (r = +i._x.call(null, g.data), o = +i._y.call(null, g.data), t === r && e === o)
    return p.next = g, n ? n[c] = p : i._root = p, i;
  do
    n = n ? n[c] = new Array(4) : i._root = new Array(4), (l = t >= (a = (f + m) / 2)) ? f = a : m = a, (u = e >= (h = (A + y) / 2)) ? A = h : y = h;
  while ((c = u << 1 | l) == (d = (o >= h) << 1 | r >= a));
  return n[d] = g, n[c] = p, i;
}
function Y(i, t, e, s, n) {
  this.node = i, this.x0 = t, this.y0 = e, this.x1 = s, this.y1 = n;
}
function Ir(i) {
  return i[0];
}
function Lr(i) {
  return i[1];
}
function js(i, t, e) {
  var s = new re(t ?? Ir, e ?? Lr, NaN, NaN, NaN, NaN);
  return i == null ? s : s.addAll(i);
}
function re(i, t, e, s, n, a) {
  this._x = i, this._y = t, this._x0 = e, this._y0 = s, this._x1 = n, this._y1 = a, this._root = void 0;
}
function Je(i) {
  for (var t = { data: i.data }, e = t; i = i.next; )
    e = e.next = { data: i.data };
  return t;
}
var D = js.prototype = re.prototype;
function Zt(i) {
  return function() {
    return i;
  };
}
function fi(i) {
  return 1e-6 * (i() - 0.5);
}
function Zr(i) {
  return i.x + i.vx;
}
function Br(i) {
  return i.y + i.vy;
}
function Es(i) {
  var t, e, s, n = 1, a = 1;
  function h() {
    for (var l, u, c, d, g, p, f, A = t.length, m = 0; m < a; ++m)
      for (u = js(t, Zr, Br).visitAfter(r), l = 0; l < A; ++l)
        c = t[l], p = e[c.index], f = p * p, d = c.x + c.vx, g = c.y + c.vy, u.visit(y);
    function y(b, k, S, v, M) {
      var C = b.data, j = b.r, z = p + j;
      if (!C)
        return k > d + z || v < d - z || S > g + z || M < g - z;
      if (C.index > c.index) {
        var F = d - C.x - C.vx, w = g - C.y - C.vy, W = F * F + w * w;
        W < z * z && (F === 0 && (W += (F = fi(s)) * F), w === 0 && (W += (w = fi(s)) * w), W = (z - (W = Math.sqrt(W))) / W * n, c.vx += (F *= W) * (z = (j *= j) / (f + j)), c.vy += (w *= W) * z, C.vx -= F * (z = 1 - z), C.vy -= w * z);
      }
    }
  }
  function r(l) {
    if (l.data)
      return l.r = e[l.data.index];
    for (var u = l.r = 0; u < 4; ++u)
      l[u] && l[u].r > l.r && (l.r = l[u].r);
  }
  function o() {
    if (t) {
      var l, u, c = t.length;
      for (e = new Array(c), l = 0; l < c; ++l)
        u = t[l], e[u.index] = +i(u, l, t);
    }
  }
  return typeof i != "function" && (i = Zt(i == null ? 1 : +i)), h.initialize = function(l, u) {
    t = l, s = u, o();
  }, h.iterations = function(l) {
    return arguments.length ? (a = +l, h) : a;
  }, h.strength = function(l) {
    return arguments.length ? (n = +l, h) : n;
  }, h.radius = function(l) {
    return arguments.length ? (i = typeof l == "function" ? l : Zt(+l), o(), h) : i;
  }, h;
}
function Tr(i) {
  return i.index;
}
function _e(i, t) {
  var e = i.get(t);
  if (!e)
    throw new Error("node not found: " + t);
  return e;
}
D.copy = function() {
  var i, t, e = new re(this._x, this._y, this._x0, this._y0, this._x1, this._y1), s = this._root;
  if (!s)
    return e;
  if (!s.length)
    return e._root = Je(s), e;
  for (i = [{ source: s, target: e._root = new Array(4) }]; s = i.pop(); )
    for (var n = 0; n < 4; ++n)
      (t = s.source[n]) && (t.length ? i.push({ source: t, target: s.target[n] = new Array(4) }) : s.target[n] = Je(t));
  return e;
}, D.add = function(i) {
  const t = +this._x.call(null, i), e = +this._y.call(null, i);
  return Ue(this.cover(t, e), t, e, i);
}, D.addAll = function(i) {
  var t, e, s, n, a = i.length, h = new Array(a), r = new Array(a), o = 1 / 0, l = 1 / 0, u = -1 / 0, c = -1 / 0;
  for (e = 0; e < a; ++e)
    isNaN(s = +this._x.call(null, t = i[e])) || isNaN(n = +this._y.call(null, t)) || (h[e] = s, r[e] = n, s < o && (o = s), s > u && (u = s), n < l && (l = n), n > c && (c = n));
  if (o > u || l > c)
    return this;
  for (this.cover(o, l).cover(u, c), e = 0; e < a; ++e)
    Ue(this, h[e], r[e], i[e]);
  return this;
}, D.cover = function(i, t) {
  if (isNaN(i = +i) || isNaN(t = +t))
    return this;
  var e = this._x0, s = this._y0, n = this._x1, a = this._y1;
  if (isNaN(e))
    n = (e = Math.floor(i)) + 1, a = (s = Math.floor(t)) + 1;
  else {
    for (var h, r, o = n - e || 1, l = this._root; e > i || i >= n || s > t || t >= a; )
      switch (r = (t < s) << 1 | i < e, (h = new Array(4))[r] = l, l = h, o *= 2, r) {
        case 0:
          n = e + o, a = s + o;
          break;
        case 1:
          e = n - o, a = s + o;
          break;
        case 2:
          n = e + o, s = a - o;
          break;
        case 3:
          e = n - o, s = a - o;
      }
    this._root && this._root.length && (this._root = l);
  }
  return this._x0 = e, this._y0 = s, this._x1 = n, this._y1 = a, this;
}, D.data = function() {
  var i = [];
  return this.visit(function(t) {
    if (!t.length)
      do
        i.push(t.data);
      while (t = t.next);
  }), i;
}, D.extent = function(i) {
  return arguments.length ? this.cover(+i[0][0], +i[0][1]).cover(+i[1][0], +i[1][1]) : isNaN(this._x0) ? void 0 : [[this._x0, this._y0], [this._x1, this._y1]];
}, D.find = function(i, t, e) {
  var s, n, a, h, r, o, l, u = this._x0, c = this._y0, d = this._x1, g = this._y1, p = [], f = this._root;
  for (f && p.push(new Y(f, u, c, d, g)), e == null ? e = 1 / 0 : (u = i - e, c = t - e, d = i + e, g = t + e, e *= e); o = p.pop(); )
    if (!(!(f = o.node) || (n = o.x0) > d || (a = o.y0) > g || (h = o.x1) < u || (r = o.y1) < c))
      if (f.length) {
        var A = (n + h) / 2, m = (a + r) / 2;
        p.push(new Y(f[3], A, m, h, r), new Y(f[2], n, m, A, r), new Y(f[1], A, a, h, m), new Y(f[0], n, a, A, m)), (l = (t >= m) << 1 | i >= A) && (o = p[p.length - 1], p[p.length - 1] = p[p.length - 1 - l], p[p.length - 1 - l] = o);
      } else {
        var y = i - +this._x.call(null, f.data), b = t - +this._y.call(null, f.data), k = y * y + b * b;
        if (k < e) {
          var S = Math.sqrt(e = k);
          u = i - S, c = t - S, d = i + S, g = t + S, s = f.data;
        }
      }
  return s;
}, D.remove = function(i) {
  if (isNaN(a = +this._x.call(null, i)) || isNaN(h = +this._y.call(null, i)))
    return this;
  var t, e, s, n, a, h, r, o, l, u, c, d, g = this._root, p = this._x0, f = this._y0, A = this._x1, m = this._y1;
  if (!g)
    return this;
  if (g.length)
    for (; ; ) {
      if ((l = a >= (r = (p + A) / 2)) ? p = r : A = r, (u = h >= (o = (f + m) / 2)) ? f = o : m = o, t = g, !(g = g[c = u << 1 | l]))
        return this;
      if (!g.length)
        break;
      (t[c + 1 & 3] || t[c + 2 & 3] || t[c + 3 & 3]) && (e = t, d = c);
    }
  for (; g.data !== i; )
    if (s = g, !(g = g.next))
      return this;
  return (n = g.next) && delete g.next, s ? (n ? s.next = n : delete s.next, this) : t ? (n ? t[c] = n : delete t[c], (g = t[0] || t[1] || t[2] || t[3]) && g === (t[3] || t[2] || t[1] || t[0]) && !g.length && (e ? e[d] = g : this._root = g), this) : (this._root = n, this);
}, D.removeAll = function(i) {
  for (var t = 0, e = i.length; t < e; ++t)
    this.remove(i[t]);
  return this;
}, D.root = function() {
  return this._root;
}, D.size = function() {
  var i = 0;
  return this.visit(function(t) {
    if (!t.length)
      do
        ++i;
      while (t = t.next);
  }), i;
}, D.visit = function(i) {
  var t, e, s, n, a, h, r = [], o = this._root;
  for (o && r.push(new Y(o, this._x0, this._y0, this._x1, this._y1)); t = r.pop(); )
    if (!i(o = t.node, s = t.x0, n = t.y0, a = t.x1, h = t.y1) && o.length) {
      var l = (s + a) / 2, u = (n + h) / 2;
      (e = o[3]) && r.push(new Y(e, l, u, a, h)), (e = o[2]) && r.push(new Y(e, s, u, l, h)), (e = o[1]) && r.push(new Y(e, l, n, a, u)), (e = o[0]) && r.push(new Y(e, s, n, l, u));
    }
  return this;
}, D.visitAfter = function(i) {
  var t, e = [], s = [];
  for (this._root && e.push(new Y(this._root, this._x0, this._y0, this._x1, this._y1)); t = e.pop(); ) {
    var n = t.node;
    if (n.length) {
      var a, h = t.x0, r = t.y0, o = t.x1, l = t.y1, u = (h + o) / 2, c = (r + l) / 2;
      (a = n[0]) && e.push(new Y(a, h, r, u, c)), (a = n[1]) && e.push(new Y(a, u, r, o, c)), (a = n[2]) && e.push(new Y(a, h, c, u, l)), (a = n[3]) && e.push(new Y(a, u, c, o, l));
    }
    s.push(t);
  }
  for (; t = s.pop(); )
    i(t.node, t.x0, t.y0, t.x1, t.y1);
  return this;
}, D.x = function(i) {
  return arguments.length ? (this._x = i, this) : this._x;
}, D.y = function(i) {
  return arguments.length ? (this._y = i, this) : this._y;
};
var Vr = { value: () => {
} };
function Os() {
  for (var i, t = 0, e = arguments.length, s = {}; t < e; ++t) {
    if (!(i = arguments[t] + "") || i in s || /[\s.]/.test(i))
      throw new Error("illegal type: " + i);
    s[i] = [];
  }
  return new ei(s);
}
function ei(i) {
  this._ = i;
}
function Yr(i, t) {
  for (var e, s = 0, n = i.length; s < n; ++s)
    if ((e = i[s]).name === t)
      return e.value;
}
function $e(i, t, e) {
  for (var s = 0, n = i.length; s < n; ++s)
    if (i[s].name === t) {
      i[s] = Vr, i = i.slice(0, s).concat(i.slice(s + 1));
      break;
    }
  return e != null && i.push({ name: t, value: e }), i;
}
ei.prototype = Os.prototype = { constructor: ei, on: function(i, t) {
  var e, s, n = this._, a = (s = n, (i + "").trim().split(/^|\s+/).map(function(o) {
    var l = "", u = o.indexOf(".");
    if (u >= 0 && (l = o.slice(u + 1), o = o.slice(0, u)), o && !s.hasOwnProperty(o))
      throw new Error("unknown type: " + o);
    return { type: o, name: l };
  })), h = -1, r = a.length;
  if (!(arguments.length < 2)) {
    if (t != null && typeof t != "function")
      throw new Error("invalid callback: " + t);
    for (; ++h < r; )
      if (e = (i = a[h]).type)
        n[e] = $e(n[e], i.name, t);
      else if (t == null)
        for (e in n)
          n[e] = $e(n[e], i.name, null);
    return this;
  }
  for (; ++h < r; )
    if ((e = (i = a[h]).type) && (e = Yr(n[e], i.name)))
      return e;
}, copy: function() {
  var i = {}, t = this._;
  for (var e in t)
    i[e] = t[e].slice();
  return new ei(i);
}, call: function(i, t) {
  if ((e = arguments.length - 2) > 0)
    for (var e, s, n = new Array(e), a = 0; a < e; ++a)
      n[a] = arguments[a + 2];
  if (!this._.hasOwnProperty(i))
    throw new Error("unknown type: " + i);
  for (a = 0, e = (s = this._[i]).length; a < e; ++a)
    s[a].value.apply(t, n);
}, apply: function(i, t, e) {
  if (!this._.hasOwnProperty(i))
    throw new Error("unknown type: " + i);
  for (var s = this._[i], n = 0, a = s.length; n < a; ++n)
    s[n].value.apply(t, e);
} };
var si, Ot, Pt = 0, Gt = 0, jt = 0, pi = 0, At = 0, yi = 0, Vt = typeof performance == "object" && performance.now ? performance : Date, Gs = typeof window == "object" && window.requestAnimationFrame ? window.requestAnimationFrame.bind(window) : function(i) {
  setTimeout(i, 17);
};
function Is() {
  return At || (Gs(Dr), At = Vt.now() + yi);
}
function Dr() {
  At = 0;
}
function Ui() {
  this._call = this._time = this._next = null;
}
function Ls(i, t, e) {
  var s = new Ui();
  return s.restart(i, t, e), s;
}
function ts() {
  At = (pi = Vt.now()) + yi, Pt = Gt = 0;
  try {
    (function() {
      Is(), ++Pt;
      for (var i, t = si; t; )
        (i = At - t._time) >= 0 && t._call.call(void 0, i), t = t._next;
      --Pt;
    })();
  } finally {
    Pt = 0, function() {
      for (var i, t, e = si, s = 1 / 0; e; )
        e._call ? (s > e._time && (s = e._time), i = e, e = e._next) : (t = e._next, e._next = null, e = i ? i._next = t : si = t);
      Ot = i, Ji(s);
    }(), At = 0;
  }
}
function qr() {
  var i = Vt.now(), t = i - pi;
  t > 1e3 && (yi -= t, pi = i);
}
function Ji(i) {
  Pt || (Gt && (Gt = clearTimeout(Gt)), i - At > 24 ? (i < 1 / 0 && (Gt = setTimeout(ts, i - Vt.now() - yi)), jt && (jt = clearInterval(jt))) : (jt || (pi = Vt.now(), jt = setInterval(qr, 1e3)), Pt = 1, Gs(ts)));
}
Ui.prototype = Ls.prototype = { constructor: Ui, restart: function(i, t, e) {
  if (typeof i != "function")
    throw new TypeError("callback is not a function");
  e = (e == null ? Is() : +e) + (t == null ? 0 : +t), this._next || Ot === this || (Ot ? Ot._next = this : si = this, Ot = this), this._call = i, this._time = e, Ji();
}, stop: function() {
  this._call && (this._call = null, this._time = 1 / 0, Ji());
} };
const is = 4294967296;
var Hr = Math.PI * (3 - Math.sqrt(5));
function Zs(i) {
  var t, e = 1, s = 1e-3, n = 1 - Math.pow(s, 1 / 300), a = 0, h = 0.6, r = /* @__PURE__ */ new Map(), o = Ls(c), l = Os("tick", "end"), u = function() {
    let f = 1;
    return () => (f = (1664525 * f + 1013904223) % is) / is;
  }();
  function c() {
    d(), l.call("tick", t), e < s && (o.stop(), l.call("end", t));
  }
  function d(f) {
    var A, m, y = i.length;
    f === void 0 && (f = 1);
    for (var b = 0; b < f; ++b)
      for (e += (a - e) * n, r.forEach(function(k) {
        k(e);
      }), A = 0; A < y; ++A)
        (m = i[A]).fx == null ? m.x += m.vx *= h : (m.x = m.fx, m.vx = 0), m.fy == null ? m.y += m.vy *= h : (m.y = m.fy, m.vy = 0);
    return t;
  }
  function g() {
    for (var f, A = 0, m = i.length; A < m; ++A) {
      if ((f = i[A]).index = A, f.fx != null && (f.x = f.fx), f.fy != null && (f.y = f.fy), isNaN(f.x) || isNaN(f.y)) {
        var y = 10 * Math.sqrt(0.5 + A), b = A * Hr;
        f.x = y * Math.cos(b), f.y = y * Math.sin(b);
      }
      (isNaN(f.vx) || isNaN(f.vy)) && (f.vx = f.vy = 0);
    }
  }
  function p(f) {
    return f.initialize && f.initialize(i, u), f;
  }
  return i == null && (i = []), g(), t = { tick: d, restart: function() {
    return o.restart(c), t;
  }, stop: function() {
    return o.stop(), t;
  }, nodes: function(f) {
    return arguments.length ? (i = f, g(), r.forEach(p), t) : i;
  }, alpha: function(f) {
    return arguments.length ? (e = +f, t) : e;
  }, alphaMin: function(f) {
    return arguments.length ? (s = +f, t) : s;
  }, alphaDecay: function(f) {
    return arguments.length ? (n = +f, t) : +n;
  }, alphaTarget: function(f) {
    return arguments.length ? (a = +f, t) : a;
  }, velocityDecay: function(f) {
    return arguments.length ? (h = 1 - f, t) : 1 - h;
  }, randomSource: function(f) {
    return arguments.length ? (u = f, r.forEach(p), t) : u;
  }, force: function(f, A) {
    return arguments.length > 1 ? (A == null ? r.delete(f) : r.set(f, p(A)), t) : r.get(f);
  }, find: function(f, A, m) {
    var y, b, k, S, v, M = 0, C = i.length;
    for (m == null ? m = 1 / 0 : m *= m, M = 0; M < C; ++M)
      (k = (y = f - (S = i[M]).x) * y + (b = A - S.y) * b) < m && (v = S, m = k);
    return v;
  }, on: function(f, A) {
    return arguments.length > 1 ? (l.on(f, A), t) : l.on(f);
  } };
}
class Jr extends ot {
  constructor(t) {
    super(t), this.data = t.data, this.links = t.links, this.margin = t.margin || { top: 50, right: 20, bottom: 10, left: 20 }, this.colors = t.colors || yt, this.highlight = t.highlight, this.roughness = st({ roughness: t.roughness, ceiling: 30, defaultValue: 0 }), this.strokeWidth = t.strokeWidth || 0.75, this.innerStrokeWidth = t.innerStrokeWidth || 0.75, this.fillWeight = t.fillWeight || 0.85, this.color = t.color || "skyblue", this.collision = t.collision || 1.4, this.radiusExtent = t.radiusExtent || [5, 20], this.radius = t.radius || "radius", this.textCallback = t.textCallback || ((e) => ""), this.colorCallback = t.colorCallback || ((e) => this.color), this.roughnessExtent = t.roughnessExtent || [0, 10], this.responsive = !0, this.boundRedraw = this.redraw.bind(this, t), this.legend = t.legend || !1, this.legendPosition = t.legendPosition || "right", this.initChartValues(t), this.resolveFont(), this.drawChart = this.resolveData(t.data, t.links), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title);
  }
  resizeHandler() {
    this.responsive && this.boundRedraw();
  }
  remove() {
    x(this.el).select("svg").remove();
  }
  redraw(t) {
    this.remove(), this.initChartValues(t), this.resolveFont(), this.drawChart = this.resolveData(t.data, t.links), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title);
  }
  initChartValues(t) {
    this.roughness = t.roughness || this.roughness, this.collision = t.collision || this.collision, this.color = t.color || this.color, this.stroke = t.stroke || this.stroke, this.strokeWidth = t.strokeWidth || this.strokeWidth, this.axisStrokeWidth = t.axisStrokeWidth || this.axisStrokeWidth, this.axisRoughness = t.axisRoughness || this.axisRoughness, this.innerStrokeWidth = t.innerStrokeWidth || this.innerStrokeWidth, this.fillWeight = t.fillWeight || this.fillWeight, this.fillStyle = t.fillStyle || this.fillStyle, this.title = t.title || this.title, this.textCallback = t.textCallback || ((a) => "");
    const e = x(this.el).node().getBoundingClientRect(), s = e.width, n = e.height;
    this.width = s - this.margin.left - this.margin.right, this.height = n - this.margin.top - this.margin.bottom, this.roughId = this.el + "_svg", this.graphClass = this.el.substring(1, this.el.length), this.interactionG = "g." + this.graphClass, this.setSvg();
  }
  resolveData(t, e) {
    return () => {
      this.data = t, this.links = e, this.drawFromObject();
    };
  }
  setTitle(t) {
    this.svg.append("text").attr("x", this.width / 2).attr("y", 0 - this.margin.top / 3).attr("class", "title").attr("text-anchor", "middle").style("font-size", this.titleFontSize === void 0 ? `${Math.min(40, Math.min(this.width, this.height) / 4)}px` : this.titleFontSize).style("font-family", this.fontFamily).style("opacity", 0.8).text(t);
  }
  addInteraction() {
    const t = this;
    let e;
    P(".nodeGroup").on("mouseover", function(s) {
      x(this).raise(), e = x(this).selectAll("path").style("stroke"), t.highlight === void 0 ? x(this).selectAll("path:nth-child(1)").style("opacity", 0.4) : x(this).selectAll("path:nth-child(1)").style("stroke", t.highlight), x(this).selectAll("path:nth-child(2)").style("stroke-width", t.strokeWidth + 1.2), x(this).select(".node-text").attr("opacity", 1);
    }).on("mouseleave", function(s) {
      x(this).selectAll("path:nth-child(1)").style("opacity", 1), x(this).selectAll("path:nth-child(1)").style("stroke", e), x(this).selectAll("path:nth-child(2)").style("stroke-width", t.innerStrokeWidth), x(this).select(".node-text").attr("opacity", 0);
    });
  }
  initRoughObjects() {
    this.roughSvg = document.getElementById(this.roughId), this.rcAxis = T(this.roughSvg, { options: { strokeWidth: this.strokeWidth >= 3 ? 3 : this.strokeWidth } }), this.rc = T(this.roughSvg, { options: { strokeWidth: this.innerStrokeWidth, fill: this.color, stroke: this.stroke === "none" ? void 0 : this.stroke, roughness: this.roughness, bowing: this.bowing, fillStyle: this.fillStyle } });
  }
  drawFromObject() {
    const t = this;
    let e, s;
    if (typeof this.radius == "number")
      e = Z().domain([0, 1]).range([this.extent[0], this.radiusExtent[1]]);
    else {
      const r = ut(this.data, (l) => +l[this.radius]), o = q(this.data, (l) => +l[this.radius]);
      e = Z().domain([r, o]).range([this.radiusExtent[0], this.radiusExtent[1]]);
    }
    if (typeof this.roughness == "number")
      s = Z().domain([0, 1]).range([this.roughnessExtent[0], this.roughnessExtent[1]]);
    else {
      const r = ut(this.data, (l) => +l[this.radius]), o = q(this.data, (l) => +l[this.radius]);
      s = Z().domain([r, o]).range([this.roughnessExtent[0], this.roughnessExtent[1]]);
    }
    this.initRoughObjects();
    const n = this.svg.selectAll(".link").data(this.links).enter().append("line").attr("class", "link"), a = this.svg.selectAll(".nodeGroup").data(this.data).enter().append("g").attr("class", "nodeGroup");
    a.each(function(r, o) {
      const l = typeof t.radius == "number" ? t.radius : e(r[t.radius]), u = typeof t.roughness == "number" ? t.roughness : s(r[t.roughness]), c = t.rc.circle(0, 0, l, { fill: t.colorCallback(r), simplification: t.simplification, fillWeight: t.fillWeight, roughness: u });
      this.appendChild(c), c.setAttribute("class", t.graphClass + "_node"), x(this).append("circle").attr("class", "node-circle").attr("r", 0.5 * l).attr("fill", "transparent").attr("stroke-width", 0).attr("stroke", "none"), x(this).append("text").attr("class", "node-text").attr("x", 0).attr("y", -10).attr("text-anchor", "middle").style("pointer-events", "none").attr("stroke", "black").attr("fill", "white").attr("stroke-linejoin", "fill").attr("paint-order", "stroke fill").attr("stroke-width", "5px").attr("opacity", 0).text((d) => t.textCallback(d));
    });
    const h = Zs(this.data);
    if (h.alpha(1).restart(), h.force("collide", Es().radius((r) => r.radius * this.collision)).force("center", Cs(this.width / 2, this.height / 2)).force("link", function(r) {
      var o, l, u, c, d, g, p = Tr, f = function(v) {
        return 1 / Math.min(c[v.source.index], c[v.target.index]);
      }, A = Zt(30), m = 1;
      function y(v) {
        for (var M = 0, C = r.length; M < m; ++M)
          for (var j, z, F, w, W, E, G, O = 0; O < C; ++O)
            z = (j = r[O]).source, w = (F = j.target).x + F.vx - z.x - z.vx || fi(g), W = F.y + F.vy - z.y - z.vy || fi(g), w *= E = ((E = Math.sqrt(w * w + W * W)) - l[O]) / E * v * o[O], W *= E, F.vx -= w * (G = d[O]), F.vy -= W * G, z.vx += w * (G = 1 - G), z.vy += W * G;
      }
      function b() {
        if (u) {
          var v, M, C = u.length, j = r.length, z = new Map(u.map((F, w) => [p(F, w, u), F]));
          for (v = 0, c = new Array(C); v < j; ++v)
            (M = r[v]).index = v, typeof M.source != "object" && (M.source = _e(z, M.source)), typeof M.target != "object" && (M.target = _e(z, M.target)), c[M.source.index] = (c[M.source.index] || 0) + 1, c[M.target.index] = (c[M.target.index] || 0) + 1;
          for (v = 0, d = new Array(j); v < j; ++v)
            M = r[v], d[v] = c[M.source.index] / (c[M.source.index] + c[M.target.index]);
          o = new Array(j), k(), l = new Array(j), S();
        }
      }
      function k() {
        if (u)
          for (var v = 0, M = r.length; v < M; ++v)
            o[v] = +f(r[v], v, r);
      }
      function S() {
        if (u)
          for (var v = 0, M = r.length; v < M; ++v)
            l[v] = +A(r[v], v, r);
      }
      return r == null && (r = []), y.initialize = function(v, M) {
        u = v, g = M, b();
      }, y.links = function(v) {
        return arguments.length ? (r = v, b(), y) : r;
      }, y.id = function(v) {
        return arguments.length ? (p = v, y) : p;
      }, y.iterations = function(v) {
        return arguments.length ? (m = +v, y) : m;
      }, y.strength = function(v) {
        return arguments.length ? (f = typeof v == "function" ? v : Zt(+v), k(), y) : f;
      }, y.distance = function(v) {
        return arguments.length ? (A = typeof v == "function" ? v : Zt(+v), S(), y) : A;
      }, y;
    }(this.links).distance(100)), h.on("tick", () => {
      a.attr("transform", (r) => `translate(${r.x}, ${r.y})`), n.attr("x1", (r) => r.source.x).attr("y1", (r) => r.source.y).attr("x2", (r) => r.target.x).attr("y2", (r) => r.target.y);
    }), P(".nodeGroup").selectAll("path:nth-child(2)").style("stroke-width", this.strokeWidth), this.interactive === !0 && this.addInteraction(), this.legend) {
      const r = this.legend;
      this.colors = this.legend.map((u) => u.color);
      const o = 6 * r.reduce((u, c) => u > c.text.length ? u : c.text.length, 0) + 35, l = 11 * r.length + 8;
      dt(this, r, o, l);
    }
  }
}
class _r extends ot {
  constructor(t) {
    super(t), this.data = t.data, this.margin = t.margin || { top: 50, right: 20, bottom: 10, left: 20 }, this.colors = t.colors || yt, this.highlight = t.highlight, this.roughness = st({ roughness: t.roughness, ceiling: 30, defaultValue: 0 }), this.strokeWidth = t.strokeWidth || 0.75, this.innerStrokeWidth = t.innerStrokeWidth || 0.75, this.fillWeight = t.fillWeight || 0.85, this.color = t.color || "pink", this.collision = t.collision || 1, this.radiusExtent = t.radiusExtent || [5, 20], this.radius = t.radius || "radius", this.roughnessExtent = t.roughnessExtent || [0, 10], this.responsive = !0, this.boundRedraw = this.redraw.bind(this, t), this.textCallback = t.textCallback || ((e) => ""), this.colorCallback = t.colorCallback || ((e) => this.color), this.legend = t.legend || !1, this.legendPosition = t.legendPosition || "right", this.initChartValues(t), this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title);
  }
  resizeHandler() {
    this.responsive && this.boundRedraw();
  }
  remove() {
    x(this.el).select("svg").remove();
  }
  redraw(t) {
    this.remove(), this.initChartValues(t), this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title);
  }
  initChartValues(t) {
    this.roughness = t.roughness || this.roughness, this.collision = t.collision || this.collision, this.color = t.color || this.color, this.stroke = t.stroke || this.stroke, this.strokeWidth = t.strokeWidth || this.strokeWidth, this.axisStrokeWidth = t.axisStrokeWidth || this.axisStrokeWidth, this.axisRoughness = t.axisRoughness || this.axisRoughness, this.innerStrokeWidth = t.innerStrokeWidth || this.innerStrokeWidth, this.fillWeight = t.fillWeight || this.fillWeight, this.fillStyle = t.fillStyle || this.fillStyle, this.title = t.title || this.title, this.textCallback = t.textCallback || ((a) => "");
    const e = x(this.el).node().getBoundingClientRect(), s = e.width, n = e.height;
    this.width = s - this.margin.left - this.margin.right, this.height = n - this.margin.top - this.margin.bottom, this.roughId = this.el + "_svg", this.graphClass = this.el.substring(1, this.el.length), this.interactionG = "g." + this.graphClass, this.setSvg();
  }
  resolveData(t) {
    return () => {
      this.data = t, this.drawFromObject();
    };
  }
  setTitle(t) {
    this.svg.append("text").attr("x", this.width / 2).attr("y", 0 - this.margin.top / 3).attr("class", "title").attr("text-anchor", "middle").style("font-size", this.titleFontSize === void 0 ? `${Math.min(40, Math.min(this.width, this.height) / 4)}px` : this.titleFontSize).style("font-family", this.fontFamily).style("opacity", 0.8).text(t);
  }
  addInteraction() {
    const t = this;
    let e;
    P(".nodeGroup").on("mouseover", function(s) {
      e = x(this).selectAll("path").style("stroke"), x(this).raise(), t.highlight === void 0 ? x(this).selectAll("path:nth-child(1)").style("opacity", 0.4) : x(this).selectAll("path:nth-child(1)").style("stroke", t.highlight), x(this).selectAll("path:nth-child(2)").style("stroke-width", t.strokeWidth + 1.2), x(this).select(".node-text").attr("opacity", 1), x(this).select(".node-text").raise();
    }).on("mouseleave", function(s) {
      x(this).selectAll("path:nth-child(1)").style("opacity", 1), x(this).selectAll("path:nth-child(1)").style("stroke", e), x(this).selectAll("path:nth-child(2)").style("stroke-width", t.strokeWidth), x(this).select(".node-text").attr("opacity", 0);
    });
  }
  initRoughObjects() {
    this.roughSvg = document.getElementById(this.roughId), this.rcAxis = T(this.roughSvg, { options: { strokeWidth: this.strokeWidth >= 3 ? 3 : this.strokeWidth } }), this.rc = T(this.roughSvg, { options: { strokeWidth: this.innerStrokeWidth, fill: this.color, stroke: this.stroke === "none" ? void 0 : this.stroke, roughness: this.roughness, bowing: this.bowing, fillStyle: this.fillStyle } });
  }
  drawFromObject() {
    const t = this;
    let e, s;
    if (typeof this.radius == "number")
      e = Z().domain([0, 1]).range([this.radiusExtent[0], this.radiusExtent[1]]);
    else {
      const r = ut(this.data, (l) => +l[this.radius]), o = q(this.data, (l) => +l[this.radius]);
      e = Z().domain([r, o]).range([this.radiusExtent[0], this.radiusExtent[1]]);
    }
    if (typeof this.roughness == "number")
      s = Z().domain([0, 1]).range([this.roughnessExtent[0], this.roughnessExtent[1]]);
    else {
      const r = ut(this.data, (l) => +l[this.radius]), o = q(this.data, (l) => +l[this.radius]);
      s = Z().domain([r, o]).range([this.roughnessExtent[0], this.roughnessExtent[1]]);
    }
    this.initRoughObjects();
    let n = this.svg.selectAll(".nodeGroup").data(this.data), a = n.enter().append("g").attr("class", "nodeGroup");
    n = n.merge(a), n.each(function(r, o) {
      const l = typeof t.radius == "number" ? t.radius : e(r[t.radius]), u = typeof t.roughness == "number" ? t.roughness : s(r[t.roughness]), c = t.rc.circle(0, 0, l, { fill: t.colorCallback(r), simplification: t.simplification, fillWeight: t.fillWeight, roughness: u });
      this.appendChild(c).setAttribute("class", t.graphClass + "_node"), x(this).append("circle").attr("class", "node-circle").attr("r", 0.5 * l).attr("fill", "transparent").attr("stroke-width", 0).attr("stroke", "none"), x(this).append("text").attr("class", "node-text").attr("x", 0).attr("y", -10).attr("text-anchor", "middle").style("pointer-events", "none").attr("stroke", "black").attr("fill", "white").attr("stroke-linejoin", "fill").attr("paint-order", "stroke fill").attr("stroke-width", "5px").attr("opacity", 0).text((d) => t.textCallback(d));
    });
    const h = Zs(this.data);
    if (h.alpha(1).restart(), h.force("collide", Es().radius((r) => r.radius * this.collision * 1.2)).force("center", Cs(this.width / 2, this.height / 2)), h.on("tick", () => {
      n.attr("transform", (r) => `translate(${r.x}, ${r.y})`), n.attr("attrX", (r) => +r.x), n.attr("attrY", (r) => +r.y);
    }), P(".nodeGroup").selectAll("path:nth-child(2)").style("stroke-width", this.strokeWidth), this.interactive === !0 && this.addInteraction(), this.legend) {
      const r = this.legend;
      this.colors = this.legend.map((u) => u.color);
      const o = 6 * r.reduce((u, c) => u > c.text.length ? u : c.text.length, 0) + 35, l = 11 * r.length + 8;
      dt(this, r, o, l);
    }
  }
}
class $r extends ot {
  constructor(t) {
    super(t), this.data = t.data, this.margin = t.margin || { top: 50, right: 20, bottom: 10, left: 20 }, this.colors = t.colors || yt, this.highlight = t.highlight, this.roughness = st({ roughness: t.roughness, ceiling: 30, defaultValue: 0 }), this.strokeWidth = t.strokeWidth || 0.75, this.innerStrokeWidth = t.innerStrokeWidth || 0.75, this.fillWeight = t.fillWeight || 0.85, this.labels = this.dataFormat === "object" ? "labels" : t.labels, this.values = this.dataFormat === "object" ? "values" : t.values, this.labels !== void 0 && this.values !== void 0 ? (this.legend = t.legend !== !1, this.legendPosition = t.legendPosition || "right", this.responsive = !0, this.boundRedraw = this.redraw.bind(this, t), this.initChartValues(t), this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title)) : console.log(`Error for ${this.el}: Must include labels and values when        instantiating Donut chart. Skipping chart.`);
  }
  resizeHandler() {
    this.responsive && this.boundRedraw();
  }
  remove() {
    x(this.el).select("svg").remove();
  }
  redraw(t) {
    this.remove(), this.initChartValues(t), this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title);
  }
  initChartValues(t) {
    this.roughness = t.roughness || this.roughness, this.stroke = t.stroke || this.stroke, this.strokeWidth = t.strokeWidth || this.strokeWidth, this.axisStrokeWidth = t.axisStrokeWidth || this.axisStrokeWidth, this.axisRoughness = t.axisRoughness || this.axisRoughness, this.innerStrokeWidth = t.innerStrokeWidth || this.innerStrokeWidth, this.fillWeight = t.fillWeight || this.fillWeight, this.fillStyle = t.fillStyle || this.fillStyle;
    const e = x(this.el).node().getBoundingClientRect(), s = e.width, n = e.height;
    this.width = s - this.margin.left - this.margin.right, this.height = n - this.margin.top - this.margin.bottom, this.roughId = this.el + "_svg", this.graphClass = this.el.substring(1, this.el.length), this.interactionG = "g." + this.graphClass, this.radius = Math.min(this.width, this.height) / 2, this.setSvg();
  }
  resolveData(t) {
    return typeof t != "string" ? () => {
      this.data = t, this.drawFromObject();
    } : t.includes(".csv") ? () => {
      xt(t).then((e) => {
        this.data = e, this.drawFromFile();
      });
    } : t.includes(".tsv") ? () => {
      vt(t).then((e) => {
        this.data = e, this.drawFromFile();
      });
    } : t.includes(".json") ? () => {
      ls(t).then((e) => {
        this.data = e, this.drawFromFile();
      });
    } : void 0;
  }
  setTitle(t) {
    this.svg.append("text").attr("x", this.width / 2).attr("y", 0 - this.margin.top / 3).attr("class", "title").attr("text-anchor", "middle").style("font-size", this.titleFontSize === void 0 ? `${Math.min(40, Math.min(this.width, this.height) / 4)}px` : this.titleFontSize).style("font-family", this.fontFamily).style("opacity", 0.8).text(t);
  }
  addInteraction() {
    P(this.interactionG).append("g").attr("transform", `translate(${this.width / 2}, ${this.height / 2})`).data(this.dataFormat === "object" ? this.makePie(this.data[this.values]) : this.makePie(this.data)).append("path").attr("d", this.makeArc).attr("stroke-width", "0px").attr("fill", "transparent");
    const t = x(this.el).append("div").style("opacity", 0).attr("class", "tooltip").style("position", "absolute").style("background-color", "white").style("border", "solid").style("border-width", "1px").style("border-radius", "5px").style("padding", "3px").style("font-family", this.fontFamily).style("font-size", this.tooltipFontSize).style("pointer-events", "none"), e = this;
    let s;
    P(this.interactionG).on("mouseover", function() {
      t.style("opacity", 1), s = x(this).selectAll("path").style("stroke"), e.highlight === void 0 ? x(this).selectAll("path").style("opacity", 0.5) : x(this).selectAll("path").style("stroke", e.highlight);
    }), P(this.interactionG).on("mouseout", function() {
      t.style("opacity", 0), x(this).selectAll("path").style("stroke", s), x(this).selectAll("path").style("opacity", 1);
    }), P(this.interactionG).on("mousemove", function(n) {
      const a = x(this).attr("attrX"), h = x(this).attr("attrY"), r = bt(this);
      t.html(`<b>${a}</b>: ${h}`).style("opacity", 0.95).style("transform", `translate(${r[0] + e.margin.left}px, 
              ${r[1] - (e.height + e.margin.top + e.margin.bottom / 2)}px)`);
    });
  }
  initRoughObjects() {
    this.roughSvg = document.getElementById(this.roughId), this.rcAxis = T(this.roughSvg, { options: { strokeWidth: this.strokeWidth >= 3 ? 3 : this.strokeWidth } }), this.rc = T(this.roughSvg, { options: { fill: this.color, strokeWidth: this.innerStrokeWidth, roughness: this.roughness, bowing: this.bowing, fillStyle: this.fillStyle } });
  }
  drawFromObject() {
    this.initRoughObjects(), this.makePie = gi(), this.makeArc = di().innerRadius(0).outerRadius(this.radius), this.arcs = this.makePie(this.data[this.values]), this.arcs.forEach((n, a) => {
      if (n.value !== 0) {
        const h = this.rc.arc(this.width / 2, this.height / 2, 2 * this.radius, 2 * this.radius, n.startAngle - Math.PI / 2, n.endAngle - Math.PI / 2, !0, { fill: this.colors[a], stroke: this.colors[a] });
        h.setAttribute("class", this.graphClass);
        const r = this.roughSvg.appendChild(h);
        r.setAttribute("attrY", this.data[this.values][a]), r.setAttribute("attrX", this.data[this.labels][a]);
      }
    }), P(this.interactionG).selectAll("path:nth-child(2)").style("stroke-width", this.strokeWidth);
    const t = this.data.labels.map((n, a) => ({ color: this.colors[a], text: n })), e = 6 * t.reduce((n, a) => n > a.text.length ? n : a.text.length, 0) + 35, s = 11 * t.length + 8;
    this.legend === !0 && dt(this, t, e, s), this.interactive === !0 && this.addInteraction();
  }
  drawFromFile() {
    this.initRoughObjects(), this.makePie = gi().value((a) => a[this.values]).sort(null);
    const t = [];
    this.makeArc = di().innerRadius(0).outerRadius(this.radius), this.arcs = this.makePie(this.data), this.arcs.forEach((a, h) => {
      if (a.value !== 0) {
        const r = this.rc.arc(this.width / 2, this.height / 2, 2 * this.radius, 2 * this.radius, a.startAngle - Math.PI / 2, a.endAngle - Math.PI / 2, !0, { fill: this.colors[h], stroke: this.colors[h] });
        r.setAttribute("class", this.graphClass);
        const o = this.roughSvg.appendChild(r);
        o.setAttribute("attrY", a.data[this.values]), o.setAttribute("attrX", a.data[this.labels]);
      }
      t.push(a.data[this.labels]);
    }), P(this.interactionG).selectAll("path:nth-child(2)").style("stroke-width", this.strokeWidth);
    const e = t.map((a, h) => ({ color: this.colors[h], text: a })), s = 6 * e.reduce((a, h) => a > h.text.length ? a : h.text.length, 0) + 35, n = 11 * e.length + 8;
    this.legend === !0 && dt(this, e, s, n), this.interactive === !0 && this.addInteraction();
  }
}
const Nr = { noisyEmber: `
        background-color: hsla(8, 100%, 60%, 1);
        background-image: url("data:image/svg+xml,%3Csvg viewBox='0 0 1799 1799' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noiseFilter'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.65' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noiseFilter)'/%3E%3C/svg%3E"), radial-gradient(circle at 50% 80%, hsla(18, 100%, 65%, 1) 14%, transparent 60%), radial-gradient(circle at 80% 20%, hsla(28, 100%, 70%, 1) 10%, transparent 70%), radial-gradient(circle at 20% 40%, hsla(3, 90%, 60%, 1) 12%, transparent 75%), radial-gradient(circle at 60% 10%, hsla(14, 95%, 55%, 1) 5%, transparent 80%);
        background-blend-mode: overlay, normal, normal, normal, normal;
    `, noisyLime: `
        background-color: hsla(79, 98%, 66%, 1);
        background-image: url("data:image/svg+xml,%3Csvg viewBox='0 0 1799 1799' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noiseFilter'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.65' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noiseFilter)'/%3E%3C/svg%3E"), radial-gradient(circle at 94% 95%, hsla(170, 88%, 68%, 1) 3%, transparent 76%), radial-gradient(circle at 89% 34%, hsla(263, 96%, 51%, 1) 1%, transparent 86%), radial-gradient(circle at 86% 50%, hsla(318, 80%, 65%, 1) 7%, transparent 76%), radial-gradient(circle at 89% 79%, hsla(248, 76%, 53%, 1) 7%, transparent 87%), radial-gradient(circle at 15% 44%, hsla(4, 74%, 80%, 1) 15%, transparent 81%), radial-gradient(circle at 99% 20%, hsla(75, 88%, 92%, 1) 1%, transparent 68%);
        background-blend-mode: overlay, normal, normal, normal, normal, normal, normal;
    `, softPink: `
        background-color: hsla(320, 70%, 90%, 1);
        background-image: radial-gradient(circle at 30% 70%, hsla(320, 70%, 90%, 1) 0%, hsla(300, 70%, 80%, 0.8) 50%, hsla(280, 60%, 78%, 0.6) 100%);
        background-blend-mode: normal;
    `, noisyDusk: `
        background-color: hsla(266, 30%, 68%, 1);
        background-image: url("data:image/svg+xml,%3Csvg viewBox='0 0 1799 1799' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noiseFilter'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.65' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noiseFilter)'/%3E%3C/svg%3E"), radial-gradient(circle at 40% 80%, hsla(260, 50%, 60%, 1) 9%, transparent 65%), radial-gradient(circle at 75% 25%, hsla(300, 45%, 65%, 1) 10%, transparent 70%), radial-gradient(circle at 15% 45%, hsla(230, 40%, 70%, 1) 8%, transparent 75%), radial-gradient(circle at 60% 10%, hsla(270, 55%, 62%, 1) 6%, transparent 80%);
        background-blend-mode: overlay, normal, normal, normal, normal;
    `, noisyCitrus: `
    background-color: hsla(90, 80%, 85%, 1);
background-image: url("data:image/svg+xml,%3Csvg viewBox='0 0 1799 1799' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noiseFilter'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.65' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noiseFilter)'/%3E%3C/svg%3E"), radial-gradient(circle at 60% 70%, hsla(85, 95%, 70%, 1) 10%, transparent 70%), radial-gradient(circle at 30% 30%, hsla(140, 60%, 75%, 1) 6%, transparent 65%), radial-gradient(circle at 80% 20%, hsla(100, 40%, 80%, 1) 8%, transparent 75%), radial-gradient(circle at 10% 60%, hsla(60, 70%, 85%, 1) 5%, transparent 70%);
background-blend-mode: overlay, normal, normal, normal, normal;`, roseGold: `
    background-color: hsl(25, 80%, 94%);
background-image: radial-gradient(at 100% 0%, hsl(30, 100%, 90%) 0px, transparent 50%), radial-gradient(at 0% 100%, hsl(350, 90%, 92%) 0px, transparent 50%);
`, sunset: `
    background-color: hsla(28, 100%, 85%, 1);
background-image: url("data:image/svg+xml,%3Csvg viewBox='0 0 1799 1799' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noiseFilter'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.65' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noiseFilter)'/%3E%3C/svg%3E"), radial-gradient(circle at 90% 85%, hsla(340, 82%, 70%, 1) 7%, transparent 68%), radial-gradient(circle at 25% 60%, hsla(17, 100%, 70%, 1) 10%, transparent 75%), radial-gradient(circle at 70% 30%, hsla(42, 100%, 80%, 1) 6%, transparent 70%), radial-gradient(circle at 10% 30%, hsla(12, 92%, 60%, 1) 12%, transparent 65%), radial-gradient(circle at 80% 10%, hsla(322, 95%, 65%, 1) 4%, transparent 85%);
background-blend-mode: overlay, normal, normal, normal, normal, normal;
    `, bloom: `
    background-color: hsla(320, 70%, 90%, 1);
background-image: url("data:image/svg+xml,%3Csvg viewBox='0 0 1799 1799' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noiseFilter'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.65' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noiseFilter)'/%3E%3C/svg%3E"), radial-gradient(circle at 70% 75%, hsla(300, 70%, 80%, 1) 10%, transparent 70%), radial-gradient(circle at 20% 30%, hsla(330, 80%, 85%, 1) 8%, transparent 75%), radial-gradient(circle at 60% 20%, hsla(280, 60%, 78%, 1) 6%, transparent 65%), radial-gradient(circle at 15% 80%, hsla(340, 75%, 87%, 1) 7%, transparent 70%);
background-blend-mode: overlay, normal, normal, normal, normal;
    ` };
class ta extends ot {
  constructor(t) {
    super(t), this.margin = t.margin || { top: 20, right: 20, bottom: 20, left: 20 }, this.width = t.width || 200, this.height = t.height || 100, this.roughness = st({ roughness: t.roughness }), this.stroke = t.stroke || "black", this.strokeWidth = t.strokeWidth || 1, this.fillStyle = t.fillStyle || "none", this.fillWeight = t.fillWeight || 0.5, this.backgroundGradient = t.backgroundGradient || "none", this.initChartValues(t), this.resolveFont(), this.drawRectangle(), t.title !== void 0 && this.setTitle(t.title);
  }
  initChartValues(t) {
    this.roughness = t.roughness || this.roughness, this.stroke = t.stroke || this.stroke, this.strokeWidth = t.strokeWidth || this.strokeWidth, this.fillWeight = t.fillWeight || this.fillWeight, this.fillStyle = t.fillStyle || this.fillStyle, this.backgroundGradient = t.backgroundGradient || this.backgroundGradient, this.roughId = this.el + "_svg", this.graphClass = this.el.substring(1, this.el.length), this.setSvg();
  }
  initRoughObjects() {
    this.roughSvg = document.getElementById(this.roughId), this.rc = T(this.roughSvg, { options: { stroke: this.stroke === "none" ? void 0 : this.stroke, strokeWidth: this.strokeWidth, roughness: this.roughness, bowing: this.bowing, fillStyle: this.fillStyle, fillWeight: this.fillWeight } });
  }
  drawRectangle() {
    if (this.initRoughObjects(), this.backgroundGradient !== "none") {
      const e = document.createElementNS("http://www.w3.org/2000/svg", "rect");
      e.setAttribute("x", "0"), e.setAttribute("y", "0"), e.setAttribute("width", this.width.toString()), e.setAttribute("height", this.height.toString()), ((s, n, a) => {
        const h = Nr[s];
        if (!h)
          return void console.warn(`Gradient "${s}" not found`);
        const r = parseFloat(a.getAttribute("x") || "0"), o = parseFloat(a.getAttribute("y") || "0"), l = parseFloat(a.getAttribute("width")), u = parseFloat(a.getAttribute("height"));
        a.setAttribute("fill", "none");
        const c = document.createElementNS("http://www.w3.org/2000/svg", "foreignObject");
        c.setAttribute("x", r.toString()), c.setAttribute("y", o.toString()), c.setAttribute("width", l.toString()), c.setAttribute("height", u.toString());
        const d = document.createElement("div");
        d.style.cssText = `
        width: 100%;
        height: 100%;
        ${h}
    `, c.appendChild(d);
        const g = n.querySelector("foreignObject");
        g && g.remove(), n.appendChild(c);
      })(this.backgroundGradient, this.svg.node(), e);
    }
    const t = this.rc.rectangle(0, 0, this.width, this.height, { stroke: this.stroke === "none" ? void 0 : this.stroke, strokeWidth: this.strokeWidth, roughness: this.roughness, fillWeight: this.fillWeight, fill: "none" });
    this.svg.node().appendChild(t);
  }
  setTitle(t) {
    this.svg.append("text").attr("x", this.width / 2).attr("y", -this.margin.top / 2).attr("text-anchor", "middle").attr("font-family", this.fontFamily).attr("font-size", this.titleFontSize).text(t);
  }
}
const Gi = ["pink", "skyblue", "coral", "gold", "teal", "darkgreen", "brown", "slateblue", "orange"];
class ia extends ot {
  constructor(t) {
    super(t), this.margin = t.margin || { top: 50, right: 20, bottom: 50, left: 100 }, this.colorVar = t.colorVar, this.roughness = st({ roughness: t.roughness }), this.highlight = t.highlight, this.highlightLabel = t.highlightLabel || "xy", this.radiusExtent = t.radiusExtent || [5, 20], this.radius = t.radius || 20, this.axisStrokeWidth = t.axisStrokeWidth || 0.4, this.axisRoughness = t.axisRoughness || 0.9, this.curbZero = t.curbZero === !0, this.innerStrokeWidth = t.innerStrokeWidth || 1, this.stroke = t.stroke || "black", this.fillWeight = t.fillWeight || 0.85, this.colors = t.colors || Gi, this.strokeWidth = t.strokeWidth || 1, this.axisFontSize = t.axisFontSize, this.x = this.dataFormat === "object" ? "x" : t.x, this.y = this.dataFormat === "object" ? "y" : t.y, this.xValueFormat = t.xValueFormat, this.yValueFormat = t.yValueFormat, this.xLabel = t.xLabel || "", this.yLabel = t.yLabel || "", this.labelFontSize = t.labelFontSize || "1rem", this.responsive = !0, this.boundRedraw = this.redraw.bind(this, t), this.radiusScale, this.initChartValues(t), this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title), window.addEventListener("resize", this.resizeHandler.bind(this));
  }
  resizeHandler() {
    this.responsive && this.boundRedraw();
  }
  remove() {
    x(this.el).select("svg").remove();
  }
  redraw(t) {
    this.remove(), this.initChartValues(t), this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart();
  }
  initChartValues(t) {
    this.roughness = t.roughness || this.roughness, this.stroke = t.stroke || this.stroke, this.strokeWidth = t.strokeWidth || this.strokeWidth, this.axisStrokeWidth = t.axisStrokeWidth || this.axisStrokeWidth, this.axisRoughness = t.axisRoughness || this.axisRoughness, this.innerStrokeWidth = t.innerStrokeWidth || this.innerStrokeWidth, this.fillWeight = t.fillWeight || this.fillWeight, this.fillStyle = t.fillStyle || this.fillStyle, this.colors = t.colors || this.colors;
    const e = x(this.el).node().getBoundingClientRect(), s = e.width, n = e.height;
    this.width = s - this.margin.left - this.margin.right, this.height = n - this.margin.top - this.margin.bottom, this.roughId = this.el + "_svg", this.graphClass = this.el.substring(1, this.el.length), this.interactionG = "g." + this.graphClass, this.setSvg();
  }
  resolveData(t) {
    return typeof t != "string" ? () => {
      this.data = t, this.drawFromObject();
    } : t.includes(".csv") ? () => {
      xt(t).then((e) => {
        this.data = e, this.drawFromFile();
      });
    } : t.includes(".tsv") ? () => {
      vt(t).then((e) => {
        this.data = e, this.drawFromFile();
      });
    } : void 0;
  }
  addScaleLine() {
    let t, e;
    if (this.dataFormat !== "file")
      t = allDataExtent(this.data);
    else {
      const a = this.dataSources.map((o) => et(this.data, (l) => +l[o])), h = ut(a, (o) => o[0]), r = q(a, (o) => o[1]);
      t = [h, r];
    }
    if (this.x === void 0) {
      const a = q(Object.keys(this.data).map((h) => this.data[h].length));
      e = this.dataFormat === "file" ? [0, this.data.length] : [0, a];
    } else
      e = et(this.x);
    const s = t, n = s[1] - s[0];
    this.xScale = this.x === void 0 ? scalePoint().range([0, this.width]).domain([...Array(e[1]).keys()]) : scalePoint().range([0, this.width]).domain(this.x), this.yScale = Z().range([this.height, 0]).domain([0, s[1] + 0.05 * n]);
  }
  addScales() {
    const t = this.dataFormat === "file" ? et(this.data, (h) => +h[this.x]) : et(this.data[this.x]), e = t[1] - t[0], s = this.dataFormat === "file" ? et(this.data, (h) => +h[this.y]) : et(this.data[this.y]), n = s[1] - s[0], a = this.dataFormat === "file" ? et(this.data, (h) => h[this.colorVar]) : [1, 1];
    if (this.dataFormat === "file") {
      const h = et(this.data, (o) => +o[this.radius]), r = Math.min(this.width, this.height) / 2 / 2;
      this.radiusScale = Z().range([8, r]).domain(h);
    } else
      this.radiusScale = Z().domain([0, 20]).range([this.radiusExtent[0], this.radiusExtent[1]]);
    this.curbZero === !0 && (s[0] > 0 && (s[0] = 0), t[0] > 0 && (t[0] = 0)), this.xScale = Z().range([0, this.width]).domain([t[0] - 0.05 * e, t[1] + 0.05 * e]), this.yScale = Z().range([this.height, 0]).domain([s[0] - 0.05 * n, s[1] + 0.05 * n]), this.colorScale = mi().range(this.colors).domain(a);
  }
  addLabels() {
    this.xLabel !== "" && this.svg.append("text").attr("x", this.width / 2).attr("y", this.height + this.margin.bottom / 1.3).attr("dx", "1em").attr("class", "labelText").style("text-anchor", "middle").style("font-family", this.fontFamily).style("font-size", this.labelFontSize).text(this.xLabel), this.yLabel !== "" && this.svg.append("text").attr("transform", "rotate(-90)").attr("y", 0 - this.margin.left / 2).attr("x", 0 - this.height / 2).attr("dy", "1em").attr("class", "labelText").style("text-anchor", "middle").style("font-family", this.fontFamily).style("font-size", this.labelFontSize).text(this.yLabel);
  }
  addAxes() {
    const t = Yt(this.xScale).tickSize(0).tickFormat((s) => this.xValueFormat ? ht(this.xValueFormat)(s) : s), e = Dt(this.yScale).tickSize(0).tickFormat((s) => this.yValueFormat ? ht(this.yValueFormat)(s) : s);
    this.svg.append("g").attr("transform", "translate(0," + this.height + ")").call(t).attr("class", `xAxis${this.graphClass}`).selectAll("text").attr("transform", "translate(-10, 0)rotate(-45)").style("text-anchor", "end").style("font-family", this.fontFamily).style("font-size", this.axisFontSize === void 0 ? `${Math.min(0.95, Math.min(this.width, this.height) / 140)}rem` : this.axisFontSize), this.svg.append("g").call(e).attr("class", `yAxis${this.graphClass}`).selectAll("text").style("font-family", this.fontFamily).style("font-size", this.axisFontSize === void 0 ? `${Math.min(0.95, Math.min(this.width, this.height) / 140)}rem` : this.axisFontSize), P("path.domain").attr("stroke", "transparent"), P("g.tick").style("opacity", 1);
  }
  makeAxesRough(t, e) {
    const s = `xAxis${this.graphClass}`, n = `yAxis${this.graphClass}`, a = `rough-${s}`, h = `rough-${n}`;
    x(`.${s}`).selectAll("path.domain").each(function(r, o) {
      const l = x(this).node().getAttribute("d"), u = e.path(l, { stroke: "black", fillStyle: "hachure" });
      u.setAttribute("class", a), t.appendChild(u);
    }), P(`.${a}`).attr("transform", `translate(0, ${this.height})`), x(`.${n}`).selectAll("path.domain").each(function(r, o) {
      const l = x(this).node().getAttribute("d"), u = e.path(l, { stroke: "black", fillStyle: "hachure" });
      u.setAttribute("class", h), t.appendChild(u);
    });
  }
  setTitle(t) {
    this.svg.append("text").attr("x", this.width / 2).attr("y", 0 - this.margin.top / 2).attr("text-anchor", "middle").style("font-size", this.titleFontSize === void 0 ? `${Math.min(20, Math.min(this.width, this.height) / 4)}px` : this.titleFontSize).style("font-family", this.fontFamily).style("opacity", 0.8).text(t);
  }
  addInteraction() {
    const t = P(this.interactionG).data(this.dataFormat === "file" ? this.data : this.data.x).append("circle").attr("cx", (a, h) => this.dataFormat === "file" ? this.xScale(+a[this.x]) : this.xScale(+this.data[this.x][h])).attr("cy", (a, h) => this.dataFormat === "file" ? this.yScale(+a[this.y]) : this.yScale(+this.data[this.y][h]));
    this.dataFormat === "file" ? t.attr("r", (a) => typeof this.radius == "number" ? 0.7 * this.radius : 0.6 * this.radiusScale(+a[this.radius])).attr("fill", "transparent") : t.attr("r", (a, h) => {
      const r = this.data[this.radius][h];
      return typeof this.radius == "number" ? 0.7 * this.radius : this.radiusScale(r);
    }).attr("fill", "transparent");
    let e = x(this.el).append("div").style("opacity", 0).attr("class", "tooltip").style("position", "absolute").style("background-color", "white").style("border", "solid").style("border-width", "1px").style("border-radius", "5px").style("padding", "3px").style("font-family", this.fontFamily).style("font-size", this.tooltipFontSize).style("pointer-events", "none");
    const s = this;
    let n;
    P(this.interactionG).on("mouseover", function() {
      e.style("opacity", 1), n = x(this).selectAll("path").style("stroke"), s.highlight === void 0 ? x(this).selectAll("path:nth-child(1)").style("opacity", 0.4) : x(this).selectAll("path:nth-child(1)").style("stroke", s.highlight), x(this).selectAll("path:nth-child(2)").style("stroke-width", s.strokeWidth + 1.2);
    }), P(this.interactionG).on("mouseout", function() {
      e.style("opacity", 0), x(this).selectAll("path").style("opacity", 1), x(this).selectAll("path:nth-child(1)").style("stroke", n), x(this).selectAll("path:nth-child(2)").style("stroke", s.stroke), x(this).selectAll("path:nth-child(2)").style("stroke-width", s.strokeWidth);
    }), P(this.interactionG).on("mousemove", function(a) {
      const h = x(this).attr("attrX"), r = x(this).attr("attrY"), o = x(this).attr("attrHighlightLabel"), l = bt(this);
      e.html(s.highlightLabel === "xy" ? `<b>x</b>: ${h} <br><b>y</b>: ${r}` : `<b>${o}</b>`).attr("class", function(u) {
      }).style("transform", `translate(${l[0] + s.margin.left}px, 
          ${l[1] - (s.height + s.margin.top + s.margin.bottom / 2)}px)`);
    });
  }
  initRoughObjects() {
    this.roughSvg = document.getElementById(this.roughId), this.rcAxis = T(this.roughSvg, { options: { strokeWidth: this.axisStrokeWidth, roughness: this.axisRoughness } }), this.rc = T(this.roughSvg, { options: { stroke: this.stroke === "none" ? void 0 : this.stroke, strokeWidth: this.innerStrokeWidth, roughness: this.roughness, bowing: this.bowing, fillStyle: this.fillStyle } });
  }
  drawFromObject() {
    const t = this;
    let e;
    if (this.radiusScale = Z().domain([0, 20]).range([this.radiusExtent[0], this.radiusExtent[1]]), typeof this.radius == "number")
      e = Z().domain([0, 1]).range([this.radiusExtent[0], this.radiusExtent[1]]);
    else {
      const s = ut(this.data[this.radius]), n = q(this.data[this.radius]);
      e = Z().domain([s, n]).range([this.radiusExtent[0], this.radiusExtent[1]]);
    }
    typeof this.colors == "string" && (this.colors = this.colors), this.colors === void 0 && (this.colors = Gi[0]), this.initRoughObjects(), this.addScales(), this.addAxes(), this.makeAxesRough(this.roughSvg, this.rcAxis), this.addLabels(), this.data.x.forEach((s, n) => {
      const a = typeof t.radius == "number" ? t.radius : e(+this.data[t.radius][n]), h = this.rc.circle(this.xScale(+s), this.yScale(+this.data[this.y][n]), a, { fill: typeof this.colors == "string" ? this.colors : this.colors.length === 1 ? this.colors[0] : this.colors[n], simplification: this.simplification, fillWeight: this.fillWeight }), r = this.roughSvg.appendChild(h);
      r.setAttribute("class", this.graphClass), r.setAttribute("attrX", s), r.setAttribute("attrY", this.data[this.y][n]), r.setAttribute("attrHighlightLabel", this.data[this.highlightLabel]);
    }), P(this.interactionG).selectAll("path:nth-child(2)").style("stroke-width", this.strokeWidth), this.interactive === !0 && this.addInteraction();
  }
  drawFromFile() {
    this.colors === void 0 && (this.colors = Gi), this.initRoughObjects(), this.addScales(), this.addAxes(), this.makeAxesRough(this.roughSvg, this.rcAxis), this.addLabels(), this.data.forEach((t, e) => {
      const s = this.rc.circle(this.xScale(+t[this.x]), this.yScale(+t[this.y]), typeof this.radius == "number" ? this.radius : this.radiusScale(+t[this.radius]), { fill: this.colorVar === void 0 ? this.colors[0] : this.colorScale(t[this.colorVar]), simplification: this.simplification, fillWeight: this.fillWeight }), n = this.roughSvg.appendChild(s);
      n.setAttribute("class", this.graphClass), n.setAttribute("attrX", t[this.x]), n.setAttribute("attrY", t[this.y]), n.setAttribute("attrHighlightLabel", t[this.highlightLabel]);
    }), P(this.interactionG).selectAll("path:nth-child(2)").style("stroke-width", this.strokeWidth), this.interactive === !0 && this.addInteraction();
  }
}
class ea extends ot {
  constructor(t) {
    super(t), this.data = t.data, this.margin = t.margin || { top: 50, right: 20, bottom: 70, left: 100 }, this.color = t.color || "red", this.highlight = t.highlight || "coral", this.roughness = st({ roughness: t.roughness }), this.stroke = t.stroke || "black", this.strokeWidth = t.strokeWidth || 1, this.axisStrokeWidth = t.axisStrokeWidth || 0.5, this.axisRoughness = t.axisRoughness || 0.5, this.innerStrokeWidth = t.innerStrokeWidth || 1, this.fillWeight = t.fillWeight || 0.5, this.axisFontSize = t.axisFontSize, this.labels = t.labels, this.values = t.values, this.stackColorMapping = {}, this.padding = t.padding || 0.1, this.xLabel = t.xLabel || "", this.yLabel = t.yLabel || "", this.labelFontSize = t.labelFontSize || "1rem", this.responsive = !0, this.boundRedraw = this.redraw.bind(this, t), this.initChartValues(t), this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title), window.addEventListener("resize", this.resizeHandler.bind(this));
  }
  resizeHandler() {
    this.responsive && this.boundRedraw();
  }
  remove() {
    x(this.el).select("svg").remove();
  }
  redraw(t) {
    this.remove(), this.initChartValues(t), this.resolveFont(), this.drawChart = this.resolveData(t.data), this.drawChart(), t.title !== "undefined" && this.setTitle(t.title);
  }
  initChartValues(t) {
    this.roughness = t.roughness || this.roughness, this.stroke = t.stroke || this.stroke, this.strokeWidth = t.strokeWidth || this.strokeWidth, this.axisStrokeWidth = t.axisStrokeWidth || this.axisStrokeWidth, this.axisRoughness = t.axisRoughness || this.axisRoughness, this.innerStrokeWidth = t.innerStrokeWidth || this.innerStrokeWidth, this.fillWeight = t.fillWeight || this.fillWeight, this.fillStyle = t.fillStyle || this.fillStyle;
    const e = x(this.el).node().getBoundingClientRect(), s = e.width, n = e.height;
    this.width = s - this.margin.left - this.margin.right, this.height = n - this.margin.top - this.margin.bottom, this.roughId = this.el + "_svg", this.graphClass = this.el.substring(1, this.el.length), this.interactionG = "g." + this.graphClass, this.setSvg();
  }
  getTotal(t) {
    for (let e = 0; e < t.length; e++) {
      let s = 0;
      for (let n = 0; n < t.columns.length; ++n)
        t.columns[n] !== this.labels && (s += t[e][t.columns[n]] = +t[e][t.columns[n]]);
      t[e].total = s;
    }
    return t;
  }
  updateColorMapping(t) {
    this.stackColorMapping[t] || (this.stackColorMapping[t] = yt[Object.keys(this.stackColorMapping).length]);
  }
  resolveData(t) {
    return typeof t != "string" ? () => {
      this.data = t, t = t.map((e) => (Object.keys(e).includes("total") && (e.total = 0), e));
      for (let e = 0; e < t.length; ++e) {
        let s = 0;
        Object.keys(t[e]).forEach((n) => {
          n !== this.labels && n !== "total" && (this.updateColorMapping(n), s += t[e][n]);
        }), t[e].total = s;
      }
      this.drawFromObject();
    } : t.includes(".csv") ? () => {
      xt(t).then((e) => {
        this.getTotal(e), this.data = e, this.drawFromFile();
      });
    } : t.includes(".tsv") ? () => {
      vt(t).then((e) => {
        this.getTotal(e), this.data = e, this.drawFromFile();
      });
    } : void 0;
  }
  addScales() {
    this.xScale = qt().rangeRound([0, this.width]).padding(this.padding).domain(this.data.map((e) => e[this.labels])), this.yScale = Z().rangeRound([this.height, 0]).domain([0, q(this.data, (e) => e.total)]).nice();
    const t = this.dataFormat === "object" ? this.data.map((e) => e[this.labels]) : this.data.columns;
    this.zScale = mi().range(["#98abc5", "#8a89a6", "#7b6888", "#6b486b", "#a05d56", "#d0743c", "#ff8c00"]).domain(t);
  }
  addLabels() {
    this.xLabel !== "" && this.svg.append("text").attr("x", this.width / 2).attr("y", this.height + this.margin.bottom / 2).attr("dx", "1em").attr("class", "labelText").style("text-anchor", "middle").style("font-family", this.fontFamily).style("font-size", this.labelFontSize).text(this.xLabel), this.yLabel !== "" && this.svg.append("text").attr("transform", "rotate(-90)").attr("y", 0 - this.margin.left / 1.4).attr("x", 0 - this.height / 2).attr("dy", "1em").attr("class", "labelText").style("text-anchor", "middle").style("font-family", this.fontFamily).style("font-size", this.labelFontSize).text(this.yLabel);
  }
  addAxes() {
    const t = Yt(this.xScale).tickSize(0);
    this.svg.append("g").attr("transform", "translate(0," + this.height + ")").call(t).attr("class", `xAxis${this.graphClass}`).selectAll("text").attr("transform", "translate(-10,0)rotate(-45)").style("text-anchor", "end").style("font-family", this.fontFamily).style("font-size", this.axisFontSize === void 0 ? `${Math.min(0.8, Math.min(this.width, this.height) / 140)}rem` : this.axisFontSize).style("opacity", 0.9);
    const e = Dt(this.yScale).tickSize(0);
    this.svg.append("g").call(e).attr("class", `yAxis${this.graphClass}`).selectAll("text").style("font-family", this.fontFamily).style("font-size", this.axisFontSize === void 0 ? `${Math.min(0.95, Math.min(this.width, this.height) / 140)}rem` : this.axisFontSize).style("opacity", 0.9), P("path.domain").attr("stroke", "transparent");
  }
  makeAxesRough(t, e) {
    const s = `xAxis${this.graphClass}`, n = `yAxis${this.graphClass}`, a = `rough-${s}`, h = `rough-${n}`;
    x(`.${s}`).selectAll("path.domain").each(function(r, o) {
      const l = x(this).node().getAttribute("d"), u = e.path(l, { fillStyle: "hachure" });
      u.setAttribute("class", a), t.appendChild(u);
    }), P(`.${a}`).attr("transform", `translate(0, ${this.height})`), x(`.${n}`).selectAll("path.domain").each(function(r, o) {
      const l = x(this).node().getAttribute("d"), u = e.path(l, { fillStyle: "hachure" });
      u.setAttribute("class", h), t.appendChild(u);
    });
  }
  setTitle(t) {
    this.svg.append("text").attr("x", this.width / 2).attr("y", 0 - this.margin.top / 2).attr("class", "title").attr("text-anchor", "middle").style("font-size", this.titleFontSize === void 0 ? `${Math.min(40, Math.min(this.width, this.height) / 5)}px` : this.titleFontSize).style("font-family", this.fontFamily).style("opacity", 0.8).text(t);
  }
  addInteraction() {
    P(this.interactionG).each(function(n, a) {
      const h = this.attributes;
      x(this).append("rect").attr("x", h.x.value).attr("y", h.y.value).attr("width", h.width.value).attr("height", h.height.value).attr("fill", "transparent");
    });
    const t = x(this.el).append("div").style("opacity", 0).attr("class", "tooltip").style("position", "absolute").style("background-color", "white").style("border", "solid").style("border-width", "1px").style("border-radius", "5px").style("padding", "3px").style("font-family", this.fontFamily).style("font-size", this.tooltipFontSize).style("pointer-events", "none"), e = this;
    let s;
    P(this.interactionG).on("mouseover", function() {
      t.style("opacity", 1), s = x(this).selectAll("path").style("stroke"), x(this).select("path").style("stroke", e.highlight), x(this).selectAll("path:nth-child(2)").style("stroke-width", e.strokeWidth + 1.2);
    }), P(this.interactionG).on("mouseout", function() {
      t.style("opacity", 0), x(this).select("path").style("stroke", s), x(this).selectAll("path:nth-child(2)").style("stroke-width", e.strokeWidth);
    }), P(this.interactionG).on("mousemove", function(n) {
      const a = x(this).attr("attrX"), h = x(this).attr("attrY"), r = bt(this);
      t.html(`<b>${a}</b>: ${h}`).style("opacity", 0.95).attr("class", function(o) {
      }).style("transform", `translate(${r[0] + e.margin.left}px, 
          ${r[1] - (e.height + e.margin.top + e.margin.bottom / 2)}px)`);
    });
  }
  initRoughObjects() {
    this.roughSvg = document.getElementById(this.roughId), this.rcAxis = T(this.roughSvg, { options: { strokeWidth: this.axisStrokeWidth, roughness: this.axisRoughness } }), this.rc = T(this.roughSvg, { options: { stroke: this.stroke === "none" ? void 0 : this.stroke, strokeWidth: this.innerStrokeWidth, roughness: this.roughness, bowing: this.bowing, fillStyle: this.fillStyle } });
  }
  stacking() {
    this.data.forEach((t) => {
      const e = Object.keys(t);
      let s = 0;
      e.forEach((n, a) => {
        if (a > 0 && n !== "total") {
          s += parseInt(t[n], 10);
          const h = this.xScale(t[this.labels]), r = this.yScale(s), o = this.xScale.bandwidth(), l = this.height - this.yScale(+t[n]), u = this.rc.rectangle(h, r, o, l, { fill: this.stackColorMapping[n] || this.colors[a], stroke: this.stackColorMapping[n] || this.colors[a], simplification: this.simplification, fillWeight: this.fillWeight }), c = this.roughSvg.appendChild(u);
          c.setAttribute("class", this.graphClass), c.setAttribute("attrX", t[this.labels]), c.setAttribute("keyY", n), c.setAttribute("attrY", +t[n]), c.setAttribute("x", h), c.setAttribute("y", r), c.setAttribute("width", o), c.setAttribute("height", l);
        }
      });
    });
  }
  drawFromObject() {
    this.initRoughObjects(), this.addScales(), this.addAxes(), this.makeAxesRough(this.roughSvg, this.rcAxis), this.addLabels(), this.stacking(), P(this.interactionG).selectAll("path:nth-child(2)").style("stroke-width", this.strokeWidth), this.interactive === !0 && this.addInteraction();
  }
  drawFromFile() {
    this.initRoughObjects(), this.addScales(), this.addAxes(), this.makeAxesRough(this.roughSvg, this.rcAxis), this.addLabels(), this.stacking(), P(this.interactionG).selectAll("path:nth-child(2)").style("stroke-width", this.strokeWidth), this.interactive === !0 && this.addInteraction();
  }
}
export {
  Xr as Bar,
  Kr as BarH,
  Qr as Donut,
  _r as Force,
  Ur as Line,
  Jr as Network,
  $r as Pie,
  ta as Rectangle,
  ia as Scatter,
  ea as StackedBar
};
